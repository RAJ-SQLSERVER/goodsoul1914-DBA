...
CREATE OR ALTER PROCEDURE dbo.sp_indexdetails (@databasename sysname = NULL,
                                               @tablename    sysname,
                                               @debug        TINYINT = 0
)
AS
...
SELECT 
    ...
    partition_sums.in_row_data_page_count,
...
JOIN '      + QUOTENAME(@databasename) + N'.sys.tables AS t ON si.object_id=t.object_id
JOIN '      + QUOTENAME(@databasename) + N'.sys.schemas AS sc ON t.schema_id=sc.schema_id
LEFT JOIN ' + QUOTENAME(@databasename)
            + N'.sys.dm_db_index_usage_stats AS stat ON 
    stat.database_id = DB_ID() 
    and si.object_id=stat.object_id 
    and si.index_id=stat.index_id
LEFT JOIN ' + QUOTENAME(@databasename)
            + N'.sys.partition_schemes AS psc ON si.data_space_id=psc.data_space_id
LEFT JOIN ' + QUOTENAME(@databasename)
            + N'.sys.partition_functions AS pf ON psc.function_id=pf.function_id
LEFT JOIN ' + QUOTENAME(@databasename)
            + N'.sys.filegroups AS fg ON si.data_space_id=fg.data_space_id
...
/* Partitions */ OUTER APPLY ( 
    ...
    FROM '  + QUOTENAME(@databasename) + N'.sys.partitions AS p
    JOIN '  + QUOTENAME(@databasename)
            + N'.sys.dm_db_partition_stats AS ps ON
