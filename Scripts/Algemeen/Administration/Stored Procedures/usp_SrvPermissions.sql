USE DBA;
GO
IF OBJECT_ID('dbo.usp_SrvPermissions') IS NULL
...
https://github.com/sqlstudent144/SQL-Server-Scripts/blob/master/usp_SrvPermissions.sql
...
*********************************************************************************************/
ALTER PROCEDURE dbo.usp_SrvPermissions
(
...
SELECT @Version2005orLower = CASE
                                 WHEN PARSENAME(CAST(SERVERPROPERTY('productversion') AS VARCHAR(20)), 4) < 10 THEN
                                     1
                                 ELSE
                                     0
                             END;
...
SET @sql
    = N'SELECT Logins.principal_id AS SrvPrincipalId, Logins.name AS SrvPrincipal, Logins.type, Logins.type_desc, 
      ...
      + CASE
            WHEN @Version2005orLower = 0 THEN
                N','' +  
      ...
      + N'
               ...
               CASE WHEN Logins.type IN (''S'',''U'',''G'') THEN -- Note: Types, S, U and G are the only ones that have additional options.
                   ...
                   ELSE '''' END +
                   ISNULL('' DEFAULT_DATABASE = '' + QUOTENAME(Logins.default_database_name' + @Collation
      + N'), '''') + 
                   CASE WHEN Logins.default_database_name IS NOT NULL AND Logins.default_language_name IS NOT NULL THEN '','' ELSE '''' END + 
                   ISNULL('' DEFAULT_LANGUAGE = '' + QUOTENAME(Logins.default_language_name' + @Collation
      + N'), '''') +
...
IF LEN(ISNULL(@Principal, @Role)) > 0
    IF @Print = 1
        SET @sql
            = @sql + NCHAR(13) + N'  AND Logins.name ' + @LikeOperator + N' '
...
IF @DBName IS NOT NULL
    SET @sql
        = @sql + NCHAR(13) + N'  AND Logins.SID IN (SELECT SID FROM [' + @DBName
          + N'].sys.database_principals 
                                                        WHERE type IN (''G'',''S'',''U'',''K'',''C''))';

IF @IncludeMSShipped = 0
    SET @sql
        = @sql + NCHAR(13) + N'  AND Logins.is_fixed_role = 0 ' + NCHAR(13)
          + N'  AND Logins.name NOT IN (''sa'',''public'') ';
...
BEGIN
    ...
    EXEC sp_executesql @sql,
                       N'@Principal sysname, @Role sysname, @Type varchar(30)',
                       @Principal,
                       @Role,
                       @Type;
END;
...
SET @sql
    = N'SELECT Logins.principal_id AS LoginPrincipalId, Logins.name AS LoginName, Roles.name AS RoleName, 
      ...
      + N','''''''')+'', @rolename = ''+QUOTENAME(Roles.name' + @Collation
      + N','''''''') + '';'' END AS DropScript, 
       CASE WHEN Logins.principal_id < 100 THEN NULL ELSE 
...
BEGIN
    ...
    SET @sql = N'INSERT INTO ##SrvRoles ' + NCHAR(13) + @sql;
...
IF @DBName IS NOT NULL
    SET @sql
        = @sql + NCHAR(13) + N' AND Grantee.SID IN (SELECT SID FROM [' + @DBName
          + N'].sys.database_principals 
                                    WHERE type IN (''G'',''S'',''U'',''K'',''C''))';
...
IF @IncludeMSShipped = 0
    SET @sql
        = @sql + NCHAR(13) + N'  AND Grantee.is_fixed_role = 0 ' + NCHAR(13)
          + N'  AND Grantee.name NOT IN (''sa'',''public'') ';
...
BEGIN
    ...
    BEGIN
        SELECT SrvPrincipal,
               ...
               STUFF(
                        (
                            SELECT ', ' + ##SrvPermissions.state_desc + ' ' + ##SrvPermissions.permission_name + ' '
                                   + CASE
                                     ...
                                     END
                            FROM
                            (SELECT DISTINCT * FROM ##SrvPermissions) ##SrvPermissions
                            WHERE ##SrvPrincipals.SrvPrincipalId = ##SrvPermissions.GranteePrincipalId
                            ORDER BY ##SrvPermissions.state_desc,
                                     ##SrvPermissions.permission_name
                            FOR XML PATH(''), TYPE
                        ).value('.', 'VARCHAR(MAX)'),
                        1,
                        2,
