...
--:setvar path "D:\Dropbox\SQL Server\Documents\Mark Boomaars\DBA\Triggers"
--:r $(path)\trg_BlockMasterUserObjects.sql
--GO
...
:setvar path "D:\Dropbox\SQL Server\Documents\Mark Boomaars\DBA\Functions"
:r $(path)\GetNums.sql
GO
...
-------------------------------------------------------------------------------
-- Create database management jobs
-------------------------------------------------------------------------------

-- DBA: Fix plan cache bloat
DECLARE @jobId BINARY(16);
...
IF
(
    SELECT FailSafeOperator FROM @AlertInfo
) IS NULL
...
DECLARE @Error855AlertName sysname
    = @ServerName + N' Alert - Error 855: Uncorrectable hardware memory corruption detected';
