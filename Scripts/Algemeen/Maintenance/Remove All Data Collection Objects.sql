-- Remove All Data Collection Objects
---------------------------------------------------------------------------------------------------
USE MSDB;
GO
...
FOR
SELECT collection_job_id AS job_id
FROM syscollector_collection_sets
WHERE collection_job_id IS NOT NULL

UNION

SELECT upload_job_id AS job_id
FROM syscollector_collection_sets
...
FETCH NEXT
FROM datacollector_jobs_cursor
INTO @job_id;
...
BEGIN
	...
	BEGIN
		...
		EXEC dbo.sp_delete_job @job_id = @job_id,
