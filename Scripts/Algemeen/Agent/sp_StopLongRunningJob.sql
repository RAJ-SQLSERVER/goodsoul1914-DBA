...
IF OBJECT_ID('dbo.sp_StopLongRunningJob') IS NULL
	EXECUTE ('CREATE PROCEDURE dbo.sp_StopLongRunningJob AS SELECT 1 as DummyCode;');
GO

-- EXEC dbo.sp_StopLongRunningJob @p_JobName = 'CW Labeling Staging', @p_TimeLimit_Minutes = 180

ALTER PROCEDURE [dbo].[sp_StopLongRunningJob] @p_JobName VARCHAR(125), @p_TimeLimit_Minutes INT = 180, @p_ForceJobStop BIT = 1 , @p_recipients VARCHAR(1000) = 'It-Ops-DBA@contso.com', @p_cc VARCHAR(1000) = NULL
AS
BEGIN
	...
	*/
	...
	IF OBJECT_ID('WhatIsRunning') IS NOT NULL
	BEGIN
		DELETE WhatIsRunning WHERE [Source] = ('DBA Check ' + @p_JobName) AND [CollectionTime] <= DATEADD(day,-7,GETDATE());
	END
	...
	BEGIN
		BEGIN TRY
			--SELECT * FROM WhatIsRunning;
			INSERT WhatIsRunning
