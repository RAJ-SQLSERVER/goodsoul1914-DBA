...
INNER JOIN (
	SELECT j.name,
		...
		CASE LEN(CONVERT(VARCHAR(5), DATEDIFF(MI, JA.RUN_REQUESTED_DATE, GETDATE()) / 60))
			WHEN 1
				THEN '0' + CONVERT(VARCHAR(5), DATEDIFF(mi, ja.run_requested_date, GETDATE()) / 60)
			...
			WHEN 1
				THEN '0' + CONVERT(VARCHAR(5), DATEDIFF(mi, ja.run_requested_date, GETDATE()) % 60)
			ELSE CONVERT(VARCHAR(5), DATEDIFF(mi, ja.run_requested_date, GETDATE()) % 60)
			END + ':' + CASE LEN(CONVERT(VARCHAR(5), DATEDIFF(SS, JA.RUN_REQUESTED_DATE, GETDATE()) % 60))
			WHEN 1
				THEN '0' + CONVERT(VARCHAR(5), DATEDIFF(ss, ja.run_requested_date, GETDATE()) % 60)
			ELSE CONVERT(VARCHAR(5), DATEDIFF(ss, ja.run_requested_date, GETDATE()) % 60)
			END AS RunningForTime
	FROM msdb.dbo.sysjobactivity AS ja
	LEFT OUTER JOIN msdb.dbo.sysjobhistory AS jh ON ja.job_history_id = jh.instance_id
	INNER JOIN msdb.dbo.sysjobs_view AS j ON ja.job_id = j.job_id
	WHERE ja.session_id = (
			SELECT MAX(session_id) AS EXPR1
			FROM msdb.dbo.sysjobactivity
			)
	) AS R ON R.job_id = a.Job_Id
	AND a.Running = 1;
