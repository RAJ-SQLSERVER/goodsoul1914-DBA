...
IF OBJECT_ID ('SendJobOutput', 'P') IS NULL
    EXEC ('CREATE PROCEDURE dbo.SendJobOutput AS BEGIN SELECT 1 END');
GO

CREATE OR ALTER PROCEDURE dbo.SendJobOutput @StartDate  INT,
                                            @StartTime  INT,
                                            @job_name   sysname,
...
BEGIN
    ...
    Usage: To be used in a SQL Agent job step:
...
*/
    ...
    DECLARE @message NVARCHAR(MAX) = N'<html><body><table><tr><th>JobName</th><th>DateTime</th><th>Step Name</th><th>Message</th></tr>';
