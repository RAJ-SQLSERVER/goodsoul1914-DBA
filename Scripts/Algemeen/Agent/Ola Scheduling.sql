...
SELECT CONVERT (sysname, NULL) AS "job_name",
       CONVERT (sysname, NULL) AS "schedule_name",
       CONVERT (INT, NULL) AS "start_time",
       CONVERT (VARCHAR(50), NULL) AS "days",
...
BEGIN
    ...
    BEGIN
        SELECT @cur_schedule_id = (
            ...
            WHERE jobs.name = @cur_job_name
                  AND (
                      ISNULL (@cur_delete_any_schedule, @default_delete_any_schedule) = 'Y'
                      OR sch.name = @cur_schedule_name
                  )
        );
        ...
        BEGIN
            EXEC msdb.dbo.sp_delete_schedule @schedule_id = @cur_schedule_id,
                                             @force_delete = 1;
    ...
    --/
    SELECT @cur_freq_type = CASE @cur_days
                                WHEN 'all' THEN 4
                                ELSE 8
                            END;
    ...
    --/
    SELECT @cur_freq_interval = 0,
           @cur_days = ' ' + @cur_days + ' ';

    IF CHARINDEX (' all ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 1;

    IF CHARINDEX (' sun ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 1;

    IF CHARINDEX (' mon ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 2;

    IF CHARINDEX (' tue ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 4;

    IF CHARINDEX (' wed ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 8;

    IF CHARINDEX (' thu ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 16;

    IF CHARINDEX (' fri ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 32;

    IF CHARINDEX (' sat ', @cur_days) > 0
        SET @cur_freq_interval = @cur_freq_interval + 64;
    ...
    BEGIN
        ...
        BEGIN
            EXEC msdb.dbo.sp_add_jobschedule @job_name = @cur_job_name,
                                             @name = @cur_schedule_name,
                                             @enabled = 1,
                                             ...
                                             @schedule_id = @cur_schedule_id OUTPUT;

            SET @message = '     ' + 'Schedule created for days' + @cur_days + 'at '
                           + CONVERT (VARCHAR, @cur_start_time);

            RAISERROR (@message, 0, 1) WITH NOWAIT;
        END;
        ELSE
        BEGIN
            SELECT @cur_freq_subday_type = CASE LOWER (LEFT(@cur_intraday_unit, 1))
                                               WHEN 'h' THEN 8
                                               WHEN 'm' THEN 4
                                               WHEN 's' THEN 2
                                               ELSE 0
            ...
            BEGIN
                RAISERROR (
                    '''intraday_unit'' must contain a valid value when ''subday_interval'' contains a number greater than 0. Please check your configuration.',
                    16,
                    1
                );
            END;
            ...
            SET @message = '     ' + 'Schedule created for days' + @cur_days + 'every '
                           + CONVERT (VARCHAR, @cur_subday_interval) + ' ' + @cur_intraday_unit;
