IF OBJECT_ID ('dbo.sp_GetDisabledJobNotification') IS NULL
    EXEC ('CREATE PROCEDURE dbo.sp_GetDisabledJobNotification AS SELECT 1 AS dummy');
GO

CREATE OR ALTER PROCEDURE dbo.sp_GetDisabledJobNotification @p_JobName        VARCHAR(125),
                                                            @p_EnableJob      BIT           = 1,
...
Example:			EXEC dbo.sp_GetDisabledJobNotification 
						@p_JobName = 'TVAnytime', 
						@p_EnableJob = 1, 
...
************************************************************/
    ...
    IF @p_Verbose = 1 PRINT 'Declaring local variables..';
    ...
    IF @p_Verbose = 1 PRINT 'Checking if job named ''' + @p_JobName + ''' exists';
    ...
    BEGIN
        ...
        SELECT @p_EnableJob AS "@p_EnableJob",
               @_isJobFound AS "@_isJobFound",
               @_isJobDisabled AS "@_isJobDisabled";
    END;
    IF @p_EnableJob = 1
       OR @_isJobFound = 0
       OR @_isJobDisabled = 1
    BEGIN -- Process Main Logic
        IF @p_Verbose = 1 PRINT 'Inside Main Logic..';

        IF @_isJobFound = 0
           OR @_isJobDisabled = 1
        BEGIN
            IF @p_Verbose = 1 PRINT 'Set @_sendMailFlag to 1';
            SET @_sendMailFlag = 1;
        END;
        ELSE RETURN;
        ...
        IF @_isJobFound = 0
        BEGIN
            IF @p_Verbose = 1 PRINT 'Inside @_isJobFound = 0';
        ...
        BEGIN
            IF @p_Verbose = 1 PRINT 'Inside @_isJobDisabled = 0';
            IF @p_EnableJob = 1
            BEGIN
                IF @p_Verbose = 1 PRINT 'Inside @_isJobDisabled = 0, AND, @p_EnableJob = 1';

                -- changes the name, description, and disables status of the job NightlyBackups.  
                EXEC @_UpdateJobReturnCode = msdb.dbo.sp_update_job @job_name = @p_JobName,
                                                                    @enabled = 1;

                IF @_UpdateJobReturnCode = 0
                BEGIN
                    IF @p_Verbose = 1 PRINT '  Job successfully enabled.';

                    SET @_mailSubject = 'Job ' + QUOTENAME (@p_JobName) + ' has been ENABLED.';
                    SET @_mailBody = 'Job ' + QUOTENAME (@p_JobName)
                                     + ' was found in DISABLED state. It has been enabled successfully.'
                                     + '
		<p>
		Thanks & Regards,<br>
		SQL Alerts<br>
		...
		<p>'        ;
                ...
                ELSE
                BEGIN
                    IF @p_Verbose = 1 PRINT '  Job enabling failed.';

                    SET @_mailSubject = 'Job ' + QUOTENAME (@p_JobName) + ' in DISABLED state.';
                    SET @_mailBody = 'Job ' + QUOTENAME (@p_JobName)
                                     + ' is in DISABLED state. Enabling the job FAILED.<br><br>Kindly involve DBA to enable the jobs manually.'
                                     + '
		<p>
		Thanks & Regards,<br>
		SQL Alerts<br>
		...
		<p>'        ;
            ...
            BEGIN
                ...
                SET @_mailSubject = 'Job ' + QUOTENAME (@p_JobName) + ' in DISABLED state.';
                SET @_mailBody = 'Job ' + QUOTENAME (@p_JobName)
                                 + ' is in DISABLED state.<br><br>Kindly take appropriate action required.'
                                 + '
		<p>
		Thanks & Regards,<br>
		SQL Alerts<br>
