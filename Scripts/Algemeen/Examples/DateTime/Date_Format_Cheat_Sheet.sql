...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            SET @sql = N'SELECT @v = CONVERT(VARCHAR(30), @d, ' + RTRIM(@s) + N');';
...
BEGIN
    ...
    AS (SELECT TOP (256)
        ...
        ORDER BY name)
    SELECT @sql = @sql + N'INSERT #s SELECT ' + rn + N' FROM 
  (SELECT n = TRY_CONVERT(VARCHAR(30),@d,' + rn + N')) AS x
  WHERE n IS NOT NULL;'
    FROM x;
...
SELECT @sql
    = @sql + N' UNION ALL SELECT [style #] = ' + style
      + N', expression = N''CONVERT(CHAR(''
    +RTRIM(LEN(CONVERT(VARCHAR(30), @d, ' + style + N')))
    +''), @d, ' + style + N')'',
    [output] = CONVERT(VARCHAR(30), @d, ' + style + N')'
FROM #s;
