USE [master];
GO
IF OBJECT_ID('dbo.sp_GenerateTableDDLScript') IS NOT NULL
...
BEGIN
    ...
    SELECT @TableName = name,
           @TableObjId = id,
           @TableSchema = OBJECT_SCHEMA_NAME(id)
    FROM sysobjects
    WHERE id = OBJECT_ID(@TableName);

    IF OBJECT_ID(@TableName) IS NULL
       OR @TableObjId IS NULL
    BEGIN
    ...
    SET @NewTableName
        = ISNULL(@NewTableName, QUOTENAME(DB_NAME(DB_ID())) + '.' + QUOTENAME(@TableSchema) + '.' + @TableName);
    ...
    SELECT DB_NAME(),
           ...
           comp.definition + CASE
                                 WHEN comp.is_persisted = 1 THEN
                                     ' PERSISTED'
                                 ELSE
                                     ''
                             END AS FieldDefinition,
           CASE
               WHEN ic.object_id IS NULL THEN
                   0
               ELSE
                   1
           ...
           CASE
               WHEN st.collation_name IS NOT NULL THEN
                   1
               ELSE
                   0
           END AS IsCharColumn
    FROM INFORMATION_SCHEMA.COLUMNS c
        ...
        LEFT OUTER JOIN [sys].[computed_columns] comp
            ON comp.object_id = sc.object_id
               AND sc.column_id = comp.column_id
    WHERE sc.object_id = @TableObjId
          AND
          (
              comp.definition IS NULL
              OR @IncludeComputedColumns = 1
          )
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Found %d fields', 0, 1, @RCount) WITH NOWAIT;
    IF @Verbose = 1
        SELECT *
        FROM @ShowFields;
    ...
    SELECT CHAR(10) + QUOTENAME(FieldName) + ' '
           + CASE
                 WHEN FieldDefinition IS NOT NULL THEN
                     'AS ' + FieldDefinition
                 WHEN DomainName IS NOT NULL
                      AND @UseSystemDataTypes = 0 THEN
                     QUOTENAME(DomainName) + CASE
                                             ...
                                             END
                 ELSE
                     QUOTENAME(UPPER(DataType))
                     + CASE
                           WHEN IsCharColumn = 1 THEN
                               '(' + ISNULL(NULLIF(CAST(MaxLength AS VARCHAR(10)), '-1'), 'MAX') + ')'
                           ELSE
                               ''
                       END
                     + CASE
                           WHEN @IncludeIdentity = 1
                                AND IdentityColumn = 1 THEN
                     ...
                     + CASE
                           WHEN ColumnDefaultName IS NOT NULL
                                AND @IncludeDefaults = 1 THEN
                               ' CONSTRAINT ' + QUOTENAME(ColumnDefaultName + @ConstraintsNameAppend) + ' DEFAULT'
                               + UPPER(ColumnDefaultValue)
                           ELSE
                               ''
                       END
             END + CASE
                       WHEN FieldID =
                       (
                           SELECT MAX(FieldID) FROM @ShowFields
                       ) THEN
                           ''
                       ELSE
                           ','
    ...
    SELECT ', CONSTRAINT ' + QUOTENAME(name + @ConstraintsNameAppend) + ' FOREIGN KEY (' + ParentColumns
           + ') REFERENCES ' + ReferencedObject + '(' + ReferencedColumns + ')'
    FROM
    (
        SELECT ReferencedObject = QUOTENAME(OBJECT_SCHEMA_NAME(fk.referenced_object_id)) + '.'
                                  + QUOTENAME(OBJECT_NAME(fk.referenced_object_id)),
               ParentObject = QUOTENAME(OBJECT_SCHEMA_NAME(parent_object_id)) + '.'
               ...
               REVERSE(SUBSTRING(REVERSE(
                                 (
                                     ...
                                     WHERE fkc.constraint_object_id = fk.object_id
                                           AND cp.name IN
                                               (
                                                   SELECT FieldName FROM @ShowFields
                                               )
                                     FOR XML PATH('')
                                 )
                                        ),
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Found %d foreign keys', 0, 1, @RCount) WITH NOWAIT;

    INSERT INTO @Definition
    (
        FieldValue
    )
    SELECT CHAR(10) + ', CONSTRAINT ' + QUOTENAME(name + @ConstraintsNameAppend) + ' CHECK ' + definition
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Found %d check constraints', 0, 1, @RCount) WITH NOWAIT;
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Found %d primary key', 0, 1, @RCount) WITH NOWAIT;
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Found %d unique indexes', 0, 1, @RCount) WITH NOWAIT;
    ...
    SELECT CHAR(10) + ', CONSTRAINT ' + QUOTENAME(name + @ConstraintsNameAppend)
           + CASE type
                 WHEN 'PK' THEN
                     ' PRIMARY KEY ' + CASE
                                           WHEN pk.ObjectID IS NULL THEN
                                               ' NONCLUSTERED '
                                           ELSE
                                               ' CLUSTERED '
                                       END
                 WHEN 'UQ' THEN
                     ' UNIQUE '
             END + CASE
                       WHEN u.ObjectID IS NOT NULL THEN
                           ' NONCLUSTERED '
                       ELSE
                           ''
                   END + '('
           + REVERSE(SUBSTRING(REVERSE(
                               (
                                   SELECT QUOTENAME(c.name) + CASE
                                                                  WHEN cc.is_descending_key = 1 THEN
                                                                      ' DESC'
                                                                  ELSE
                                                                      ' ASC'
                                                              END + ','
                                   FROM sys.key_constraints ccok
                                       INNER JOIN sys.index_columns cc
    ...
    WHERE cco.parent_object_id = @TableObjId
          AND
          (
              @IncludePrimaryKey = 1
              OR @IncludeUniqueIndexes = 1
          );
    ...
    BEGIN
        ...
        SELECT CHAR(10) + ', INDEX ' + QUOTENAME([name]) COLLATE SQL_Latin1_General_CP1_CI_AS + ' ' + type_desc + ' ('
               + REVERSE(SUBSTRING(
                                      REVERSE(
                                      (
                                          SELECT QUOTENAME(name) + CASE
                                                                       WHEN sc.is_descending_key = 1 THEN
                                                                           ' DESC'
                                                                       ELSE
                                                                           ' ASC'
                                                                   END + ','
        ...
        SET @RCount = @@ROWCOUNT;
        IF @Verbose = 1
            RAISERROR(N'Found %d indexes', 0, 1, @RCount) WITH NOWAIT;
    END;
    ...
    SET @RCount = @@ROWCOUNT;
    IF @Verbose = 1
        RAISERROR(N'Collected %d rows for main definition', 0, 1, @RCount) WITH NOWAIT;

    SET @Result = N'';

    SELECT @Result = @Result + CHAR(13) + FieldValue
    FROM @MainDefinition
