...
SELECT *
FROM dbo.WaitStats
WHERE WaitType NOT IN ( N'BROKER_EVENTHANDLER', N'BROKER_RECEIVE_WAITFOR', N'BROKER_TASK_STOP', N'BROKER_TO_FLUSH',
...
CREATE OR ALTER PROCEDURE dbo.s_CollectWaitStats (@WaitThresholdInMs BIGINT)
AS
INSERT dbo.WaitStats (WaitType, WaitingTasksCount, WaitTimeMs, MaxWaitTimeMs, SignalWaitTimeMs, CollectionDate)
SELECT wait_type,
       waiting_tasks_count,
       wait_time_ms,
       max_wait_time_ms,
...
CREATE OR ALTER PROCEDURE dbo.s_WaitStatsHistogram @HistogramBucketSizeInMinutes INT = 60 AS;
...
SELECT WaitType,
       CollectionDate,
       WaitTimeMs - LAG (WaitTimeMs, 1, NULL) OVER (PARTITION BY (WaitType)
...
DECLARE @JobDesc sysname = ' Source:   ' + 'https://github.com/mjswart/CollectWaitStats. ;' + ' Created:   '
