...
DECLARE @ForeignKeyName sysname = 'FK_MyTable_MyOtherTable',
        @PrintOnly      BIT     = 0;
...
SELECT     @CMD
    = ISNULL(@CMD + CHAR(13) + CHAR(10) + N' AND ', N'') + N'ctable.' + QUOTENAME(cc.name) + N' = ptable.'
      + QUOTENAME(pc.name),
           @ColumnNullabilityCheck
               = @ColumnNullabilityCheck + CHAR(13) + CHAR(10) + N' AND ctable.' + QUOTENAME(cc.name) + N' IS NOT NULL'
...
SET @CMD
    = N'SELECT ctable.*
FROM ' + QUOTENAME(OBJECT_SCHEMA_NAME(@ChildTableID)) + N'.' + QUOTENAME(OBJECT_NAME(@ChildTableID))
      + N' AS ctable
WHERE NOT EXISTS
(SELECT NULL FROM ' + QUOTENAME(OBJECT_SCHEMA_NAME(@ParentTableID)) + N'.' + QUOTENAME(OBJECT_NAME(@ParentTableID))
      + N' AS ptable
WHERE ' + @CMD + N')' + ISNULL(@ColumnNullabilityCheck, N'');
