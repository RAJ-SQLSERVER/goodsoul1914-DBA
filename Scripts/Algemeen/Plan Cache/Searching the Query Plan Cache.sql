...
SELECT TOP (25)
       ...
       CAST(CAST(eqs.total_logical_reads AS DECIMAL)
            / CAST(eqs.execution_count AS DECIMAL) AS INT) AS logical_reads_per_execution,
...
WHERE d.name = 'AdventureWorks2014'
...
SELECT d.name,
       est.text AS TSQL_Text,
       CAST(CAST(eqs.total_worker_time AS DECIMAL)
            / CAST(eqs.execution_count AS DECIMAL) AS INT) AS cpu_per_execution,
       CAST(CAST(eqs.total_logical_reads AS DECIMAL)
            / CAST(eqs.execution_count AS DECIMAL) AS INT) AS logical_reads_per_execution,
       CAST(CAST(eqs.total_elapsed_time AS DECIMAL)
            / CAST(eqs.execution_count AS DECIMAL) AS INT) AS elapsed_time_per_execution,
       eqs.creation_time,
       eqs.execution_count,
       eqs.total_worker_time AS total_cpu_time,
       eqs.max_worker_time AS max_cpu_time,
       ...
       ecp.objtype,
       ecp.size_in_bytes,
	   CAST(CAST(eqs.execution_count AS DECIMAL) / CAST((CASE WHEN DATEDIFF(HOUR, eqs.creation_time, CURRENT_TIMESTAMP) = 0 THEN 1 ELSE DATEDIFF(HOUR, eqs.creation_time, CURRENT_TIMESTAMP) END) AS DECIMAL) AS INT) AS executions_per_hour
FROM sys.dm_exec_query_stats eqs
    CROSS APPLY sys.dm_exec_sql_text(eqs.plan_handle) est
    CROSS APPLY sys.dm_exec_query_plan(eqs.plan_handle) eqp
    INNER JOIN sys.databases d
...
SELECT est.text AS sql_text,
       CAST(CAST(eqs.execution_count AS DECIMAL)
            / CAST((CASE
                        WHEN DATEDIFF(HOUR, eqs.creation_time, CURRENT_TIMESTAMP) = 0 THEN
                            1
                        ELSE
                            DATEDIFF(HOUR, eqs.creation_time, CURRENT_TIMESTAMP)
                    END
