...
=======================================================
*/
DECLARE @MaxQueriesToCheckFromQueryStats INT     = 200,
...
+ CASE
      WHEN CONVERT (INT, (@@microsoftversion / 0x1000000) & 0xff) >= 13 THEN
	...
	,max_used_grant_kb	= MAX(qs.max_used_grant_kb)'
      ELSE N',total_used_grant_kb	= NULL
...
EXEC sp_executesql @CMD,
                   ...
                   @MaxQueriesToCheckFromQueryStats,
                   @DaysBackToCheck,
                   @MinimumExecCount,
                   @MinimumCPUTime,
                   @MinimumDuration;
...
FROM (
    SELECT t.text AS "SQLBatchText",
           SUBSTRING (t.text,
                      CASE
                          WHEN NULLIF(qs.statement_start_offset, -1) IS NULL THEN 0
                          ELSE qs.statement_start_offset / 2 + 1
                      END,
                      CASE
                          WHEN NULLIF(qs.statement_end_offset, -1) IS NULL THEN LEN (t.text)
                          ELSE (statement_end_offset - qs.statement_start_offset) / 2 + 1
                      END
           ) AS "SQLStmtText",
           QP.query_plan AS "QueryPlan",
           QP.query_plan.query ('.').exist ('data(//RelOp[@PhysicalOp="Parallelism"][1])') AS "HasParallelism",
           CASE
               WHEN QP.query_plan.query ('.').exist ('data(//MissingIndexes[1])') = 1 THEN 1
               ELSE 0
           END AS "HasMissingIndexes",
           STUFF (
           (
               SELECT ', ' + warning
               FROM (
                   ...
                   WHERE QP.query_plan.query ('.').exist (
                                                       'data(//RelOp/Filter/Predicate/ScalarOperator/Compare/ScalarOperator/UserDefinedFunction[1])'
                                                   ) = 1
                   UNION ALL
                   SELECT 'RemoteQuery'
                   ...
                   WHERE QP.query_plan.query ('.').exist (
                                                       'data(//StmtSimple/@StatementOptmEarlyAbortReason[.="MemoryLimitExceeded"])'
                                                   ) = 1
                   UNION ALL
...
CROSS APPLY (
    SELECT MAX (S.node_xml.value ('(@StatementSubTreeCost)[1]', 'float')) AS "HighestStatementCost",
           SUM (S.node_xml.value ('(@StatementSubTreeCost)[1]', 'float')) AS "TotalBatchCost"
