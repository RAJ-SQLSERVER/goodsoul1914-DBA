...
*/
DECLARE @SampleMode       VARCHAR(25) = NULL, -- Valid inputs are DEFAULT, NULL, LIMITED, SAMPLED, or DETAILED. The default (NULL) is LIMITED.
        @MinFragmentation INT         = 20,
        @MinPageCount     INT         = 1000,
        @IncludeHeaps     BIT         = 1;    -- Set to 1 to also check heap tables
...
BEGIN
	...
	AND page_count > ',
               @MinPageCount,
               N'
	AND t.is_ms_shipped = 0'                                         + CASE
                                                                           WHEN @IncludeHeaps = 1 THEN N''
                                                                           ELSE N'
	AND i.index_id >= 1'
                                                                       END
               + N'
...
SELECT CONCAT (QUOTENAME (databaseName), '.', QUOTENAME (schemaName), '.', QUOTENAME (tableName)) AS "full_table_name",
       *
FROM #results
