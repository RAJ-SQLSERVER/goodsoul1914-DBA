...
SELECT Schemas.name AS "SchemaName",
       ...
       ISNULL (IndexUsageStats.user_seeks, 0) + ISNULL (IndexUsageStats.user_scans, 0) AS "NumberOfReads",
       ISNULL (IndexUsageStats.user_updates, 0) AS "NumberOfWrites",
       CASE
           WHEN IndexUsageStats.user_updates > 0 THEN
               CAST((CAST((ISNULL (IndexUsageStats.user_seeks, 0) + ISNULL (IndexUsageStats.user_scans, 0)) AS DECIMAL(19, 2))
                     / CAST(ISNULL (IndexUsageStats.user_updates, 0) AS DECIMAL(19, 2))
                    ) AS DECIMAL(19, 2))
...
WHERE Indexes.type = 2 -- Non-Clustered
...
HAVING SUM (Partitions.rows) > 10000
ORDER BY NumberOfReads ASC;
GO
