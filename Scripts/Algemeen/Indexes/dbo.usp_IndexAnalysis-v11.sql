...
IF OBJECT_ID('dbo.usp_IndexAnalysis') IS NOT NULL
    DROP PROCEDURE dbo.usp_IndexAnalysis;
GO
...
2013-05-22      11.06       * Fixed bug causing mismatch with foreign key analysis
                            * Adjusted overlapping index logic which was broke with new duplicate
                                index logic
*********************************************************************************************/
CREATE PROCEDURE dbo.usp_IndexAnalysis
(
...
BEGIN TRY
    ...
    SELECT @DB_ID = DB_ID(),
           @ObjectID
               = OBJECT_ID(QUOTENAME(DB_NAME(DB_ID())) + '.' + COALESCE(QUOTENAME(PARSENAME(@TableName, 2)), '') + '.'
    ...
    SET @SQL
        = N'WITH AllocationUnits
    AS (
        SELECT p.object_id
            ,p.index_id
            ,p.partition_number 
            ,au.allocation_unit_id
        FROM ' + @DatabaseName + N'.sys.allocation_units AS au
            INNER JOIN ' + @DatabaseName
          + N'.sys.partitions AS p ON au.container_id = p.hobt_id AND (au.type = 1 OR au.type = 3)
        UNION ALL
        SELECT p.object_id
            ,p.index_id
            ,p.partition_number 
        ...
        FROM ' + @DatabaseName + N'.sys.allocation_units AS au
            INNER JOIN ' + @DatabaseName
          + N'.sys.partitions AS p ON au.container_id = p.partition_id AND au.type = 2
    )
    SELECT DB_ID()
		,au.object_id
        ,au.index_id
    ...
    FROM ' + @DatabaseName
          + N'.sys.dm_os_buffer_descriptors AS bd 
    ...
    BEGIN CATCH
        SELECT @ERROR_MESSAGE
            = N'Populate #MemoryBuffer (Line ' + CAST(ERROR_LINE() AS NVARCHAR(25)) + N'): ' + ERROR_MESSAGE(),
    ...
    SET @SQL
	    ...
	    , indexed_columns = STUFF((
			    ...
			    FROM ' + @DatabaseName + N'.sys.index_columns ic
				    INNER JOIN ' + @DatabaseName
          + N'.sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
	    ...
	    , key_columns = STUFF((
          ...
          + N'.sys.index_columns ic_key ON c.object_id = ic_key.object_id 
	    ...
	    , data_columns = CASE WHEN i.index_id IN (0,1) THEN ''ALL-COLUMNS'' ELSE STUFF((
          ...
          + N'.sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
                    LEFT OUTER JOIN ' + @DatabaseName
          + N'.sys.index_columns ic_key ON c.object_id = ic_key.object_id AND c.column_id = ic_key.column_id AND ic_key.index_id = 1
			    WHERE i.object_id = ic.object_id
			    AND i.index_id = ic.index_id
    ...
    FROM ' + @DatabaseName + N'.sys.tables t
	    ...
	    INNER JOIN ' + @DatabaseName + N'.sys.indexes i ON t.object_id = i.object_id
	    INNER JOIN ' + @DatabaseName + N'.sys.data_spaces ds ON i.data_space_id = ds.data_space_id
	    INNER JOIN ' + @DatabaseName
          + N'.sys.dm_db_partition_stats ps ON i.object_id = ps.object_id AND i.index_id = ps.index_id';

    IF @ObjectID IS NOT NULL
        SET @SQL = @SQL + CHAR(13) + N'WHERE t.object_id = @ObjectID ';
    ...
    BEGIN TRY
        ...
        EXEC sys.sp_executesql @SQL,
                               N'@DB_ID INT, @ObjectID INT',
                               @DB_ID = @DB_ID,
                               @ObjectID = @ObjectID;
    END TRY
    BEGIN CATCH
        SELECT @ERROR_MESSAGE
            = N'Populate #IndexMeta (Line ' + CAST(ERROR_LINE() AS NVARCHAR(25)) + N'): ' + ERROR_MESSAGE(),
               @ERROR_SEVERITY = ERROR_SEVERITY(),
               @ERROR_STATE = ERROR_STATE();
    ...
    SELECT IDENTITY(INT, 1, 1) AS row_id,
           ...
           im.row_count,
           ROW_NUMBER() OVER (PARTITION BY im.object_id
           ...
           COALESCE(
                       CAST(100 * (ius.user_seeks + ius.user_scans + ius.user_lookups)
                            / (NULLIF(SUM(ius.user_seeks + ius.user_scans + ius.user_lookups) OVER (PARTITION BY im.object_id), 0)
                               * 1.
                              ) AS DECIMAL(6, 2)),
                       0
                   ) AS user_total_pct,
           ...
           CASE
               WHEN ius.user_seeks + ius.user_scans + ius.user_lookups >= ius.user_updates THEN
                   CEILING(1. * (ius.user_seeks + ius.user_scans + ius.user_lookups)
                           / COALESCE(NULLIF(ius.user_seeks, 0), 1)
                          )
               ELSE
                   0
           ...
           CASE
               WHEN ius.user_seeks + ius.user_scans + ius.user_lookups <= ius.user_updates THEN
                   CEILING(1. * (ius.user_updates)
                           / COALESCE(NULLIF(ius.user_seeks + ius.user_scans + ius.user_lookups, 0), 1)
                          )
               ELSE
                   0
    ...
    FROM #IndexMeta im
        ...
        LEFT OUTER JOIN sys.dm_db_index_operational_stats(@DB_ID, NULL, NULL, NULL) ios
            ON im.object_id = ios.object_id
               AND im.index_id = ios.index_id
               AND im.partition_number = ios.partition_number
        LEFT OUTER JOIN #MemoryBuffer mb
            ON im.object_id = mb.object_id
    ...
    BEGIN
        ...
        SELECT '' AS filegroup_name,
               ...
               migs.user_scans,
               0 AS user_lookups,
               COALESCE(   equality_columns + CASE
                                              ...
                                              END,
                           SPACE(0)
                       ) + COALESCE(inequality_columns, SPACE(0)) AS indexed_columns,
               included_columns
    ...
    SET @SQL
        = N'SELECT fk.name + ''|PARENT'' AS foreign_key_name
        ,fkc.parent_object_id AS object_id
        ,STUFF((SELECT '', '' + QUOTENAME(c.name)
            FROM ' + @DatabaseName + N'.sys.foreign_key_columns ifkc
                INNER JOIN ' + @DatabaseName
          + N'.sys.columns c ON ifkc.parent_object_id = c.object_id AND ifkc.parent_column_id = c.column_id
        ...
        ,(SELECT QUOTENAME(CAST(ifkc.parent_column_id AS VARCHAR(10))+''+'',''('')
            FROM ' + @DatabaseName
          + N'.sys.foreign_key_columns ifkc
            WHERE fk.object_id = ifkc.constraint_object_id
            ORDER BY ifkc.constraint_column_id
            FOR XML PATH('''')) AS fk_columns_compare
    FROM ' + @DatabaseName + N'.sys.foreign_keys fk
        INNER JOIN ' + @DatabaseName
          + N'.sys.foreign_key_columns fkc ON fk.object_id = fkc.constraint_object_id
    ...
    SELECT fk.name + ''|REFERENCED'' as foreign_key_name
        ...
        ,STUFF((SELECT '', '' + QUOTENAME(c.name)
            FROM ' + @DatabaseName + N'.sys.foreign_key_columns ifkc
                INNER JOIN ' + @DatabaseName
          + N'.sys.columns c ON ifkc.referenced_object_id = c.object_id AND ifkc.referenced_column_id = c.column_id
            WHERE fk.object_id = ifkc.constraint_object_id
            ORDER BY ifkc.constraint_column_id
            FOR XML PATH('''')), 1, 2, '''') AS fk_columns
    ...
    BEGIN CATCH
        SELECT @ERROR_MESSAGE
            = N'Populate #ForeignKeys (Line ' + CAST(ERROR_LINE() AS NVARCHAR(25)) + N'): ' + ERROR_MESSAGE(),
               @ERROR_SEVERITY = ERROR_SEVERITY(),
               @ERROR_STATE = ERROR_STATE();
    ...
    UPDATE i
    SET duplicate_indexes = STUFF(
                            (
                                SELECT ', ' + index_name AS [data()]
                                ...
                                WHERE i.object_id = iibl.object_id
                                      ...
                                      AND REPLACE(i.indexed_columns_ids, '|', '') = REPLACE(
                                                                                               iibl.indexed_columns_ids,
                                                                                               '|',
                                                                                               ''
                                                                                           )
        ...
        overlapping_indexes = STUFF(
                              (
                                  ...
                                  WHERE i.object_id = iibl.object_id
                                        ...
                                        AND ISNULL(i.filter_definition, '') = ISNULL(iibl.filter_definition, '')
                                        AND i.index_id <> iibl.index_id
                                        AND LEFT(i.indexed_columns_ids, CHARINDEX('|', iibl.indexed_columns_ids, 1) - 1) LIKE LEFT(iibl.indexed_columns_ids, CHARINDEX(
                                                                                                                                                                          '|',
        ...
        related_foreign_keys = STUFF(
                               (
                                   SELECT ', ' + foreign_key_name AS [data()]
    ...
    BEGIN
        ...
        SELECT '' AS filegroup_name,
               ...
               fk.foreign_key_name
        FROM #ForeignKeys fk
            LEFT OUTER JOIN #IndexStatistics i
    ...
    AS (SELECT row_id,
               CAST(100. * (user_seeks + user_scans + user_lookups)
                    / (NULLIF(SUM(user_seeks + user_scans + user_lookups) OVER (PARTITION BY schema_name, table_name), 0)
                       * 1.
                      ) AS DECIMAL(12, 2)) AS estimated_user_total_pct,
    ...
    AS (SELECT row_id,
               CASE
                   WHEN user_lookups > user_seeks
                        AND type_desc IN ( 'CLUSTERED', 'HEAP', 'UNIQUE CLUSTERED' ) THEN
                       'REALIGN'
                   WHEN is_disabled = 1 THEN
                       'ENABLE'
                   WHEN duplicate_indexes IS NOT NULL
                        AND first_dup_index_id IS NOT NULL
                        AND index_id IS NOT NULL THEN
                       'DROP-DUP'
                   WHEN type_desc = '--MISSING FOREIGN KEY--' THEN
                       'CREATE'
                   ...
                   WHEN type_desc = '--NONCLUSTERED--'
                        AND ROW_NUMBER() OVER (PARTITION BY table_name ORDER BY user_total DESC) <= 10
                        AND estimated_user_total_pct > 1 THEN
                       'CREATE'
    ...
    SET index_pros = COALESCE(STUFF(   CASE
        ...
        index_cons = COALESCE(STUFF(   CASE
                                           WHEN user_seeks / NULLIF(user_scans, 0) < 1000 THEN
                                               ', SCN'
                                           ELSE
                                               ''
                                       END + CASE
                                                 WHEN duplicate_indexes IS NOT NULL THEN
                                                     ', DP'
                                                 ELSE
                                                     ''
                                             END + CASE
                                                       WHEN overlapping_indexes IS NOT NULL THEN
                                                           ', OV'
                                                       ELSE
                                                           ''
                                                   END + COALESCE(   ', ' + CASE
...
END TRY
BEGIN CATCH
    SELECT @ERROR_MESSAGE = N'Procedure Error (Line ' + CAST(ERROR_LINE() AS NVARCHAR(25)) + N'): ' + ERROR_MESSAGE(),
           @ERROR_SEVERITY = ERROR_SEVERITY(),
           @ERROR_STATE = ERROR_STATE();
...
--EXEC DBA.dbo.usp_IndexAnalysis @Output = 'DETAILED',
--                          @TableName = '[Production].[ProductDescription]',
--                          @IncludeMissingIndexes = 1,
--                          @IncludeMissingFKIndexes = 1;

--EXEC DBA.dbo.usp_IndexAnalysis @Output = 'OVERLAPPING',
--                          @TableName = '[Production].[ProductDescription]',
--                          @IncludeMissingIndexes = 1,
--                          @IncludeMissingFKIndexes = 1;
