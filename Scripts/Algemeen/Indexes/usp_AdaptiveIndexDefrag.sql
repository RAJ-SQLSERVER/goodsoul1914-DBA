-- If you are using AdaptiveIndexDefrag together with the maintenance plans 
-- in http://blogs.msdn.com/b/blogdoezequiel/archive/2012/09/18/about-maintenance-plans-grooming-sql-server.aspx
-- please note that the job that runs AdaptiveIndexDefrag is expecting msdb. 
-- As such, change the database context accordingly.
...
RAISERROR('Dropping existing objects', 0, 42) WITH NOWAIT;

IF EXISTS
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_CurrentExecStats'
)
...
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_AvgSamplingLst30Days'
)
...
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_AvgTimeLst30Days '
)
...
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_AvgFragLst30Days'
)
...
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_AvgLargestLst30Days'
)
...
(
    SELECT [object_id]
    FROM sys.views
    WHERE [name] = 'vw_AvgMostUsedLst30Days'
)
...
BEGIN
    ...
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_log'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_log_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_log',
                       'tbl_AdaptiveIndexDefrag_log_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_log_old.PK_AdaptiveIndexDefrag_log',
                       N'PK_AdaptiveIndexDefrag_log_old',
                       N'INDEX';
    END;
    IF EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Analysis_log'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Analysis_log_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_Analysis_log',
                       'tbl_AdaptiveIndexDefrag_Analysis_log_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_Analysis_log_old.PK_AdaptiveIndexDefrag_Analysis_log',
                       N'PK_AdaptiveIndexDefrag_Analysis_log_old',
                       N'INDEX';
    END;
    IF EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Stats_log'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Stats_log_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_Stats_log',
                       'tbl_AdaptiveIndexDefrag_Stats_log_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_Stats_log_old.PK_AdaptiveIndexDefrag_Stats_log',
                       N'PK_AdaptiveIndexDefrag_Stats_log_old',
                       N'INDEX';
    END;
    IF EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Exceptions'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Exceptions_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_Exceptions',
                       'tbl_AdaptiveIndexDefrag_Exceptions_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_Exceptions_old.PK_AdaptiveIndexDefrag_Exceptions',
                       N'PK_AdaptiveIndexDefrag_Exceptions_old',
                       N'INDEX';
    END;
    ...
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Working'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Working_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_Working',
                       'tbl_AdaptiveIndexDefrag_Working_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_Working_old.PK_AdaptiveIndexDefrag_Working',
                       N'PK_AdaptiveIndexDefrag_Working_old',
                       N'INDEX';
    END;
    ...
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Stats_Working'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_Stats_Working_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_Stats_Working',
                       'tbl_AdaptiveIndexDefrag_Stats_Working_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_Stats_Working_old.PK_AdaptiveIndexDefrag_Stats_Working',
                       N'PK_AdaptiveIndexDefrag_Stats_Working_old',
                       N'INDEX';
    END;
    IF EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_IxDisableStatus'
    )
       AND NOT EXISTS
    (
        SELECT [object_id]
        FROM sys.tables
        WHERE [name] = 'tbl_AdaptiveIndexDefrag_IxDisableStatus_old'
    )
    BEGIN
        EXEC sp_rename 'tbl_AdaptiveIndexDefrag_IxDisableStatus',
                       'tbl_AdaptiveIndexDefrag_IxDisableStatus_old';
        EXEC sp_rename N'tbl_AdaptiveIndexDefrag_IxDisableStatus_old.PK_AdaptiveIndexDefrag_IxDisableStatus',
                       N'PK_AdaptiveIndexDefrag_IxDisableStatus_old',
                       N'INDEX';
    END;
END;
...
(
    ...
    (
        ...
        errorMessage VARCHAR(1000) NULL
            CONSTRAINT PK_AdaptiveIndexDefrag_log
            PRIMARY KEY CLUSTERED (indexDefrag_id)
    );

CREATE INDEX IX_tbl_AdaptiveIndexDefrag_log
ON [dbo].[tbl_AdaptiveIndexDefrag_log] (
                                           [dbID],
                                           [objectID],
                                           [indexName],
                                           [dateTimeEnd]
                                       );
CREATE INDEX IX_tbl_AdaptiveIndexDefrag_log_dateTimeEnd
ON [dbo].[tbl_AdaptiveIndexDefrag_log] (
                                           [indexDefrag_id],
                                           [dateTimeEnd]
                                       );
...
(
    ...
    (
        ...
        durationSeconds INT NULL,
        errorMessage VARCHAR(1000) NULL
            CONSTRAINT PK_AdaptiveIndexDefrag_Analysis_log
            PRIMARY KEY CLUSTERED (analysis_id)
    );

CREATE INDEX IX_tbl_AdaptiveIndexDefrag_Analysis_log_dateTimeEnd
ON [dbo].[tbl_AdaptiveIndexDefrag_Analysis_log] (
                                                    [analysis_id],
                                                    [dateTimeEnd]
                                                );
...
(
    ...
    (
		...
		For multiple days, sum the corresponding values*/
            CONSTRAINT PK_AdaptiveIndexDefrag_Exceptions
...
(
    ...
    (
        ...
        printStatus BIT
            DEFAULT (0),                    -- Used for loop control when printing the SQL commands.
        exclusionMask INT
            DEFAULT (0)
            CONSTRAINT PK_AdaptiveIndexDefrag_Working
            PRIMARY KEY CLUSTERED (
                                      dbID,
                                      objectID,
                                      indexID,
...
(
    ...
    (
        ...
        printStatus BIT
            DEFAULT (0) -- Used for loop control when printing the SQL commands.
            CONSTRAINT PK_AdaptiveIndexDefrag_Stats_Working
            PRIMARY KEY CLUSTERED (
                                      dbID,
                                      objectID,
                                      statsID,
...
(
    ...
    (
        ...
        sqlStatement VARCHAR(4000) NULL,
        errorMessage VARCHAR(1000) NULL
            CONSTRAINT PK_AdaptiveIndexDefrag_Stats_log
            PRIMARY KEY CLUSTERED (statsUpdate_id)
    );

CREATE INDEX IX_tbl_AdaptiveIndexDefrag_Stats_log
ON [dbo].[tbl_AdaptiveIndexDefrag_Stats_log] (
                                                 [dbID],
                                                 [objectID],
                                                 [statsName],
                                                 [dateTimeEnd]
...
(
    ...
    (
        ...
        dateTimeChange DATETIME NOT NULL
            CONSTRAINT PK_AdaptiveIndexDefrag_IxDisableStatus
            PRIMARY KEY CLUSTERED (disable_id)
    );
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
            ...
            INSERT INTO dbo.tbl_AdaptiveIndexDefrag_log
            (
                [dbID],
                [dbName],
                [objectID],
        ...
        (
            ...
            INSERT INTO dbo.tbl_AdaptiveIndexDefrag_Analysis_log
            (
                [Operation],
                [dbID],
                [dbName],
        ...
        BEGIN
            IF
            (
                SELECT COUNT(sc.column_id)
                FROM sys.tables st
                    INNER JOIN sys.columns sc
                        ON st.[object_id] = sc.[object_id]
                WHERE (
                          sc.[name] = 'partitionNumber'
                          OR sc.[name] = 'rows'
                          OR sc.[name] = 'rows_sampled'
                          OR sc.[name] = 'modification_counter'
                      )
                      AND st.[name] = 'tbl_AdaptiveIndexDefrag_Stats_log_old'
            ) = 4
            BEGIN
        ...
        BEGIN
            ...
            (
                ...
                FROM sys.tables st
                    INNER JOIN sys.columns sc
                        ON st.[object_id] = sc.[object_id]
                WHERE (sc.[name] = 'compression_type')
                      AND st.[name] = 'tbl_AdaptiveIndexDefrag_Working_old'
            )
            BEGIN
...
FROM dbo.tbl_AdaptiveIndexDefrag_Working_old;');
        ...
        BEGIN
            ...
            (
                ...
                WHERE (
                          sc.[name] = 'partitionNumber'
                          OR sc.[name] = 'is_incremental'
                      )
                      AND st.[name] = 'tbl_AdaptiveIndexDefrag_Stats_Working_old'
            )
            BEGIN
...
FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working_old;');
        ...
        (
            ...
            INSERT INTO dbo.tbl_AdaptiveIndexDefrag_IxDisableStatus
            (
                [dbID],
                [objectID],
                [indexID],
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_log_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_log
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Analysis_log_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Analysis_log
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Stats_log_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Stats_log
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Exceptions_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Exceptions
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Working_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Working
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
        )
    ...
    BEGIN
        IF
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_IxDisableStatus_old
        ) =
        (
            SELECT COUNT(*) FROM dbo.tbl_AdaptiveIndexDefrag_IxDisableStatus
        )
...
BEGIN
    BEGIN TRY
        ...
        IF @minFragmentation IS NULL
           OR @minFragmentation NOT
           BETWEEN 0.00 AND 100.0
            SET @minFragmentation = 5.0;

        IF @rebuildThreshold IS NULL
           OR @rebuildThreshold NOT
           BETWEEN 0.00 AND 100.0
            SET @rebuildThreshold = 30.0;

        IF @rebuildThreshold_cs IS NULL
           OR @rebuildThreshold_cs NOT
           BETWEEN 0.00 AND 100.0
            SET @rebuildThreshold_cs = 10.0;

        IF @statsThreshold IS NOT NULL
           AND @statsThreshold NOT
           BETWEEN 0.001 AND 100.0
        ...
        IF @dataCompression IS NOT NULL
           AND UPPER(@dataCompression) NOT IN ( 'NONE', 'ROW', 'PAGE' )
            SET @dataCompression = NULL;

        /* Validate if table name is fully qualified and database scope is set */
        IF @tblName IS NOT NULL
           AND @tblName NOT LIKE '%.%'
        BEGIN
            RAISERROR(
                         'WARNING: Table name must be fully qualified. Input format should be <schema>.<table_name>.',
                         15,
                         42
                     ) WITH NOWAIT;
            RETURN;
        ...
        IF @dbScope IS NOT NULL
           AND LOWER(@dbScope) NOT IN
               (
                   SELECT LOWER([name])
                   FROM sys.databases
                   WHERE LOWER([name]) NOT IN ( 'master', 'tempdb', 'model', 'reportservertempdb', 'semanticsdb' )
        ...
        IF @defragOrderColumn IS NULL
           OR LOWER(@defragOrderColumn) NOT IN ( 'range_scan_count', 'fragmentation', 'page_count' )
        BEGIN
        ...
        IF @defragSortOrder IS NULL
           OR UPPER(@defragSortOrder) NOT IN ( 'ASC', 'DESC' )
        ...
        IF @ixtypeOption IS NOT NULL
           AND @ixtypeOption NOT IN ( 0, 1 )
            SET @ixtypeOption = NULL;

        IF @statsSample IS NOT NULL
           AND
           (
               ISNUMERIC(@statsSample) = 1
               AND @statsSample NOT
           BETWEEN 1 AND 100
           )
           OR
           (
               ISNUMERIC(@statsSample) = 0
               AND UPPER(@statsSample) NOT IN ( 'FULLSCAN', 'RESAMPLE' )
           )
            SET @statsSample = NULL;

        IF (
               @persistStatsSample IS NOT NULL
               AND @persistStatsSample NOT IN ( 0, 1 )
           )
           OR
           (
               @sqlmajorver <> 13
        ...
        IF @dbScope IS NOT NULL
           AND @sqlmajorver >= 11
           AND @engineedition NOT IN ( 5, 6 ) -- Except SQL Database and SQL DW
        BEGIN
            DECLARE @sqlcmdAO NVARCHAR(3000),
                    @paramsAO NVARCHAR(50),
                    @DBinAG INT;
            SET @sqlcmdAO
                = N'IF LOWER(@dbScopeIN) IN (SELECT LOWER(DB_NAME(dr.database_id))
...
END'        ;
            ...
            EXECUTE sp_executesql @sqlcmdAO,
                                  @paramsAO,
                                  @dbScopeIN = @dbScope,
                                  @DBinAG_OUT = @DBinAG OUTPUT;

            IF @DBinAG = 1
            BEGIN
                RAISERROR(
                             'WARNING: Cannot defrag database in scope because it is part of an Always On secondary replica.',
                             15,
                             42
                         ) WITH NOWAIT;
                RETURN;
        ...
        IF @forceRescan = 0
           AND @dbScope IS NOT NULL -- Specific scope was set
        BEGIN
            IF
            (
                SELECT COUNT(DISTINCT [dbID]) FROM dbo.tbl_AdaptiveIndexDefrag_Working
            ) > 1
            OR QUOTENAME(LOWER(@dbScope)) NOT IN
               (
                   SELECT DISTINCT
                          LOWER([dbName])
        ...
        (
            ...
            WHERE defragDate IS NULL
               ...
               AND @ixtypeOption IS NULL
           )
           AND NOT EXISTS
        (
            SELECT TOP 1
                   *
            FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working AS idss
            WHERE idss.updateDate IS NULL
                  AND NOT EXISTS
            (
                SELECT TOP 1
                       objectID
                FROM dbo.tbl_AdaptiveIndexDefrag_Working ids
                WHERE ids.[dbID] = idss.[dbID]
                      AND ids.objectID = idss.objectID
                      AND idss.statsName = ids.indexName
                      AND idss.updateDate IS NULL
                      AND ids.exclusionMask & POWER(2, DATEPART(WEEKDAY, GETDATE()) - 1) = 0
            )
        )
        BEGIN
            SET @forceRescan = 1;
            RAISERROR(
                         'No indexes of the chosen type left to defrag nor statistics left to update. Forcing rescan...',
                         0,
                         42
                     ) WITH NOWAIT;
        END;
        ...
        (
            ...
            WHERE (
                      fill_factor IS NULL
                      OR is_padded IS NULL
                      OR compression_type IS NULL
                  )
                  AND [exclusionMask] & POWER(2, DATEPART(WEEKDAY, GETDATE()) - 1) = 0
        )
               )
        ...
        IF @forceRescan = 0
           AND @dbScope IS NULL
        BEGIN
            DECLARE @sqlcmd_CntSrc NVARCHAR(3000),
                    @params_CntSrc NVARCHAR(50),
                    @CountSrc INT;
            DECLARE @sqlcmd_CntTgt NVARCHAR(3000),
                    @params_CntTgt NVARCHAR(50),
                    @CountTgt INT;
            DECLARE @dbIDIX INT,
                    @hasIXs BIT,
                    @hasIXsCntsqlcmd NVARCHAR(3000),
                    @hasIXsCntsqlcmdParams NVARCHAR(50);

            -- What is in working tables plus exceptions that still exist in server
            SET @sqlcmd_CntSrc
                = N'SELECT @CountSrc_OUT = COUNT(DISTINCT Working.[dbID]) FROM
(SELECT DISTINCT [dbID] FROM [' + DB_NAME()
                  + N'].dbo.tbl_AdaptiveIndexDefrag_Working
UNION
SELECT DISTINCT [dbID] FROM [' + DB_NAME()
                  + N'].dbo.tbl_AdaptiveIndexDefrag_Stats_Working
UNION
SELECT DISTINCT [dbID] FROM [' + DB_NAME()
                  + N'].dbo.tbl_AdaptiveIndexDefrag_Exceptions
	...
	WHERE LOWER(sd.[name]) NOT IN (''master'', ''tempdb'', ''model'', ''reportservertempdb'',''semanticsdb'')
		...
		AND is_distributor = 0)
            ...
            (
                ...
                WHERE [object_id] = OBJECT_ID('tempdb.dbo.#tblIndexFindInDatabaseList')
            )
            ...
            SET @sqlcmd_CntTgt
                = N'SELECT [database_id], 0, 0 -- not yet scanned
FROM sys.databases
WHERE LOWER([name]) = ISNULL(LOWER(@dbScopeIN), LOWER([name]))	
	...
	AND is_distributor = 0';
            ...
            IF @sqlmajorver >= 11
               ...
               (
                   SELECT @@VERSION
               ) NOT LIKE 'Microsoft SQL Azure%' -- Except all local Always On secondary replicas
            BEGIN
                SET @sqlcmd_CntTgt
                    = @sqlcmd_CntTgt + CHAR(10)
                      + N'AND [database_id] NOT IN (SELECT dr.database_id FROM sys.dm_hadr_database_replica_states dr
INNER JOIN sys.dm_hadr_availability_replica_states rs ON dr.group_id = rs.group_id
INNER JOIN sys.databases d ON dr.database_id = d.database_id
WHERE rs.role = 2 -- Is Secondary
	AND dr.is_local = 1
	AND rs.is_local = 1)';
            ...
            EXECUTE sp_executesql @sqlcmd_CntTgt,
                                  @params_CntTgt,
                                  @dbScopeIN = @dbScope;

            WHILE
            (
                SELECT COUNT(*) FROM #tblIndexFindInDatabaseList WHERE scanStatus = 0
            ) > 0
            BEGIN
                SELECT TOP 1
                       @dbIDIX = [dbID]
                FROM #tblIndexFindInDatabaseList
                WHERE scanStatus = 0;

                SET @hasIXsCntsqlcmd
                    = N'IF EXISTS (SELECT TOP 1 [index_id] from [' + DB_NAME(@dbIDIX)
                      + N'].sys.indexes AS si
INNER JOIN ['                          + DB_NAME(@dbIDIX)
                      + N'].sys.objects so ON si.object_id = so.object_id
WHERE so.is_ms_shipped = 0 AND [index_id] > 0 AND si.is_hypothetical = 0
	AND si.[object_id] NOT IN (SELECT sit.[object_id] FROM [' + DB_NAME(@dbIDIX)
                      + N'].sys.internal_tables AS sit))
OR
EXISTS (SELECT TOP 1 [stats_id] from [' + DB_NAME(@dbIDIX) + N'].sys.stats AS ss
INNER JOIN ['                          + DB_NAME(@dbIDIX)
                      + N'].sys.objects so ON ss.[object_id] = so.[object_id]
WHERE so.is_ms_shipped = 0
AND ss.[object_id] NOT IN (SELECT sit.[object_id] FROM [' + DB_NAME(@dbIDIX)
                      + N'].sys.internal_tables AS sit))	
BEGIN SET @hasIXsOUT = 1 END ELSE BEGIN SET @hasIXsOUT = 0 END';
                SET @hasIXsCntsqlcmdParams = N'@hasIXsOUT int OUTPUT';
                EXECUTE sp_executesql @hasIXsCntsqlcmd,
                                      @hasIXsCntsqlcmdParams,
                                      @hasIXsOUT = @hasIXs OUTPUT;
            ...
            EXECUTE sp_executesql @sqlcmd_CntSrc,
                                  @params_CntSrc,
                                  @CountSrc_OUT = @CountSrc OUTPUT;
            SELECT @CountTgt = COUNT([dbID])
            FROM #tblIndexFindInDatabaseList
            WHERE hasIXs = 1;
        ...
        (
            SELECT [object_id]
            FROM tempdb.sys.objects (NOLOCK)
            WHERE [object_id] = OBJECT_ID('tempdb.dbo.#tblIndexDefragDatabaseList')
        )
        ...
        (
            SELECT [object_id]
            FROM tempdb.sys.objects (NOLOCK)
            WHERE [object_id] = OBJECT_ID('tempdb.dbo.#tblIndexDefragmaxPartitionList')
        )
        ...
        (
            SELECT [object_id]
            FROM tempdb.sys.objects (NOLOCK)
            WHERE [object_id] = OBJECT_ID('tempdb.dbo.#tblIndexDefragScanWorking')
        )
        ...
        IF @maxDopRestriction IS NOT NULL
           AND @maxDopRestriction > @cpucount
           AND @cpucount <= 8
        BEGIN
        ...
        ELSE IF @maxDopRestriction IS NOT NULL
                ...
                (
                    (
                        @maxDopRestriction > @cpucount
                        AND @cpucount > 8
                    )
                    OR @maxDopRestriction > 8
                )
        BEGIN
        ...
        ) IN ( 1804890536, 1872460670, 610778273, -2117995310 )
        ...
        BEGIN
            SELECT @debugMessage
                = CHAR(10) + N'Executing AdaptiveIndexDefrag v' + @ver + N' on ' + @@VERSION
                  + N'
...
Rebuild indexes with fragmentation greater than ' + CAST(@rebuildThreshold AS NVARCHAR(10))
                  + N';
Rebuild columnstore indexes with fragmentation greater than ' + CAST(@rebuildThreshold_cs AS NVARCHAR(10)) + N';
'                                  + CASE
                                         WHEN @disableNCIX = 1 THEN
                                             'Non-clustered indexes will be disabled prior to rebuild;
'
                                         ELSE
                                             ''
                                     END + N'Defragment ' + CASE
                                                                WHEN @ixtypeOption IS NULL THEN
                                                                    'ALL indexes'
                                                                WHEN @ixtypeOption = 1 THEN
                                                                    'only CLUSTERED indexes'
                                                                ELSE
                                                                    'only NON-CLUSTERED, XML and Spatial indexes'
                                                            END + N';
Commands'                          + CASE
                                         WHEN @Exec_Print = 1 THEN
                                             ' WILL'
                                         ELSE
                                             ' WILL NOT'
                                     END + N' be executed automatically;
Defragment indexes in '            + @defragSortOrder + N' order of the ' + UPPER(@defragOrderColumn)
                  + N' value;
Time limit'                        + CASE
                                         WHEN @timeLimit IS NULL THEN
                                             ' was not specified;'
                                         ELSE
                                             ' was specified and is ' + CAST(@timeLimit AS NVARCHAR(10))
                                     END + N' minutes;
'                                  + CASE
                                         WHEN @dbScope IS NULL THEN
                                             'ALL databases'
                                         ELSE
                                             'The [' + @dbScope + '] database'
                                     END + N' will be defragmented;
'                                  + CASE
...
'                                  + N'We' + CASE
            ...
            (
                ...
                WHERE defragDate IS NULL
                                                      ...
                                                      AND @forceRescan = 0 THEN
                                                     ' will resume any existing previous run. If so, we WILL NOT'
                                                 ELSE
                                                     ' WILL'
                                             END + N' be rescanning indexes;
The scan will be performed in '    + @scanMode + N' mode;
LOBs will '                        + CASE
                                         WHEN @dealLOB = 1 THEN
                                             'NOT '
                                         ELSE
                                             ''
                                     END + N'be compacted;
Limit defrags to indexes'
                  + CASE
                        WHEN @maxPageCount IS NULL THEN
                            ' with more than ' + CAST(@minPageCount AS NVARCHAR(10))
                        ELSE
                            ' between ' + CAST(@minPageCount AS NVARCHAR(10)) + ' and '
                            + CAST(@maxPageCount AS NVARCHAR(10))
                    END + N' pages;
Indexes will be defragmented'      + CASE
                                         WHEN @onlineRebuild = 0
                                              OR @editionCheck = 0 THEN
                                             ' OFFLINE;'
                                         ELSE
                                             ' ONLINE;'
                                     END + N'
Indexes will be sorted in'         + CASE
                                         WHEN @sortInTempDB = 0 THEN
                                             ' the DATABASE;'
                                         ELSE
                                             ' TEMPDB;'
                                     END + N'
Indexes will use '                 + CASE
                                         WHEN @dataCompression IS NULL THEN
                                             'CURRENT compression setting;'
                                         WHEN @dataCompression = 'NONE' THEN
                                             'NO compression on all indexes;'
                                         WHEN @dataCompression = 'PAGE' THEN
                                             'PAGE compression on all indexes;'
                                         WHEN @dataCompression = 'ROW' THEN
                                             'ROW compression on all indexes;'
                                         ELSE
                                             @dataCompression + 'COLUMNSTORE compression on all indexes;'
                                     END + N'
Indexes will keep'                 + CASE
                                         WHEN @fillfactor = 1 THEN
                                             ' its EXISTING'
                                         ELSE
                                             ' the DEFAULT'
                                     END + N' Fill Factor;'
                  + CASE
                        WHEN @dealMaxPartition = 1
                             AND @editionCheck = 1 THEN
                            '
The right-most populated partitions will be ignored;'
                        WHEN @dealMaxPartition = 0
                             AND @editionCheck = 1 THEN
                            '		
Only the right-most populated partitions will be considered if greater than ' + CAST(@minPageCount AS NVARCHAR(10))
                            + ' page(s);'
                        ELSE
                            CHAR(10) + 'All partitions will be considered;'
                    END + CHAR(10) + N'Statistics ' + CASE
                                                          WHEN @updateStats = 1 THEN
                                                              'WILL'
                                                          ELSE
                                                              'WILL NOT'
                                                      END + N' be updated '
                  + CASE
                        WHEN @updateStatsWhere = 1 THEN
                            'on reorganized indexes;'
                        ELSE
                            'on all stats belonging to parent table;'
                    END + CASE
                              WHEN @updateStats = 1
                                   AND @statsSample IS NOT NULL
                                   AND ISNUMERIC(@statsSample) = 0 THEN
                                  CHAR(10) + 'Statistics will be updated with ' + @statsSample
                              ELSE
                                  ''
                          END
                  + CASE
                        WHEN @updateStats = 1
                             AND @statsSample IS NOT NULL
                             AND ISNUMERIC(@statsSample) = 1 THEN
                            CHAR(10) + 'Statistics will be updated with ' + RTRIM(@statsSample) + ' sampling rate'
                        ELSE
                            ''
                    END + CASE
                              WHEN @updateStats = 1
                                   AND @statsSample IS NOT NULL
                                   AND ISNUMERIC(@statsSample) = 1
                                   AND @persistStatsSample = 1 THEN
                                  ' and persisting specified sampling rate percent;'
                              ELSE
                                  ''
                          END + CASE
                                    WHEN @updateStats = 1
                                         ...
                                         (
                                             (
                                                 ISNUMERIC(@statsSample) = 1
                                                 AND @persistStatsSample = 0
                                             )
                                             OR @persistStatsSample IS NULL
                                         ) THEN
                                        ' and NOT persisting specified sampling rate percent;'
                                    ELSE
                                        ''
                                END + CHAR(10) + N'Statistics will be updated using '
                  + CASE
                        WHEN @statsThreshold IS NOT NULL
                             AND @statsThreshold
                             BETWEEN 0.001 AND 100.0 THEN
                            'a threshold of ' + CONVERT(VARCHAR, @statsThreshold) + ' percent'
                        ELSE
                            'a calculated threshold similar to TF2371'
                    END + +N' on tables' + CASE
                                               WHEN @statsThreshold IS NOT NULL
                                                    AND @statsThreshold
                                                    BETWEEN 0.01 AND 100.0
                                                    AND @statsMinRows IS NOT NULL THEN
                                                   ' with a min of ' + CONVERT(VARCHAR, @statsMinRows) + ' rows.'
                                               WHEN @statsMinRows IS NOT NULL THEN
                                                   ' of any size.'
                                               ELSE
                                                   '.'
                                           END + CHAR(10)
                  + N'Statistics will be updated with Incremental property (if any) '
                  + CASE
                        WHEN @statsIncremental = 1 THEN
                            'as ON'
                        WHEN @statsIncremental = 0 THEN
                            'as OFF'
                        ELSE
                            'not changed from current setting'
                    END + N'.' + CHAR(10) + N'Defragmentation will use '
                  + CASE
                        WHEN @editionCheck = 0
                             OR @maxDopRestriction IS NULL THEN
                            'system defaults for processors;'
                        ELSE
                            CAST(@maxDopRestriction AS VARCHAR(3)) + ' processors;'
                    END + CHAR(10) + N'Lock timeout is set to ' + CASE
                                                                      WHEN @offlinelocktimeout <> -1
                                                                           AND @offlinelocktimeout IS NOT NULL THEN
                                                                          CONVERT(NVARCHAR(15), @offlinelocktimeout)
                                                                      ELSE
                                                                          'system default'
                                                                  END + N' for offline rebuilds;' + CHAR(10)
                  + N'Starting with SQL Server 2014, lock timeout is set to '
                  + CONVERT(NVARCHAR(15), @onlinelocktimeout) + N' for online rebuilds;' + CHAR(10)
                  + N'Starting with SQL Server 2014, lock timeout action is set to '
                  + CASE
                        WHEN @abortAfterwait = 0 THEN
                            'BLOCKERS'
                        WHEN @abortAfterwait = 1 THEN
                            'SELF'
                        ELSE
                            'NONE'
                    END + N' for online rebuilds;' + CHAR(10) + CASE
                                                                    WHEN @printCmds = 1 THEN
                                                                        ' DO print'
                                                                    ELSE
                                                                        ' DO NOT print'
                                                                END + N' the sql commands;' + CHAR(10)
                  + CASE
                        WHEN @outputResults = 1 THEN
                            ' DO output'
                        ELSE
                            ' DO NOT output'
                    END + N' fragmentation levels;
Wait '                             + @defragDelay + N' (hh:mm:ss) between index operations;
Execute in'                        + CASE
                                         WHEN @debugMode = 1 THEN
                                             ' DEBUG'
                                         ELSE
                                             ' SILENT'
                                     END + N' mode.' + CHAR(10);
        ...
        BEGIN
            ...
            /* Retrieve the list of databases to loop, excluding Always On secondary replicas */
            DECLARE @sqlcmdAO2 NVARCHAR(4000),
                    @paramsAO2 NVARCHAR(50);
            ...
            SET @sqlcmdAO2
                = N'SELECT [database_id], name, 0 -- not yet scanned for fragmentation
FROM sys.databases
WHERE LOWER([name]) = ISNULL(LOWER(@dbScopeIN), LOWER([name]))	
	AND LOWER([name]) NOT IN (''master'', ''tempdb'', ''model'', ''reportservertempdb'',''semanticsdb'') -- exclude system databases
	AND [state] = 0 -- must be ONLINE
...
WHERE rs.role = 2 -- Is Secondary
	...
	AND rs.is_local = 1)';
            ...
            WHILE
            (
                SELECT COUNT(*) FROM #tblIndexDefragDatabaseList WHERE scanStatus = 0
            ) > 0
            BEGIN
                SELECT TOP 1
                       @dbID = dbID
                FROM #tblIndexDefragDatabaseList
                WHERE scanStatus = 0;

                SELECT @ixCntSource = COUNT([indexName])
                FROM dbo.tbl_AdaptiveIndexDefrag_Exceptions
                WHERE [dbID] = @dbID
                      AND exclusionMask & POWER(2, DATEPART(WEEKDAY, GETDATE()) - 1) = 0;

                SET @ixCntsqlcmd
                    = N'SELECT @ixCntTargetOUT = COUNT(si.index_id) FROM [' + DB_NAME(@dbID)
                      + N'].sys.indexes si
INNER JOIN ['                      + DB_NAME(@dbID)
                      + N'].sys.objects so ON si.object_id = so.object_id
WHERE so.is_ms_shipped = 0 AND si.index_id > 0 AND si.is_hypothetical = 0
	AND si.[object_id] NOT IN (SELECT sit.[object_id] FROM [' + DB_NAME(@dbID) + N'].sys.internal_tables AS sit)'; -- Exclude Heaps, Internal and Hypothetical objects
                SET @ixCntsqlcmdParams = N'@ixCntTargetOUT int OUTPUT';
                EXECUTE sp_executesql @ixCntsqlcmd,
                                      @ixCntsqlcmdParams,
                                      @ixCntTargetOUT = @ixCntTarget OUTPUT;

                IF @ixCntSource = @ixCntTarget
                   AND @ixCntSource > 0 -- All database objects are excluded, so skip database scanning
                BEGIN
                    ...
                    IF @debugMode = 1
                        SELECT @debugMessage
                            = N'  Database ' + DB_NAME(@dbID) + N' is fully excluded from todays work.';
                ...
                IF @ixCntSource < @ixCntTarget
                   AND @ixCntSource > 0 -- Only some database objects are excluded, so scan anyway and deal with exclusions on a granular level
                BEGIN
                    UPDATE #tblIndexDefragDatabaseList
                    SET scanStatus = 1
                    WHERE dbID = @dbID;
                    ...
                    IF @debugMode = 1
                        SELECT @debugMessage
                            = N'  Database ' + DB_NAME(@dbID) + N' is partially excluded from todays work.';
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF @debugMode = 1
                    SELECT @debugMessage = N'  Working on database [' + DB_NAME(@dbID) + N']...';
                ...
                IF @dbScope IS NOT NULL
                   AND @tblName IS NOT NULL
                    SELECT @tblNameFQN = @dbScope + N'.' + @tblName;

                /* Set partitioning rebuild options; requires Enterprise Edition */
                IF @dealMaxPartition IS NOT NULL
                   AND @editionCheck = 0
                ...
                SELECT @TableScanSQL
                    = CASE
                          WHEN @engineedition NOT IN ( 5, 6 ) THEN
                              'USE [' + DB_NAME(@dbID) + '];
'
                          ELSE
                              ''
                      END
                      + N'SELECT si.[object_id], si.index_id, si.type, sp.partition_number, 0
FROM sys.indexes si
...
INNER JOIN sys.schemas AS t ON t.[schema_id] = mst.[schema_id]'
                      + CASE
                            WHEN @dbScope IS NULL
                                 AND @tblName IS NULL THEN
                                CHAR(10) + 'LEFT JOIN [' + DB_NAME(@AID_dbID)
                                + '].dbo.tbl_AdaptiveIndexDefrag_Exceptions AS ide ON ide.[dbID] = '
                                + CONVERT(NVARCHAR(10), @dbID)
                                + ' AND ide.objectID = si.[object_id] AND ide.indexID = si.index_id'
                            ELSE
                                ''
                        END + CHAR(10) + N'WHERE mst.is_ms_shipped = 0 '
                      + CASE
                            WHEN @dbScope IS NULL
                                 AND @tblName IS NULL THEN
                                'AND (ide.exclusionMask & POWER(2, DATEPART(weekday, GETDATE())-1) = 0 OR ide.exclusionMask IS NULL)'
                            ELSE
                                ''
                        END + N'
	AND si.[object_id] NOT IN (SELECT sit.[object_id] FROM [' + DB_NAME(@dbID) + N'].sys.internal_tables AS sit)'
                      + CASE
                            WHEN @dbScope IS NOT NULL
                                 AND @tblName IS NOT NULL THEN
                                '
	AND t.name + ''.'' + mst.name = ''' + @tblName + ''';'
                            ELSE
                                ';'
                        END;
                ...
                IF @dealMaxPartition = 1
                   OR @dealMaxPartition = 0
                BEGIN
                    IF @debugMode = 1
                        RAISERROR('    Setting partition handling...', 0, 42) WITH NOWAIT;

                    SET @dealMaxPartitionSQL
                        = N'SELECT [object_id], index_id, MAX(partition_number) AS [maxPartition] FROM ['
                          + DB_NAME(@dbID)
                          + N'].sys.partitions WHERE partition_number > 1 AND [rows] > 0 GROUP BY object_id, index_id;';
                ...
                BEGIN
                    ...
                    FROM #tblIndexDefragScanWorking AS ids
                        INNER JOIN #tblIndexDefragmaxPartitionList AS mpl
                            ON ids.objectID = mpl.objectID
                ...
                (
                    SELECT COUNT(*)
                    FROM #tblIndexDefragScanWorking
                    WHERE is_done = 0
                          AND [type] IN ( 1, 2 )
                ) > 0
                BEGIN
                    ...
                    BEGIN
                        SELECT @debugMessage
                            = N'      Analyzing index ID ' + CONVERT(VARCHAR(20), @indexID) + N' on table ['
                              + OBJECT_NAME(@objectID, @dbID) + N']...';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                    END;
                    ...
                    BEGIN TRY
                        IF @getBlobfrag = 1
                        BEGIN
                            INSERT INTO dbo.tbl_AdaptiveIndexDefrag_Working
                            (
                                dbID,
                                dbName,
                                objectID,
                            ...
                            SELECT @dbID AS [dbID],
                                   QUOTENAME(DB_NAME(ps.database_id)) AS [dbName],
                    ...
                    BEGIN CATCH
                        ...
                        BEGIN
                            SET @debugMessage
                                = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                  + N' has occurred while determining which rowstore indexes to defrag. Message: '
                                  + ERROR_MESSAGE() + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10)) + N')';
                            ...
                            SET dateTimeEnd = @dateTimeEnd,
                                ...
                                errorMessage = LTRIM(@debugMessage)
...
OPTION (MAXDOP 2)'          ;
                            SET @ColumnStoreGetIXSQL_Param
                                = N'@dbID_In int, @objectID_In int, @indexID_In int, @partitionNumber_In smallint, @minPageCount_in int';
                            ...
                            EXECUTE sp_executesql @ColumnStoreGetIXSQL,
                                                  ...
                                                  @indexID_In = @indexID,
                                                  @partitionNumber_In = @partitionNumber,
                                                  @minPageCount_in = @minPageCount;
                        END TRY
                        BEGIN CATCH
                            IF @debugMode = 1
                            BEGIN
                                SET @debugMessage
                                    = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                      + N' has occurred while determining which columnstore indexes to defrag. Message: '
                                      + ERROR_MESSAGE() + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10))
                                      + N')';
                ...
                SELECT @updateSQL
                    = N'UPDATE ids		
SET schemaName = QUOTENAME(s.name), objectName = QUOTENAME(o.name), indexName = QUOTENAME(i.name), is_primary_key = i.is_primary_key, fill_factor = i.fill_factor, is_disabled = i.is_disabled, is_padded = i.is_padded, is_hypothetical = i.is_hypothetical, has_filter = '
                      + CASE
                            WHEN @sqlmajorver >= 10 THEN
                                'i.has_filter'
                            ELSE
                                '0'
                        END + N', allow_page_locks = i.allow_page_locks, type = i.type
FROM [' +           DB_NAME(@AID_dbID) + N'].dbo.tbl_AdaptiveIndexDefrag_Working ids WITH (NOLOCK)
INNER JOIN [' +     DB_NAME(@dbID) + N'].sys.objects AS o WITH (NOLOCK) ON ids.objectID = o.object_id
INNER JOIN [' +     DB_NAME(@dbID)
                      + N'].sys.indexes AS i WITH (NOLOCK) ON o.object_id = i.object_id AND ids.indexID = i.index_id
INNER JOIN [' +     DB_NAME(@dbID)
                      + N'].sys.schemas AS s WITH (NOLOCK) ON o.schema_id = s.schema_id
WHERE o.object_id = ids.objectID AND i.index_id = ids.indexID AND i.type > 0
AND o.object_id NOT IN (SELECT sit.object_id FROM [' + DB_NAME(@dbID)
                      + N'].sys.internal_tables AS sit)
...
FROM [' +               DB_NAME(@AID_dbID) + N'].dbo.tbl_AdaptiveIndexDefrag_Working ids WITH (NOLOCK)
INNER JOIN [' +         DB_NAME(@dbID)
                          + N'].sys.partitions AS p WITH (NOLOCK) ON ids.objectID = p.[object_id] AND ids.indexID = p.index_id AND ids.partitionNumber = p.partition_number
...
WHERE ids.[dbID] = ' +  CAST(@dbID AS NVARCHAR(10));
                ...
                DECLARE @tblNameOnly NVARCHAR(1000),
                        @schemaNameOnly NVARCHAR(128);
                IF @tblName IS NOT NULL
                BEGIN
                    SELECT @tblNameOnly = RIGHT(@tblName, LEN(@tblName) - CHARINDEX('.', @tblName, 1)),
                           @schemaNameOnly = LEFT(@tblName, CHARINDEX('.', @tblName, 1) - 1);
                END;
...
'
                          ELSE
                              ''
                      END + N'SELECT DISTINCT ' + CAST(@dbID AS NVARCHAR(10)) + N', ''[' + DB_NAME(@dbID)
                      + N']'', ss.[object_id], ss.stats_id, ' + CASE
                                                                    WHEN
                                                                    (
                                                                        (
                                                                            @sqlmajorver = 12
                                                                ...
                                                                END
                      + N'
	QUOTENAME(s.name), QUOTENAME(so.name), QUOTENAME(ss.name), ss.[no_recompute], ' + CASE
                                                                                          WHEN @sqlmajorver < 12 THEN
                                                                                              '0 AS '
                                                                                          ELSE
                                                                                              'ss.'
                                                                                      END
                      + N'[is_incremental], GETDATE() AS scanDate
FROM sys.stats AS ss WITH (NOLOCK)
INNER JOIN sys.objects AS so WITH (NOLOCK) ON ss.[object_id] = so.[object_id]
INNER JOIN sys.schemas AS s WITH (NOLOCK) ON so.[schema_id] = s.[schema_id]' + CASE
                                                                                   WHEN @updateStatsWhere = 0 THEN
                                                                                       'LEFT JOIN'
                                                                                   ELSE
                                                                                       'INNER JOIN'
                                                                               END
                      + -- @updateStatsWhere = 0 then table-wide statistics;
                    N' sys.indexes AS si WITH (NOLOCK) ON ss.[object_id] = si.[object_id] and ss.name = si.name'
                      + CASE
                            WHEN @dbScope IS NULL
                                 AND @tblName IS NULL THEN
                                CHAR(10) + CHAR(10) + 'LEFT JOIN [' + DB_NAME(@AID_dbID)
                                + '].dbo.tbl_AdaptiveIndexDefrag_Exceptions AS ide ON ide.[dbID] = '
                                + CONVERT(NVARCHAR(10), @dbID) + ' AND ide.objectID = so.[object_id]'
                            ELSE
                                ''
                        END
                      + CASE
                            ...
                            (
                                ...
                                CHAR(10)
                                + 'CROSS APPLY sys.dm_db_stats_properties_internal(ss.[object_id], ss.stats_id) sp'
                            ELSE
                                ''
                        END + CHAR(10) + N'WHERE is_ms_shipped = 0 ' + CASE
                                                                           WHEN @sqlmajorver >= 12 THEN
                                                                               'AND ss.is_temporary = 0'
                                                                           ELSE
                                                                               ''
                                                                       END
                      + N'
	...
	AND so.[type] IN (''U'',''V'')
	AND (si.[type] IS NULL OR si.[type] NOT IN (5,6,7))'
                      + -- Avoid error 35337
                    CASE
                        WHEN @dbScope IS NOT NULL
                             AND @tblName IS NOT NULL THEN
                            CHAR(10) + 'AND s.name = ''' + @schemaNameOnly + ''' AND so.name = ''' + @tblNameOnly
                            + ''';'
                        ELSE
                            ';'
                    END;

                INSERT INTO dbo.tbl_AdaptiveIndexDefrag_Stats_Working
                (
                    dbID,
                    dbName,
                    objectID,
            ...
            (
                ...
                WHERE ids.is_disabled = 1
                      OR ids.is_hypothetical = 1
            ) > 0
                DELETE ids
                FROM dbo.tbl_AdaptiveIndexDefrag_Working AS ids
                    LEFT JOIN dbo.tbl_AdaptiveIndexDefrag_IxDisableStatus AS ids_disable
                        ON ids.objectID = ids_disable.objectID
                           AND ids.indexID = ids_disable.indexID
                           AND ids.[dbID] = ids_disable.dbID
                WHERE ids.is_disabled = 1
                      OR ids.is_hypothetical = 1
                         AND ids_disable.indexID IS NULL;
			...
			*/
            ...
            FROM dbo.tbl_AdaptiveIndexDefrag_Working AS ids
                INNER JOIN dbo.tbl_AdaptiveIndexDefrag_Exceptions AS ide
                    ON ids.[dbID] = ide.[dbID]
        ...
        IF @debugMode = 1
            SELECT @debugMessage
                = N'Looping through batch list... There are ' + CAST(COUNT(DISTINCT indexName) AS NVARCHAR(10))
                  + N' indexes to defragment in ' + CAST(COUNT(DISTINCT dbName) AS NVARCHAR(10)) + N' database(s)!'
            FROM dbo.tbl_AdaptiveIndexDefrag_Working
            WHERE defragDate IS NULL
                  AND page_count
                  BETWEEN @minPageCount AND ISNULL(@maxPageCount, page_count)
                  AND exclusionMask & POWER(2, DATEPART(WEEKDAY, GETDATE()) - 1) = 0;
        ...
        BEGIN
            ...
            BEGIN
                SELECT @debugMessage
                    = N'Looping through batch list... There are ' + CAST(COUNT(DISTINCT statsName) AS NVARCHAR(10))
                      + N' index related statistics to update in ' + CAST(COUNT(DISTINCT idss.dbName) AS NVARCHAR(10))
                      + N' database(s)!'
                FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss
                    INNER JOIN dbo.tbl_AdaptiveIndexDefrag_Working ids
                        ON ids.[dbID] = idss.[dbID]
                           AND ids.objectID = idss.objectID
                WHERE idss.schemaName = ids.schemaName
                      AND idss.statsName = ids.indexName
                      AND idss.updateDate IS NULL
                      AND ids.exclusionMask & POWER(2, DATEPART(WEEKDAY, GETDATE()) - 1) = 0;
            END;
            ...
            BEGIN
                ...
                SELECT @debugMessage
                    = @debugMessage + N' plus ' + CAST(COUNT(DISTINCT statsName) AS NVARCHAR(10))
                      + N' other statistics to update in ' + CAST(COUNT(DISTINCT dbName) AS NVARCHAR(10))
                      + N' database(s)!'
                FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss
                WHERE idss.updateDate IS NULL
                      AND NOT EXISTS
        ...
        BEGIN
            ...
            SET @getIndexSQL
                = N'SELECT TOP 1 @objectID_Out = objectID, @indexID_Out = indexID, @dbID_Out = dbID									
FROM dbo.tbl_AdaptiveIndexDefrag_Working WHERE defragDate IS NULL ' + CASE
                                                                          WHEN @Exec_Print = 0 THEN
                                                                              'AND printStatus = 0 '
                                                                          ELSE
                                                                              ''
                                                                      END
...
ORDER BY + ' + @defragOrderColumn + N' ' + @defragSortOrder + N', objectID DESC';

            SET @getIndexSQL_Param
                = N'@objectID_Out int OUTPUT, @indexID_Out int OUTPUT, @dbID_Out int OUTPUT, @p_minPageCount int, @p_maxPageCount int';
            ...
            IF @partitionCount > 1
               AND @dealMaxPartition IS NOT NULL
               AND @editionCheck = 1
            BEGIN
                IF @debugMode = 1
            ...
            IF @updateStats = 1
            BEGIN
                SELECT @maxpartitionNumber = MAX(partitionNumber),
                       @minpartitionNumber = MIN(partitionNumber)
                FROM dbo.tbl_AdaptiveIndexDefrag_Working
                WHERE objectID = @objectID
                      AND indexID = @indexID
                      AND dbID = @dbID;
            ...
            IF @ixtype = 2
               AND @sqlmajorver < 11 -- Nonclustered and LOBs in INCLUDED columns? Up to SQL 2008R2
            BEGIN
                SELECT @LOB_SQL
                    = N'SELECT @containsLOB_OUT = COUNT(*) FROM ' + @dbName
                      + N'.sys.columns c WITH (NOLOCK)
INNER JOIN '                      + @dbName
                      + N'.sys.index_columns ic WITH (NOLOCK) ON c.[object_id] = ic.[object_id] AND c.column_id = ic.column_id
INNER JOIN '                      + @dbName
                      + N'.sys.indexes i WITH (NOLOCK) ON i.[object_id] = ic.[object_id] and i.index_id = ic.index_id
WHERE max_length = -1 AND ic.is_included_column = 1
AND i.object_id = '               + CAST(@objectID AS NVARCHAR(10)) + N' AND i.index_id = '
                      + CAST(@indexID AS NVARCHAR(10)) + N';',
            ...
            BEGIN
                SELECT @LOB_SQL
                    = N'SELECT @containsLOB_OUT = COUNT(*) FROM ' + @dbName
                      + N'.sys.columns c WITH (NOLOCK)
INNER JOIN '                      + @dbName
                      + N'.sys.indexes i WITH (NOLOCK) ON c.[object_id] = i.[object_id]
WHERE system_type_id IN (34, 35, 99) ' + CASE
                                             WHEN @sqlmajorver < 11 THEN
                                                 'OR max_length = -1 '
                                             ELSE
                                                 ''
                                         END + N'AND i.object_id = ' + CAST(@objectID AS NVARCHAR(10))
                      + N' AND i.index_id = ' + CAST(@indexID AS NVARCHAR(10)) + N';',
                ...
                IF @debugMode = 1
                   ...
                   AND @onlineRebuild = 1
                    RAISERROR(
                                 '    Online rebuild not possible on clustered index when certain LOBs exist in table...',
                                 0,
                                 42
                             ) WITH NOWAIT;
            END;
            ...
            BEGIN
                SELECT @CStore_SQL
                    = N'SELECT @containsColumnstore_OUT = COUNT(*) FROM ' + @dbName
                      + N'.sys.indexes i WITH (NOLOCK) WHERE i.object_id = ' + CAST(@objectID AS NVARCHAR(10))
                      + N' AND i.type IN (5,6);',
                       @CStore_SQL_Param = N'@containsColumnstore_OUT int OUTPUT';
                EXECUTE sp_executesql @CStore_SQL,
                                      @CStore_SQL_Param,
                                      @containsColumnstore_OUT = @containsColumnstore OUTPUT;

                IF @debugMode = 1
                   AND @containsColumnstore > 0
                   AND @onlineRebuild = 1
            ...
            IF @forceRescan = 0
            BEGIN
                SELECT @allowPageLockSQL
                    = N'SELECT @allowPageLocks_OUT = allow_page_locks FROM ' + @dbName
                      + N'.sys.indexes WHERE [object_id] = ' + CAST(@objectID AS NVARCHAR(10)) + N' AND [index_id] = '
                      + CAST(@indexID AS NVARCHAR(10)) + N';',
                       @allowPageLockSQL_Param = N'@allowPageLocks_OUT bit OUTPUT';
                EXECUTE sp_executesql @allowPageLockSQL,
                                      @allowPageLockSQL_Param,
                                      @allowPageLocks_OUT = @allowPageLocks OUTPUT;
            END;
            ELSE
            BEGIN
                SELECT @allowPageLocks = [allow_page_locks]
                FROM dbo.tbl_AdaptiveIndexDefrag_Working
                WHERE objectID = @objectID
                      AND indexID = @indexID
                      AND [dbID] = @dbID;
            ...
            BEGIN
                SELECT @debugMessage
                    = N'    Found ' + CONVERT(NVARCHAR(10), @fragmentation) + N' percent fragmentation on index '
                      + @indexName + N'...';
                RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
            END;
            ...
            IF (
                   @fragmentation < @rebuildThreshold
                   AND @ixtype IN ( 1, 2 )
                   AND @has_filter = 0
                   AND @allowPageLocks = 1
               )
               OR
               (
                   @fragmentation < @rebuildThreshold_cs
                   AND @ixtype IN ( 5, 6 )
               )
            BEGIN
                IF @debugMode = 1
                ...
                /* Set Reorg command */
                SET @sqlcommand
                    = N'ALTER INDEX ' + @indexName + N' ON ' + @dbName + N'.' + @schemaName + N'.' + @objectName
                      + N' REORGANIZE';
            ...
            BEGIN
                ...
                IF @is_padded = 1
                   AND
                   (
                       @dealMaxPartition IS NULL
                       OR
                       (
                           @dealMaxPartition IS NOT NULL
                           AND @partitionCount = 1
                       )
                   )
                ...
                IF @dataCompression IN ( 'NONE', 'PAGE', 'ROW' )
                   ...
                   (
                       ...
                       OR @editionCheck = 1
                   ) -- SQL 2016 SP1 or higher	for compression in non-EE
                    SET @rebuildcommand = @rebuildcommand + N'DATA_COMPRESSION = ' + @dataCompression + N', ';
                ...
                IF @dataCompression IS NULL
                   ...
                   (
                    ...
                    SET @rebuildcommand = @rebuildcommand + N'DATA_COMPRESSION = ' + @currCompression + N', ';
                ...
                IF @dataCompression IN ( 'NONE', 'PAGE', 'ROW' )
                   ...
                   AND @editionCheck = 1
                    SET @rebuildcommand
                        = @rebuildcommand + N'DATA_COMPRESSION = ' + @dataCompression + N' ON PARTITIONS ('
                          + CAST(@partitionNumber AS NVARCHAR(10)) + N'), ';
                ...
                IF @dataCompression IS NULL
                   ...
                   AND @editionCheck = 1
                    SET @rebuildcommand
                        = @rebuildcommand + N'DATA_COMPRESSION = ' + @currCompression + N' ON PARTITIONS ('
                          + CAST(@partitionNumber AS NVARCHAR(10)) + N'), ';
                ...
                IF @sqlmajorver > 11
                   ...
                   AND @containsLOB = 0
                    SELECT @rebuildcommand
                        = @rebuildcommand + N'ONLINE = ON (WAIT_AT_LOW_PRIORITY (MAX_DURATION = '
                          + CONVERT(NVARCHAR(15), @onlinelocktimeout) + N', ABORT_AFTER_WAIT = '
                          + CASE
                                WHEN @abortAfterwait = 0 THEN
                                    'BLOCKERS'
                                WHEN @abortAfterwait = 1 THEN
                ...
                IF @fillfactor = 1
                   ...
                   AND @ixtype IN ( 1, 2 )
                    SET @rebuildcommand
                        = @rebuildcommand + N'FILLFACTOR = ' + CONVERT(   NVARCHAR,
                                                                          CASE
                                                                              WHEN @fill_factor = 0 THEN
                                                                                  100
                                                                              ELSE
                                                                                  @fill_factor
                                                                          END
                ...
                IF @maxDopRestriction IS NOT NULL
                   AND @editionCheck = 1
                    SET @rebuildcommand
                        = @rebuildcommand + N'MAXDOP = ' + CAST(@maxDopRestriction AS VARCHAR(3)) + N');';
                ELSE
                ...
                /* Is stat incremental? */
                SELECT TOP 1
                       @stats_isincremental = [is_incremental]
                FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
                WHERE dbName = @dbName
                ...
                DECLARE @sqlcmdRI NVARCHAR(100),
                        @paramsRI NVARCHAR(50),
                        @HasRI INT;
                SET @HasRI = 0;
                IF @sqlmajorver > 14
                BEGIN
                    SET @sqlcmdRI = CASE
                                        ...
                                        ELSE
                                            ''
                                    END + N'; SELECT @HasRI_OUT = COUNT(*) FROM sys.index_resumable_operations';
                ...
                ELSE IF (
                        ...
                        AND @sqldisablecommand IS NULL
                        AND @ixtype IN ( 1, 2 )
                BEGIN
                    SET @sqlprecommand
                        = N'UPDATE STATISTICS ' + @dbName + N'.' + @schemaName + N'.' + @objectName + N' ('
                          + @indexName + N')';
                    ...
                    (
                        @sqlmajorver >= 14
                        AND @sqlbuild >= 3015
                    )/*OR (@sqlmajorver = 13 AND @sqlbuild >= xxxx)*/
                           ) -- SQL 2016 SP2, SQL 2017 CU3 or higher for MAXDOP in stats
                        SET @sqlprecommand
                            = @sqlprecommand + N' WITH MAXDOP = ' + CAST(@maxDopRestriction AS VARCHAR(3)) + N'; ';
                    ELSE
                        SET @sqlprecommand = @sqlprecommand + N'; ';
                END;
                ELSE IF @sqlmajorver >= 13
                        AND @partitionCount > 1
                        AND @stats_isincremental = 1
                        AND @sqldisablecommand IS NULL
                        AND @ixtype IN ( 1, 2 )
                BEGIN
                    SET @sqlprecommand
                        = N'UPDATE STATISTICS ' + @dbName + N'.' + @schemaName + N'.' + @objectName + N' ('
                          + @indexName + N') WITH RESAMPLE ON PARTITIONS(' + CONVERT(NVARCHAR(10), @partitionNumber)
                          + N')';
                ...
                SET @sqlcommand
                    = N'ALTER INDEX ' + @indexName + N' ON ' + @dbName + N'.' + @schemaName + N'.' + @objectName
                      + REPLACE(@rebuildcommand, ', )', ')');

                /* For offline rebuilds, set lock timeout if not default */
                IF @onlineRebuild = 0
                   AND @offlinelocktimeout > -1
                   AND @offlinelocktimeout IS NOT NULL
                    SET @sqlcommand
                        = N'SET LOCK_TIMEOUT ' + CONVERT(NVARCHAR(15), @offlinelocktimeout) + N'; ' + @sqlcommand;
            END;
            ELSE
            BEGIN
                ...
                BEGIN
                    SET @debugMessage
                        = N'We are unable to defrag index ' + @indexName + N' on table ' + @dbName + N'.' + @schemaName
                          + N'.' + @objectName;
                    RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                END;
            END;
            ...
            IF @operationFlag = 0
               AND @sqlprecommand IS NOT NULL
                SET @sqlprecommand = NULL;

            IF @operationFlag = 0
               AND @sqldisablecommand IS NOT NULL
                SET @sqldisablecommand = NULL;
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    SELECT @dbID,
                           ...
                           @fill_factor,
                           @dateTimeStart,
                           ISNULL(@sqlprecommand, '') + @sqlcommand;
                END;
                ELSE
                BEGIN
                    INSERT INTO dbo.tbl_AdaptiveIndexDefrag_log
                ...
                IF @sqlprecommand IS NULL
                   AND @sqldisablecommand IS NULL
                BEGIN
                    SET @debugMessage = N'     ' + @sqlcommand;
                END;
                ELSE IF @sqlprecommand IS NOT NULL
                        AND @sqldisablecommand IS NULL
                BEGIN
                    SET @debugMessage = N'     ' + @sqlprecommand + CHAR(10) + N'     ' + @sqlcommand;
                END;
                ELSE IF @sqlprecommand IS NULL
                        AND @sqldisablecommand IS NOT NULL
                BEGIN
                    SET @debugMessage = N'     ' + @sqldisablecommand + CHAR(10) + N'     ' + @sqlcommand;
                END;
                ELSE IF @sqlprecommand IS NOT NULL
                        AND @sqldisablecommand IS NOT NULL
                BEGIN
                    SET @debugMessage
                        = N'     ' + @sqldisablecommand + CHAR(10) + N'     ' + @sqlprecommand + CHAR(10) + N'     '
                          + @sqlcommand;
                END;
                ...
                IF (
                   ...
                   AND @sqlcommand IS NOT NULL
                BEGIN
                    RAISERROR('  Printing SQL statements...', 0, 42) WITH NOWAIT;
                    SET @debugMessage = N'   Executing: ' + CHAR(10) + @debugMessage;
                    RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                END;
                ...
                BEGIN
                    BEGIN TRY
                        ...
                        IF @partitionCount > 1
                           ...
                           AND @editionCheck = 1
                        BEGIN
                            INSERT INTO dbo.tbl_AdaptiveIndexDefrag_Stats_log
                            (
                                dbID,
                                dbName,
                                objectID,
                    ...
                    BEGIN CATCH
                        ...
                        SET dateTimeEnd = GETDATE(),
                            durationSeconds = -1,
                            errorMessage = 'Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                           + ' has occurred executing the pre-command for index ' + @indexName
                                           + '. Message: ' + ERROR_MESSAGE() + ' (Line Number: '
                        ...
                        BEGIN
                            SET @debugMessage
                                = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                  + N' has occurred executing the pre-command. Message: ' + ERROR_MESSAGE()
                                  + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10)) + N')';
                            RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                        --RAISERROR('     An error has occurred executing the pre-command! Please review the tbl_AdaptiveIndexDefrag_log table for details.', 0, 42) WITH NOWAIT;
                        END;
                    END CATCH;
                ...
                IF @operationFlag = 1
                   ...
                   AND
                   (
                       @sqldisablecommand IS NOT NULL
                       OR LEN(@sqldisablecommand) > 0
                   )
                BEGIN
                    BEGIN TRY
                    ...
                    BEGIN CATCH
                        ...
                        BEGIN
                            SET @debugMessage
                                = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                  + N' has occurred executing the disable index command. Message: ' + ERROR_MESSAGE()
                                  + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10)) + N')';
                            RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                ...
                BEGIN CATCH
                    ...
                    SET dateTimeEnd = GETDATE(),
                        durationSeconds = -1,
                        errorMessage = 'Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                                       + ' has occurred executing this command. Message: ' + ERROR_MESSAGE()
                    ...
                    BEGIN
                        SET @debugMessage
                            = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER())
                              + N' has occurred executing this command. Message: ' + ERROR_MESSAGE()
                              + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10)) + N')';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                ...
                BEGIN
                    ...
                    SET defragDate = ISNULL(@dateTimeEnd, GETDATE()),
                        printStatus = 1
                    WHERE dbID = @dbID
                          AND objectID = @objectID
                          AND indexID = @indexID
                ...
                BEGIN
                    ...
                    SET updateDate = ISNULL(@dateTimeEnd, GETDATE()),
                        printStatus = 1
                    WHERE objectID = @objectID
                          AND dbID = @dbID
                          AND statsName = @indexName;
            ...
            BEGIN
                ...
                IF @sqlprecommand IS NULL
                   AND
                   (
                       @sqldisablecommand IS NULL
                       OR @sqldisablecommand = ''
                   )
                BEGIN
                    SET @debugMessage = N'     ' + @sqlcommand;
                END;
            ...
            IF @operationFlag = 0
               AND @updateStats = 1 -- When reorganizing, update stats afterwards
               AND @updateStatsWhere = 0
               AND @ixtype NOT IN ( 5, 6, 7 )
            BEGIN
                IF @debugMode = 1
                ...
                BEGIN TRY
                    SET @getStatSQL
                        = N'SELECT TOP 1 @statsID_Out = idss.statsID FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss WHERE idss.updateDate IS NULL '
                          + CASE
                                WHEN @Exec_Print = 0 THEN
                                    'AND idss.printStatus = 0 '
                                ELSE
                                    ''
                            END + N' AND idss.[dbID] = ' + CONVERT(NVARCHAR, @dbID) + N' AND idss.statsName = '''
                          + @indexName + N'''' + N' AND idss.objectID = ' + CONVERT(NVARCHAR, @objectID)
                          + N' AND EXISTS (SELECT TOP 1 objectID FROM dbo.tbl_AdaptiveIndexDefrag_Working ids WHERE ids.[dbID] = idss.[dbID] AND ids.objectID = idss.objectID AND idss.statsName = ids.indexName AND idss.updateDate IS NULL AND ids.defragDate IS NOT NULL AND ids.exclusionMask & POWER(2, DATEPART(weekday, GETDATE())-1) = 0)';
                    SET @getStatSQL_Param = N'@statsID_Out int OUTPUT';
                    EXECUTE sp_executesql @getStatSQL,
                                          @getStatSQL_Param,
                                          @statsID_Out = @statsID OUTPUT;
                    ...
                    BEGIN
                        SET @debugMessage
                            = N'     Error ' + CONVERT(VARCHAR(20), ERROR_NUMBER())
                              + N' has occurred while determining which statistic to update. Message: '
                              + ERROR_MESSAGE() + N' (Line Number: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + N')';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                    END;
                END CATCH;
                ...
                SELECT TOP 1
                       @statsName = statsName,
                       @partitionNumber = partitionNumber,
                       @stats_norecompute = [no_recompute],
                       @stats_isincremental = [is_incremental]
                FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
                WHERE objectID = @objectID
                      AND statsID = @statsID
                ...
                BEGIN
                    SET @debugMessage
                        = N'   Determining modification row counter for statistic ' + @statsName
                          + N' on table or view ' + @objectName + N' of DB ' + @dbName + N'...';
                ...
                IF (
                   ...
                   AND ISNULL(@stats_isincremental, 1) = 1
                   AND
                   (
                       @statsSample IS NULL
                       OR UPPER(@statsSample) = 'RESAMPLE'
                   )
                BEGIN
                    IF @debugMode = 1
                    ...
                    SELECT @rowmodctrSQL
                        = CASE
                              ...
                              ELSE
                                  ''
                          END
                          + N'; SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_incremental_stats_properties(@statsobjectID_In, @statsID_In) WHERE partition_number = @partitionNumber_In;';
                END;
                ELSE IF (
                            (
                                @sqlmajorver = 12
                    ...
                    SELECT @rowmodctrSQL
                        = CASE
                              ...
                              ELSE
                                  ''
                          END
                          + N'; SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_stats_properties_internal(@statsobjectID_In, @statsID_In) WHERE partition_number = @partitionNumber_In;';
                END;
                ELSE IF (
                            (
                                @sqlmajorver = 10
                        ...
                        AND
                        (
                            ISNULL(@stats_isincremental, 1) = 0
                            OR UPPER(@statsSample) = 'FULLSCAN'
                            OR ISNUMERIC(@statsSample) = 1
                        )
                BEGIN
                    IF @debugMode = 1
                    ...
                    SELECT @rowmodctrSQL
                        = CASE
                              ...
                              ELSE
                                  ''
                          END
                          + N'; SELECT @rowmodctr_Out = ISNULL(modification_counter,0), @rows_Out = ISNULL(rows,0), @rows_sampled_Out = ISNULL(rows_sampled,0) FROM sys.dm_db_stats_properties(@statsobjectID_In, @statsID_In);';
                END;
                ELSE
                BEGIN
                    IF @debugMode = 1
                    ...
                    SELECT TOP 1
                           @surrogateStatsID = indexID
                    FROM dbo.tbl_AdaptiveIndexDefrag_Working (NOLOCK)
                    WHERE objectID = @statsobjectID
                          AND indexName = @statsName;
                    SELECT @rowmodctrSQL
                        = CASE
                              WHEN @engineedition NOT IN ( 5, 6 ) THEN
                                  'USE ' + QUOTENAME(@dbName)
                              ELSE
                                  ''
                          END
                          + N'; SELECT @rowmodctr_Out = SUM(ISNULL(rowmodctr,0)), @rows_Out = ISNULL(rowcnt,0), @rows_sampled_Out = -1 FROM sys.sysindexes WHERE id = @statsobjectID_In AND indid = @statsID_In AND rowmodctr > 0;';

                    IF @statsID IS NULL
                        SET @statsID = @surrogateStatsID;
                END;
                SET @rowmodctrSQL_Param
                    = N'@partitionNumber_In smallint, @statsobjectID_In int, @statsID_In int, @rowmodctr_Out bigint OUTPUT, @rows_Out bigint OUTPUT, @rows_sampled_Out bigint OUTPUT';

                BEGIN TRY
                    EXECUTE sp_executesql @rowmodctrSQL,
                                          @rowmodctrSQL_Param,
                                          @statsobjectID_In = @statsobjectID,
                                          @statsID_In = @statsID,
                                          @partitionNumber_In = @partitionNumber,
                                          @rowmodctr_Out = @rowmodctr OUTPUT,
                                          @rows_Out = @rows OUTPUT,
                                          @rows_sampled_Out = @rows_sampled OUTPUT;
                    SET @rowmodctr =
                    (
                        SELECT ISNULL(@rowmodctr, 0)
                    );
                END TRY
                BEGIN CATCH
                    IF @debugMode = 1
                    BEGIN
                        SET @debugMessage
                            = N'     Error ' + CONVERT(VARCHAR(20), ERROR_NUMBER())
                              + N' has occurred while determining row modification counter. Message: '
                              + ERROR_MESSAGE() + N' (Line Number: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + N')';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                    END;
                END CATCH;
                ...
                BEGIN
                    SELECT @debugMessage
                        = N'     Found a row modification counter of ' + CONVERT(NVARCHAR(15), @rowmodctr) + N' and '
                          + CONVERT(NVARCHAR(15), @record_count) + N' rows'
                          + CASE
                                WHEN ISNULL(@stats_isincremental, 0) = 1 THEN
                                    ' on partition ' + CONVERT(NVARCHAR(15), @partitionNumber)
                                ELSE
                                    ''
                            END + N'...';
                    RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
				...
				*/
                IF (
                       @statsThreshold IS NOT NULL
                       AND @statsThreshold
                   BETWEEN 0.001 AND 100.0
                       AND @statsMinRows IS NULL
                       AND (@rowmodctr * 100) / @record_count >= @statsThreshold
                   )
                   OR
                   (
                       @statsThreshold IS NOT NULL
                       AND @statsThreshold
                   BETWEEN 0.001 AND 100.0
                       AND @statsMinRows IS NOT NULL
                       AND @record_count >= @statsMinRows
                       AND (@rowmodctr * 100) / @record_count >= @statsThreshold
                   )
                   OR
                   (
                       @statsThreshold IS NULL
                       AND
                       (
                           (
                               @record_count
                   BETWEEN 6 AND 500
                           ...
                           (
                               @record_count
                   BETWEEN 501 AND 10000
                               AND
                               (
                                   @rowmodctr >= (@record_count * 20) / 100 + 500
                                   OR @rowmodctr >= SQRT(@record_count * 1000)
                               )
                           )
                           OR -- 500 + 20 percent or simulate TF 2371
                           (
                               @record_count
                   BETWEEN 10001 AND 100000
                               AND
                               (
                                   @rowmodctr >= (@record_count * 15) / 100 + 500
                                   OR @rowmodctr >= SQRT(@record_count * 1000)
                               )
                           )
                           OR -- 500 + 15 percent or simulate TF 2371
                           (
                               @record_count
                   BETWEEN 100001 AND 1000000
                               AND
                               (
                                   @rowmodctr >= (@record_count * 10) / 100 + 500
                                   OR @rowmodctr >= SQRT(@record_count * 1000)
                               )
                           )
                           OR -- 500 + 10 percent or simulate TF 2371
                           (
                               @record_count >= 1000001
                               AND
                               (
                                   @rowmodctr >= (@record_count * 5) / 100 + 500
                                   OR @rowmodctr >= SQRT(@record_count * 1000)
                               )
                           ) -- 500 + 5 percent or simulate TF 2371
                       )
                   )
                BEGIN
                    SET @sqlcommand2
                    ...
                    IF UPPER(@statsSample) = 'FULLSCAN'
                       ...
                       (
                           @partitionNumber = 1
                           OR @partitionNumber = @maxpartitionNumber
                       )
                        SET @sqlcommand2 = @sqlcommand2 + N' WITH FULLSCAN';
                    IF ISNUMERIC(@statsSample) = 1
                       AND
                       (
                           @partitionNumber = 1
                           OR @partitionNumber = @maxpartitionNumber
                       )
                        SET @sqlcommand2 = @sqlcommand2 + N' WITH SAMPLE ' + @statsSample + N' PERCENT';
                    IF UPPER(@statsSample) = 'RESAMPLE'
                        SET @sqlcommand2 = @sqlcommand2 + N' WITH RESAMPLE';

                    IF (
                           UPPER(@statsSample) = 'FULLSCAN'
                           OR ISNUMERIC(@statsSample) = 1
                       )
                       AND @persistStatsSample = 1
                    ...
                    IF @partitionCount > 1
                       ...
                       AND UPPER(@sqlcommand2) LIKE '%WITH%'
                        SET @sqlcommand2
                            = @sqlcommand2 + N' ON PARTITIONS(' + CONVERT(NVARCHAR(10), @partitionNumber) + N');';
                    ELSE IF @partitionCount > 1
                        ...
                        SET @sqlcommand2
                            = @sqlcommand2 + N' WITH RESAMPLE ON PARTITIONS(' + CONVERT(NVARCHAR(10), @partitionNumber)
                              + N')';

                    IF @stats_norecompute = 1
                       AND UPPER(@sqlcommand2) LIKE '%WITH%'
                        SET @sqlcommand2 = @sqlcommand2 + N' ,NORECOMPUTE';
                    ELSE IF @stats_norecompute = 1
                            AND @sqlcommand2 NOT LIKE '%WITH%'
                ...
                /* Are we executing the SQL? If so, do it */
                IF @Exec_Print = 1
                   AND @sqlcommand2 IS NOT NULL
                BEGIN
                    SET @debugMessage = N'     ' + @sqlcommand2;
                    ...
                    END;
                END;
                ELSE IF @Exec_Print = 1
                        AND @sqlcommand2 IS NULL
                BEGIN
                    IF @debugMode = 1
                    BEGIN
                        SELECT @debugMessage
                            = N'     No need to update statistic ' + @statsName + N' on table or view ' + @objectName
                              + N' of DB ' + @dbName
                              + CASE
                                    WHEN @stats_isincremental = 1 THEN
                                        ', on partition ' + CONVERT(NVARCHAR(10), @partitionNumber)
                                    ELSE
                                        ''
                                END + N'...';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                    ...
                    BEGIN
                        SELECT @debugMessage
                            = N'     -- No need to update statistic ' + @statsName + N' on table or view '
                              + @objectName + N' of DB ' + @dbName
                              + CASE
                                    WHEN @stats_isincremental = 1 THEN
                                        ', on partition ' + CONVERT(NVARCHAR(10), @partitionNumber)
                                    ELSE
        ...
        BEGIN
            ...
            ) > 0 --AND NOT EXISTS (SELECT TOP 1 objectID FROM dbo.tbl_AdaptiveIndexDefrag_Working ids WHERE ids.[dbID] = idss.[dbID] AND ids.objectID = idss.objectID AND idss.statsName = ids.indexName AND idss.updateDate IS NULL AND ids.exclusionMask & POWER(2, DATEPART(weekday, GETDATE())-1) = 0)) > 0
            BEGIN
                /* Check to see if we need to exit loop because of our time limit */
                IF ISNULL(@endDateTime, GETDATE()) < GETDATE()
                    RAISERROR('Time limit has been exceeded for this maintenance window!', 16, 42) WITH NOWAIT;
                ...
                BEGIN
                    SELECT TOP 1
                           @statsID = idss.statsID,
                           @dbID = idss.dbID,
                           @statsobjectID = idss.objectID,
                           @dbName = idss.dbName,
                           @statsobjectName = idss.objectName,
                           @statsschemaName = idss.schemaName,
                           @partitionNumber = idss.partitionNumber
                    FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss
                ...
                BEGIN
                    ...
                    FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss
                    WHERE idss.updateDate IS NULL
                          AND idss.printStatus = 0; --AND NOT EXISTS (SELECT TOP 1 objectID FROM dbo.tbl_AdaptiveIndexDefrag_Working ids WHERE ids.[dbID] = idss.[dbID] AND ids.objectID = idss.objectID AND idss.statsName = ids.indexName AND idss.updateDate IS NULL AND ids.exclusionMask & POWER(2, DATEPART(weekday, GETDATE())-1) = 0) 
                END;
                ...
                BEGIN TRY
                    SELECT @getStatSQL
                        = CASE
                              ...
                              ELSE
                                  ''
                          END + N'; SELECT TOP 1 @record_count_Out = p.[rows] FROM [' + DB_NAME(@AID_dbID)
                          + N'].dbo.tbl_AdaptiveIndexDefrag_Stats_Working idss INNER JOIN sys.partitions AS p ON idss.objectID = p.[object_id] AND idss.partitionNumber = p.partition_number WHERE idss.updateDate IS NULL '
                          + CASE
                                WHEN @Exec_Print = 0 THEN
                                    'AND idss.printStatus = 0 '
                                ELSE
                                    ''
                            END
                          + N' AND idss.statsID = @statsID_In AND idss.dbID = @dbID_In AND idss.objectID = @statsobjectID_In';
                    SET @getStatSQL_Param
                        = N'@statsID_In int, @dbID_In int, @statsobjectID_In int, @record_count_Out bigint OUTPUT';
                    EXECUTE sp_executesql @getStatSQL,
                                          @getStatSQL_Param,
                                          @statsID_In = @statsID,
                                          @dbID_In = @dbID,
                                          @statsobjectID_In = @statsobjectID,
                                          @record_count_Out = @record_count OUTPUT;
                END TRY
                BEGIN CATCH
                    IF @debugMode = 1
                    BEGIN
                        SET @debugMessage
                            = N'     Error ' + CONVERT(VARCHAR(20), ERROR_NUMBER())
                              + N' has occurred while getting stat associated table row count. Message: '
                              + ERROR_MESSAGE() + N' (Line Number: ' + CAST(ERROR_LINE() AS VARCHAR(10)) + N')';
                        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                    END;
                END CATCH;
                ...
                BEGIN
                    SET @debugMessage
                        = N'   Determining modification row counter for statistic ' + @statsName
                          + N' on table or view ' + @statsobjectName + N' of DB ' + @dbName + N'...';
                    RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
                END;
				...
				*/
                ...
                BEGIN
                    SET @sqlcommand2
                        = N'UPDATE STATISTICS ' + @dbName + N'.' + @statsschemaName + N'.' + @statsobjectName + N' ('
                          + @statsName + N')';
                    IF UPPER(@statsSample) = 'FULLSCAN'
                       AND
                       (
                ...
                BEGIN
                    ...
                    BEGIN
                        SET @debugMessage
                            = N'     No need to update statistic ' + @statsName + N' on DB ' + @dbName
                              + N' and object ' + @statsobjectName
                              + CASE
                                    WHEN @stats_isincremental = 1 THEN
                                        ', on partition ' + CONVERT(NVARCHAR(10), @partitionNumber)
                                    ELSE
        ...
        BEGIN
            ...
            BEGIN
                ...
                SELECT [dbName],
                       ...
                       indexName,
                       partitionNumber,
                       CONVERT(DECIMAL(9, 2), fragmentation) AS fragmentation,
                       page_count,
                       fill_factor,
                       range_scan_count,
                       defragDate
                ...
                IF @debugMode = 1
                   ...
                   (
                       ...
                       WHERE errorMessage IS NOT NULL
                             AND dateTimeStart >= @startDateTime
                   ) > 0
                BEGIN
    ...
    BEGIN CATCH
        SET @debugMessage
            = N'     Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER()) + N' has occurred. Message: ' + ERROR_MESSAGE()
              + N' (Line Number: ' + CAST(ERROR_LINE() AS NVARCHAR(10)) + N')';
        RAISERROR(@debugMessage, 0, 42) WITH NOWAIT;
    END CATCH;
    /* Reset printStatus */
    ...
    BEGIN
        ...
        BEGIN
            IF
            (
                SELECT COUNT([errorMessage]) FROM dbo.vw_LastRun_Log
            ) > 0
            AND @ignoreDropObj = 0
            BEGIN
                RAISERROR(
                             'Defrag job found execution errors! Please review the tbl_AdaptiveIndexDefrag_log table for details.',
                             16,
                             42
                         ) WITH NOWAIT;
                RETURN -1;
        ...
        BEGIN
            IF
            (
                SELECT COUNT([errorMessage])
                FROM dbo.vw_LastRun_Log
                WHERE [errorMessage] NOT LIKE 'Table%does not exist%'
            ) > 0
            BEGIN
                RAISERROR(
                             'Defrag job found execution errors! Please review the tbl_AdaptiveIndexDefrag_log table for details.',
...
FROM dbo.tbl_AdaptiveIndexDefrag_log
WHERE errorMessage IS NOT NULL
      AND dateTimeStart >= DATEADD(dd, DATEDIFF(dd, 0, GETDATE()), -30)
UNION ALL
SELECT TOP 100 PERCENT
       dbName,
       objectName,
...
FROM dbo.tbl_AdaptiveIndexDefrag_log
WHERE errorMessage IS NOT NULL
      AND dateTimeStart >= DATEADD(hh, -24, GETDATE())
UNION ALL
SELECT TOP 100 PERCENT
       dbName,
       objectName,
...
SELECT TOP 100 PERCENT
       ...
       indexName,
       partitionNumber,
       AVG(durationSeconds) AS Avg_durationSeconds
FROM dbo.tbl_AdaptiveIndexDefrag_log
...
GROUP BY dbName,
         ...
         indexName,
         partitionNumber
ORDER BY AVG(durationSeconds) DESC,
         dbName,
         objectName,
         indexName,
         partitionNumber;
...
AS
SELECT TOP 100 PERCENT
       'Most fragmented' AS Comment,
       dbName,
       objectName,
       indexName,
       partitionNumber,
       CONVERT(DECIMAL(9, 2), AVG(fragmentation)) AS Avg_fragmentation
FROM dbo.tbl_AdaptiveIndexDefrag_Working
...
AS
SELECT TOP 100 PERCENT
       'Avg_Sampling' AS Comment,
       dbName,
       objectName,
       partitionNumber,
       statsName,
       CAST((rows_sampled / ([rows] * 1.00)) * 100.0 AS DECIMAL(5, 2)) AS sampling,
       dateTimeStart,
       dateTimeEnd,
       sqlStatement,
       errorMessage
...
SELECT TOP 100 PERCENT
       ...
       indexName,
       partitionNumber,
       AVG(page_count) * 8 AS Avg_size_KB,
       fill_factor
FROM dbo.tbl_AdaptiveIndexDefrag_Working
WHERE defragDate >= DATEADD(dd, DATEDIFF(dd, 0, GETDATE()), -30)
GROUP BY dbName,
...
AS
SELECT TOP 100 PERCENT
       'Most used' AS Comment,
       dbName,
       objectName,
       indexName,
       partitionNumber,
       AVG(range_scan_count) AS Avg_range_scan_count
FROM dbo.tbl_AdaptiveIndexDefrag_Working
WHERE defragDate >= DATEADD(dd, DATEDIFF(dd, 0, GETDATE()), -30)
GROUP BY dbName,
         objectName,
...
SELECT TOP 100 PERCENT
       ...
       [durationSeconds],
       CASE
           WHEN [sqlStatement] LIKE '%REORGANIZE%' THEN
               'Reorg'
           ELSE
               'Rebuild'
       END AS [Operation],
...
(
    SELECT TOP 1
           minIxDate = CASE
                           WHEN defragDate IS NULL THEN
                               CONVERT(DATETIME, CONVERT(NVARCHAR, scanDate, 112))
                           ELSE
                               CONVERT(DATETIME, CONVERT(NVARCHAR, defragDate, 112))
                       END
    FROM [dbo].[tbl_AdaptiveIndexDefrag_Working]
    ORDER BY defragDate ASC,
             scanDate ASC
) AS minDateIxCte
...
(
    SELECT TOP 1
           minStatDate = CASE
                             WHEN updateDate IS NULL THEN
                                 CONVERT(DATETIME, CONVERT(NVARCHAR, scanDate, 112))
                             ELSE
                                 CONVERT(DATETIME, CONVERT(NVARCHAR, updateDate, 112))
                         END
    FROM [dbo].[tbl_AdaptiveIndexDefrag_Stats_Working]
...
IF @dbname IS NULL
BEGIN;
    WITH cte1 ([Database_Name], Total_indexes)
    AS (SELECT [dbName],
               COUNT(indexID) AS Total_Indexes
        FROM dbo.tbl_AdaptiveIndexDefrag_Working
        GROUP BY [dbName]),
         cte2 ([Database_Name], Defraged_Indexes)
    AS (SELECT [dbName],
               COUNT(indexID) AS Total_Indexes
        FROM dbo.tbl_AdaptiveIndexDefrag_Working
        WHERE defragDate IS NOT NULL
              OR printStatus = 1
        GROUP BY [dbName]),
         cte3 ([Database_Name], Total_statistics)
    AS (SELECT [dbName],
               COUNT(statsID) AS Total_statistics
        FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
        GROUP BY [dbName]),
         cte4 ([Database_Name], Updated_statistics)
    AS (SELECT [dbName],
               COUNT(statsID) AS Updated_statistics
        FROM dbo.tbl_AdaptiveIndexDefrag_Stats_Working
        WHERE updateDate IS NOT NULL
              OR printStatus = 1
        GROUP BY [dbName])
    SELECT cte1.[Database_Name],
           SUM(cte1.Total_indexes) AS Total_indexes,
           SUM(ISNULL(cte2.Defraged_Indexes, 0)) AS Defraged_Indexes,
           SUM(cte3.Total_statistics) AS Total_statistics,
           SUM(ISNULL(cte4.Updated_statistics, 0)) AS Updated_statistics
    FROM cte1
        INNER JOIN cte3
            ON cte1.Database_Name = cte3.Database_Name
        LEFT JOIN cte2
            ON cte1.Database_Name = cte2.Database_Name
        LEFT JOIN cte4
            ON cte1.Database_Name = cte4.Database_Name
...
CREATE PROCEDURE usp_AdaptiveIndexDefrag_Exceptions
    @exceptionMask_DB NVARCHAR(255) = NULL,
...
IF @exceptionMask_DB IS NULL
   OR QUOTENAME(@exceptionMask_DB) NOT IN
      (
          SELECT QUOTENAME(name) FROM sys.databases
      )
    RAISERROR('Syntax error. Please input a valid database name.', 15, 42) WITH NOWAIT;

IF @exceptionMask_days IS NOT NULL
   AND
   (
       @exceptionMask_days NOT LIKE '___'
       AND @exceptionMask_days NOT LIKE '___,___'
       AND @exceptionMask_days NOT LIKE '___,___,___'
       AND @exceptionMask_days NOT LIKE '___,___,___,___'
       AND @exceptionMask_days NOT LIKE '___,___,___,___,___'
       AND @exceptionMask_days NOT LIKE '___,___,___,___,___,___'
       AND @exceptionMask_days NOT LIKE '___,___,___,___,___,___,___'
   )
    RAISERROR(
                 'Syntax error. Please input weekdays in short form, between commas, or leave NULL to always exclude.',
                 15,
                 42
             ) WITH NOWAIT;
...
IF @exceptionMask_tables IS NOT NULL
   AND @exceptionMask_tables LIKE '%.%'
...
BEGIN TRY
    ...
    IF @exceptionMask_days IS NULL
       OR @exceptionMask_days = ''
    BEGIN
    ...
    SET @exceptionMask_tables
        = CHAR(39) + REPLACE(REPLACE(@exceptionMask_tables, ' ', ''), ',', CHAR(39) + ',' + CHAR(39)) + CHAR(39);
    SET @exceptionMask_indexes
        = CHAR(39) + REPLACE(REPLACE(@exceptionMask_indexes, ' ', ''), ',', CHAR(39) + ',' + CHAR(39)) + CHAR(39);

    --Get the exceptions insert command
    IF @sqlmajorver > 9
    BEGIN
        SELECT @sqlcmd
            = N'MERGE dbo.tbl_AdaptiveIndexDefrag_Exceptions AS target
USING (SELECT ' + CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB))
              + N' AS dbID, si.[object_id] AS objectID, si.index_id AS indexID,
''' +       @exceptionMask_DB + N''' AS dbName, OBJECT_NAME(si.[object_id], '
              + CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB)) + N') AS objectName, si.[name] AS indexName,
' +         CONVERT(NVARCHAR, @exceptionMask_days) + N' AS exclusionMask
FROM ' +    QUOTENAME(@exceptionMask_DB) + N'.sys.indexes si
INNER JOIN ' + QUOTENAME(@exceptionMask_DB)
              + N'.sys.objects so ON si.object_id = so.object_id
WHERE so.is_ms_shipped = 0 AND si.index_id > 0 AND si.is_hypothetical = 0
	AND si.[object_id] NOT IN (SELECT sit.[object_id] FROM [' + @exceptionMask_DB
              + N'].sys.internal_tables AS sit)' -- Exclude Heaps, Internal and Hypothetical objects
              +           CASE
                              WHEN @exceptionMask_tables IS NOT NULL THEN
                                  ' AND OBJECT_NAME(si.[object_id], ' + CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB))
                                  + ') IN (' + @exceptionMask_tables + ')'
                              ELSE
                                  ''
                          END + CASE
                                    WHEN @exceptionMask_indexes IS NOT NULL THEN
                                        ' AND si.[name] IN (' + @exceptionMask_indexes + ')'
                                    ELSE
                                        ''
                                END
              + N') AS source
...
WHEN NOT MATCHED THEN
    ...
    BEGIN
        SELECT @sqlcmd
            = N'DELETE FROM dbo.tbl_AdaptiveIndexDefrag_Exceptions
WHERE dbID = ' + CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB))
              + CASE
                    WHEN @exceptionMask_tables IS NOT NULL THEN
                        ' AND [objectName] IN (' + @exceptionMask_tables + ')'
                    ELSE
                        ''
                END + CASE
                          WHEN @exceptionMask_indexes IS NOT NULL THEN
                              ' AND [indexName] IN (' + @exceptionMask_indexes + ');'
                          ELSE
                              ';'
                      END + N'INSERT INTO dbo.tbl_AdaptiveIndexDefrag_Exceptions
SELECT ' +  CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB))
              + N' AS dbID, si.[object_id] AS objectID, si.index_id AS indexID,
''' +       @exceptionMask_DB + N''' AS dbName, OBJECT_NAME(si.[object_id], '
              + CONVERT(NVARCHAR, DB_ID(@exceptionMask_DB)) + N') AS objectName, si.[name] AS indexName,
' +         CONVERT(NVARCHAR, @exceptionMask_days) + N' AS exclusionMask
...
BEGIN CATCH
    SET @debugMessage
        = N'Error ' + CONVERT(NVARCHAR(20), ERROR_NUMBER()) + N': ' + ERROR_MESSAGE() + N' (Line Number: '
