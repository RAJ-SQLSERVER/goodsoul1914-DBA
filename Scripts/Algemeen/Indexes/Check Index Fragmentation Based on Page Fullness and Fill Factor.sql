...
IF @OnlineRebuild = 1
   AND ISNULL(CONVERT(INT, SERVERPROPERTY('EngineEdition')), 0) NOT IN ( 3, 5, 8 )
BEGIN
...
REBUILD WITH(SORT_IN_TEMPDB=' + CASE
                                    WHEN @SortInTempDB = 1 THEN
                                        N'ON'
                                    ELSE
                                        N'OFF'
                                END + N', ONLINE=' + CASE
                                                         WHEN @OnlineRebuild = 1 THEN
                                                             N'ON'
                                                         ELSE
                                                             N'OFF'
                                                     END + N'{FILLFACTOR});
...
SELECT DatabaseName = DB_NAME(),
       ...
       Remediation = REPLACE(
                                REPLACE(
                                           REPLACE(
                                                      REPLACE(@CommandTemplate, N'{DATABASE}', QUOTENAME(DB_NAME())),
                                                      N'{TABLE}',
                                ...
                                CASE
                                    WHEN ix.fill_factor = 0
                                         AND ps.avg_fragmentation_in_percent >= @MinFragmentationToReduceFillFactor100 THEN
                                        N', FILLFACTOR=90'
                                    WHEN ix.fill_factor > 0
                                         AND ps.avg_fragmentation_in_percent <= @MaxFragmentationToSetFillFactor100 THEN
                                        N', FILLFACTOR=100'
                                    ELSE
                                        N''
                                END
