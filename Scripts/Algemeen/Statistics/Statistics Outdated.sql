...
INNER JOIN sys.tables AS ob WITH (NOLOCK) ON ob.object_id = st.object_id
INNER JOIN sys.schemas AS sc WITH (NOLOCK) ON sc.schema_id = ob.schema_id
CROSS APPLY (
	SELECT it.partition_number,
		it.last_updated,
		it.rows,
		it.rows_sampled,
		it.steps,
	...
	FROM sys.dm_db_stats_properties_internal(st.object_id, st.stats_id) AS it
	WHERE st.is_incremental = 0 -- estat�sticas n�o particionadas
		OR st.is_incremental = 1
		AND it.partition_number IS NOT NULL -- linha detalhe da estat�stica particionada
	) AS it
CROSS APPLY (
	SELECT threshold = CONVERT(INT, CASE 
				WHEN ISNULL(it.rows, 0) <= 25000
					THEN ISNULL(it.rows, 0) * 0.2 + 500 -- threshold PADR�O
