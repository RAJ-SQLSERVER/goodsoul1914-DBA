...
JOIN sys.partitions AS par ON par.object_id = s.object_id
...
JOIN sys.stats_columns AS sc ON sc.stats_id = st.stats_id
...
DECLARE curDBs CURSOR LOCAL FORWARD_ONLY
FOR
...
SELECT *,
	CASE 
		WHEN DATEDIFF(dd, COALESCE(StatsUpdatedDate, create_date), GETDATE()) > 7
			THEN 'Old Stats'
		ELSE 'Stats OK'
