...
ELSE IF @SampleRatePercent IS NOT NULL
    SET @options = ISNULL (@options + N', ', N' WITH ') + N'SAMPLE ' + CONVERT (NVARCHAR(MAX), @SampleRatePercent)
                   + N' PERCENT';
...
HAVING SUM(ps.rows) >= ' + CONVERT (NVARCHAR(MAX), @MinimumTableRows)
           + N'
) AS ps
...
AND (sp.modification_counter IS NULL
	OR (sp.modification_counter >= ' + CONVERT (NVARCHAR(MAX), @MinimumModCountr)
           + N'
	  AND sp.last_updated < DATEADD(day, -' + CONVERT (NVARCHAR(MAX), @MinimumDaysOld)
           + N', GETDATE())
	  )
    )
...
DECLARE @Msg               NVARCHAR(4000),
        @StartTime         DATETIME,
        @TimeLimitBreached BIT;

SET @Msg = N'-- ' + CONVERT (NVARCHAR(25), GETDATE (), 121) + N'  Found '
           + CONVERT (NVARCHAR(MAX), (SELECT COUNT (*) FROM #tmpStats)) + N' statistic(s) to update.';
RAISERROR (N'%s', 0, 1, @Msg) WITH NOWAIT;

DECLARE Cmds CURSOR LOCAL FAST_FORWARD FOR
SELECT N'ModCntr: ' + ISNULL (CAST(ModCntr AS NVARCHAR(MAX)), N'(unknown)') + N', TotalRows: '
       + CONVERT (NVARCHAR(MAX), ISNULL (TotalRows, 0)) + N', LastUpdate: '
       + ISNULL (CONVERT (NVARCHAR(25), LastUpdate, 121), N'(never)') AS "Msg",
       N'UPDATE STATISTICS ' + QUOTENAME (DB_NAME (DatabaseId)) COLLATE DATABASE_DEFAULT + N'.'
...
ELSE
    SET @Msg = CHAR (13) + CHAR (10) + N'-- ' + CONVERT (NVARCHAR(25), GETDATE (), 121)
               + N'  Forced stop due to time limit.';
