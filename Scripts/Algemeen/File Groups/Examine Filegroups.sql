...
(
    SELECT fg.data_space_id AS "FGID",
           ...
           ROUND (
               (CAST(SUM (f.size) AS FLOAT) / 128) - (CAST(SUM (FILEPROPERTY (f.name, 'SpaceUsed')) AS FLOAT) / 128), 2
           ) AS "Free_MB"
    FROM sys.filegroups AS fg
    LEFT JOIN sys.database_files AS f
...
FROM sys.filegroups AS fg
INNER JOIN fg_sizes AS fgs
    ON fg.data_space_id = fgs.FGID
LEFT JOIN fg_objs AS fgo
...
LEFT JOIN sys.filegroups AS f2
