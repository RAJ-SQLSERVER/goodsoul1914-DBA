...
ALTER PROCEDURE PerformanceGraph @ShowCPUAsLines      BIT     = 1,
                                 @ShowDeadlock        BIT     = 1,
...
BEGIN
    ...
    DECLARE @ts_now BIGINT = (
                SELECT TOP (1) cpu_ticks / (cpu_ticks / ms_ticks)
    ...
    WITH CPULoad AS
    (
        SELECT TOP (256) y.SQLProcessUtilization AS "SQL Server Process CPU Utilization",
    ...
    BEGIN
        ...
        VALUES (geometry::Parse ('LINESTRING(' + @CPUSQLOut + ')'), 'CPU-SQL'),
    ...
    BEGIN
        ...
        VALUES (geometry::STGeomFromText (
                              'POLYGON((' + CAST(0 - @CPURecords AS CHAR(5)) + ' 0, ' + @CPUSQLOut + ', ' + '0 0, '
                              + CAST(0 - @CPURecords AS CHAR(5)) + ' 0 ))',
                              0
                          ),
                'CPU-SQL'),
               (geometry::STGeomFromText (
                              'POLYGON((' + CAST(0 - @CPURecords AS CHAR(5)) + ' 0, ' + @CPUOtherOut + ', ' + '0 0, '
                              + CAST(0 - @CPURecords AS CHAR(5)) + ' 0 ))',
                              0
                          ),
                'CPU-Other'),
               (geometry::STGeomFromText (
                              'POLYGON((' + CAST(0 - @CPURecords AS CHAR(5)) + ' 0, ' + @CPUIdleOut + ', ' + '0 0, '
                              + CAST(0 - @CPURecords AS CHAR(5)) + ' 0 ))',
                              0
                          ),
                'CPU-Idle');
    ...
    BEGIN;
        ...
        SELECT @DeadlockPoints = STUFF ((
                                            SELECT DISTINCT ', (' + CAST((0 - xml.MinutesAgo) AS CHAR(5)) + ' 0)'
                                            FROM @XMLDeadlocks AS xml
...
-- SQL Prompt formatting on
        ...
        BEGIN -- show points
            INSERT INTO @OutputGeometry (ShapeInfo, Caption)
            VALUES (geometry::Parse ('MULTIPOINT(' + @DeadlockPoints + ')'), -- ShapeInfo - geometry
        ...
        BEGIN -- show blobs
            INSERT INTO @OutputGeometry (ShapeInfo, Caption)
            VALUES (geometry::Parse ('GEOMETRYCOLLECTION(' + @DeadlockBlobs + ')'), -- ShapeInfo - geometry
                    'Deadlocks'                                                     -- Caption - varchar(20)
                );
        END;
        ...
        BEGIN
            SELECT UTCTime,
                   ...
                   MinutesAgo,
                   XML
            FROM @XMLDeadlocks AS xd
            ORDER BY xd.MinutesAgo;
        END;
    END;
    ...
    BEGIN
        ...
        BEGIN
            INSERT INTO @OutputGeometry (ShapeInfo, Caption)
            VALUES (geometry::Parse ('LINESTRING(0 0,0 100)'), -- ShapeInfo - geometry
        ...
        BEGIN
            INSERT INTO @OutputGeometry (ShapeInfo, Caption)
            VALUES (geometry::Parse (
                                  'POLYGON((0 0,0 100, ' + CAST(@EndBarWidth AS CHAR(3)) + ' 100, '
