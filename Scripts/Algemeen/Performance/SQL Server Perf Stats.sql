...
CREATE INDEX ix_blockingSessions_1
ON #blockingSessions (blocking_session_id);

SELECT *
INTO #dm_exec_sessions_raw
FROM sys.dm_exec_sessions;

CREATE INDEX ix_dm_exec_sessions ON #dm_exec_sessions_raw (session_id);

CREATE INDEX ix_dm_exec_sessions_is_user_process
ON #dm_exec_sessions_raw (is_user_process);
...
CREATE INDEX ix_dm_exec_connections_session_id
ON #dm_exec_connections (session_id);

SELECT *
INTO #dm_tran_active_transactions
FROM sys.dm_tran_active_transactions;
CREATE INDEX ix_dm_tran_active_transactions_transaction_id
ON #dm_tran_active_transactions (transaction_id);

SELECT *
INTO #dm_tran_session_transactions
FROM sys.dm_tran_session_transactions;

CREATE INDEX ix_dm_tran_session_transactions_session_id
ON #dm_tran_session_transactions (session_id);

SELECT *
INTO #dm_os_waiting_tasks
FROM sys.dm_os_waiting_tasks;
CREATE INDEX ix_dm_os_waiting_tasks_waiting_task_address
ON #dm_os_waiting_tasks (waiting_task_address);
...
BEGIN
    ...
    ** Intra-query blocking relationships (parallel query waits) aren't "true" blocking problems that we should report on here. */
    IF NOT EXISTS (
        ...
        FROM #tmp_requests2
        WHERE blocking_session_id != 0
              AND wait_type NOT IN ( 'WAITFOR', 'EXCHANGE', 'CXPACKET' )
...
SELECT /*qry12*/ CONVERT (varchar(30), @runtime, 126) AS 'runtime',
                 ...
                 resource_monitor_state, /* online_scheduler_mask,*/ /*processor_group,*/
                 node_state_desc
...
EXEC sp_executesql @sql,
                   N'@runtime datetime, @lastmsticks bigint, @msticks bigint, @mstickstime datetime',
                   @runtime = @runtime,
                   @lastmsticks = @lastmsticks,
                   @msticks = @msticks,
...
SELECT CONVERT (varchar(30), @runtime, 121) AS runtime,
       ...
       mg.requested_memory_kb / 1024 requested_memory_mb,             --Total requested amount of memory in megabytes
...
PRINT '-- Script Source --';
SELECT 'SQL Server Perf Stats Script' AS script_name,
       '$Revision: 16 $ ($Change: ? $)' AS revision;
PRINT '';
...
BEGIN
    ...
    EXEC @sp_perf_stats_ver 'pssdiag',
                            @runtime = @runtime,
    ...
    BEGIN
        EXEC @sp_perf_stats_infrequent_ver @runtime = @runtime,
                                           ...
                                           @lastmsticks = @lastmsticks OUTPUT,
                                           @firstrun = @firstrun,
                                           @IsLite = @IsLite;
