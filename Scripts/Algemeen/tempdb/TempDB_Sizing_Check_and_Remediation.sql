...
INSERT INTO @CMDs
SELECT CASE
           WHEN current_size_MB > InitSizeMBPerFile
                OR @ForceShrink = 1 THEN
               N'USE tempdb; DBCC SHRINKFILE (N' + QUOTENAME (name, '''') + ' , '
               + CONVERT (NVARCHAR, ISNULL (@InitialSizeMBOverride, InitSizeMBPerFile)) + N') WITH NO_INFOMSGS; '
           ELSE N''
       END + 'ALTER DATABASE [tempdb] MODIFY FILE (NAME = ' + QUOTENAME (name, '''') + ', SIZE = '
       + CONVERT (NVARCHAR, ISNULL (@InitialSizeMBOverride, InitSizeMBPerFile)) + N'MB, MAXSIZE = '
       + ISNULL (CONVERT (NVARCHAR, MaxSizeMBPerFile) + 'MB', 'UNLIMITED') + N', FILEGROWTH = '
       + CONVERT (NVARCHAR, @FileGrowthMB) + N'MB);'
       + CONCAT (
             '-- Current size: ',
             current_size_MB,
             ' MB, Space Used: ',
             current_spaceused_MB,
             ' MB (',
             ROUND (current_spaceused_MB * 1.0 / InitSizeMBPerFile * 100, 2),
             ' % of desired size)'
         ) + CASE
                 WHEN current_spaceused_MB * 1.0 / InitSizeMBPerFile > @SpaceUsedMaxPercent / 100.0 THEN ' !!WARNING!!'
                 ELSE ''
             END AS Remediation_Script
--, *
FROM (
    SELECT CEILING (@MaxSizeDiskUtilizationPercent / 100.0 * total_MB_ceil / TotalNumOfFiles) AS MaxSizeMBPerFile,
    ...
    FROM (
        SELECT B.total_bytes,
               ...
               max_size * 8 / 1024 AS current_maxsize_MB,
               growth * 8 / 1024 AS current_growth_MB,
               FILEPROPERTY (A.name, 'SpaceUsed') / 128 AS current_spaceused_MB,
        ...
        WHERE A.database_id = 2
              ...
              AND NOT EXISTS (
            SELECT NULL
            FROM sys.master_files AS A1
            CROSS APPLY sys.dm_os_volume_stats (A1.database_id, A1.file_id) AS B1
            WHERE A1.database_id <> 2
                  AND B1.volume_mount_point = B.volume_mount_point
        )
    ) AS f
) AS f2
WHERE (@IncludeTransactionLog = 1 OR type = 0)
      AND (
          is_percent_growth = 1
...
BEGIN
    ...
    INSERT INTO @TF EXEC ('DBCC TRACESTATUS(1117,1118) WITH NO_INFOMSGS');

    IF @@ROWCOUNT > 0
        INSERT INTO @CMDs
        SELECT N'DBCC TRACEON(' + CONVERT (NVARCHAR, TF) + N', -1);' AS Remediation_Script
        FROM @TF
        WHERE Glob = 0;
...
IF EXISTS (SELECT * FROM @CMDs WHERE CMD LIKE '%DBCC SHRINKFILE%--% !!WARNING!!')
    RAISERROR (
        N'WARNING! One or more files have a high space used compared to the desired file size!
Please try to reduce TempDB workload, and/or increase the desired file size percentage.',
        16,
        1
    );