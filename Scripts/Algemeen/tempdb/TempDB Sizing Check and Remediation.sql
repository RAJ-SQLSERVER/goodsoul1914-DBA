...
DECLARE @CMDs AS TABLE
(
    CMD NVARCHAR(MAX)
);

INSERT INTO @CMDs
SELECT Remediation_Script = CASE
                                WHEN current_size_MB > InitSizeMBPerFile THEN
                                    N'USE tempdb; DBCC SHRINKFILE (N' + QUOTENAME(name, '''') + ' , '
                                    + CONVERT(NVARCHAR, InitSizeMBPerFile) + N'); '
                                ELSE
                                    N''
                            ...
                            + CASE
                                  WHEN current_spaceused_MB * 1.0 / InitSizeMBPerFile > @SpaceUsedMaxPercent / 100.0 THEN
                                      ' !!WARNING!!'
                                  ELSE
                                      ''
                              END
...
FROM
(
    SELECT MaxSizeMBPerFile = CEILING(@MaxSizeDiskUtilizationPercent / 100.0 * total_MB_ceil / TotalNumOfFiles),
...
BEGIN
    ...
    IF @@ROWCOUNT > 0
        INSERT INTO @CMDs
        SELECT Remediation_Script = N'DBCC TRACEON(' + CONVERT(NVARCHAR, TF) + N', -1);'
        FROM @TF
        WHERE Glob = 0;
...
WHERE @ClearServerCache = 1
      AND EXISTS
(
    SELECT TOP (1) NULL FROM @CMDs WHERE CMD NOT LIKE 'DBCC TRACE%'
)
UNION ALL
SELECT CMD AS Remediation_Script
FROM @CMDs;
...
(
    SELECT *
    FROM @CMDs
    WHERE CMD LIKE '%DBCC SHRINKFILE%--% !!WARNING!!'
)
    RAISERROR(
