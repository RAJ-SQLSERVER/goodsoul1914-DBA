...
(
	dbname   sysname, 
	DBstatus varchar(55), 
	R_model  varchar(30)); --select * from sys.master_files 

insert into #alldbstate (dbname, 
						 DBstatus, 
						 R_model) 
select name, 
	   CONVERT(varchar(20), DATABASEPROPERTYEX(name, 'status')), 
	   recovery_model_desc
from sys.databases; --select * from #dbsize 
...
from #dbsize as d
	 join #logsize as l on d.Dbname = l.Dbname
	 join #dbfreesize as fs on d.Dbname = fs.name
order by Dbname;