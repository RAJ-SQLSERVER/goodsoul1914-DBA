    ...
    = N'SELECT DBPrincipals.principal_id AS DBPrincipalId, DBPrincipals.name AS DBPrincipal, DBPrincipals.type, 
       ...
       CASE WHEN DBPrincipals.is_fixed_role = 0 AND DBPrincipals.name NOT IN (''dbo'',''guest'', ''INFORMATION_SCHEMA'', ''public'', ''sys'') THEN  
               ...
               ''DROP '' + CASE DBPrincipals.[type] WHEN ''C'' THEN NULL 
                   ...
                   ELSE ''USER'' END + 
               '' ''+QUOTENAME(DBPrincipals.name' + @Collation
      + N') + '';'' ELSE NULL END AS DropScript, 
       CASE WHEN DBPrincipals.is_fixed_role = 0 AND DBPrincipals.name NOT IN (''dbo'',''guest'', ''INFORMATION_SCHEMA'', ''public'', ''sys'') THEN  
      ...
      + N') END +  
               CASE WHEN DBPrincipals.[type] = ''R'' THEN 
                   ISNULL('' AUTHORIZATION ''+QUOTENAME(Authorizations.name' + @Collation
      + N'),'''')  
                   ...
                   WHEN DBPrincipals.[type] NOT IN (''C'',''K'') THEN 
                       ISNULL('' WITH DEFAULT_SCHEMA =  ''+
                          QUOTENAME(DBPrincipals.default_schema_name' + @Collation
      + N'),'''') 
               ELSE '''' END +
			   CASE WHEN DBPrincipals.[type] = ''S'' 
					THEN '', PASSWORD = ''''<Insert Strong Password Here ' + CAST(NEWID() AS NVARCHAR(36))
      + N'>'''' '' ELSE ''''  END + 
               '';'' 
           AS CreateScript 
...
BEGIN
    ...
    SET @sql = N'INSERT INTO ##DBRoles ' + NCHAR(13) + @sql;
...
SET @ObjectList
    = @ObjectList + N'   SELECT SCHEMA_NAME(sys.types.schema_id) ' + @Collation + N' AS SchemaName, 
       name '     + @Collation
      + N' AS name, 
       user_type_id AS id, 
       ''TYPE'' AS class_desc,
...
SET @sql
   ...
   '' '' + Permission.permission_name' + @Collation
      + N' +  
       ...
       '' FROM '' + QUOTENAME(Grantee.name' + @Collation
      + N')  + ''; '' END AS RevokeScript, 
   CASE WHEN Grantee.is_fixed_role = 0 AND Grantee.name <> ''dbo'' THEN 
   CASE WHEN Permission.[state]  = ''W'' THEN ''GRANT'' ELSE Permission.state_desc' + @Collation
      ...
      + N')  + '' '' +  
       ...
       '' AS ''+ QUOTENAME(Grantor.name' + @Collation
      + N')+'';'' END AS GrantScript 
FROM sys.database_permissions Permission 
...
BEGIN
    ...
    SET @sql = @ObjectList + N'INSERT INTO ##DBPermissions ' + NCHAR(13) + @sql;
