-------------------------------------------------------------------------------
-- Collect SQL error events
--
-- Author: Eitan Blumin (t: @EitanBlumin | b: eitanblumin.com)
-- Date: 2020-05-31
-- Last Update: 2020-07-15
-- Description: Collect T-SQL Error Events using an Extended Events Buffer
--				The script automatically detects whether you're in an Azure SQL DB, 
--				or a regular SQL Server instance.
--
-------------------------------------------------------------------------------

SET NOCOUNT ON;

DECLARE @SourceLinkedServer sysname,
        @MinimumSeverity INT,
...
DECLARE @ProgramsToIgnore AS TABLE
(
    appname sysname
);
...
UNION ALL
SELECT 'Microsoft SQL Server Management Studio - Query'
UNION ALL
SELECT 'Microsoft SQL Server Management Studio'
UNION ALL
SELECT 'SQLServerCEIP'
UNION ALL
SELECT 'check_mssql_health'
UNION ALL
SELECT 'SQL Server Performance Investigator';

/********************* /CONFIGURATION *********************/

DECLARE @CMD NVARCHAR(MAX),
        @Filters NVARCHAR(MAX),
        @ProcFilters NVARCHAR(MAX),
        @Executor NVARCHAR(1000);
DECLARE @IsAzureSQLDB BIT,
        @IsNestedTransaction BIT;
...
IF @IsAzureSQLDB = 1
   ...
   AND
   (
       @FileTargetPath IS NULL
       OR @FileTargetPath NOT LIKE 'https://%'
   )
BEGIN
    RAISERROR(
                 N'When using File Target in Azure SQL DB, you must specify a valid Azure Blog Storage URL in @FileTargetPath.',
                 11,
                 1
             );
...
IF @FileTargetPath IS NOT NULL
   AND @UseFileTarget = 1
BEGIN
    IF RIGHT(@FileTargetPath, 1) IN ( '/', '\' )
        SET @FileTargetPath = @FileTargetPath + @BufferXESessionName + N'.xel';
    IF RIGHT(@FileTargetPath, 4) <> '.xel'
        SET @FileTargetPath = @FileTargetPath + N'.xel';
END;
IF @UseFileTarget = 1
   AND @FlushBuffer = 1
...
SetUpCommand:
IF @IsNestedTransaction = 1
   OR
   (
       @IsAzureSQLDB = 1
       AND @SourceLinkedServer IS NULL
   )
BEGIN
    SET @PrintOnly = 1;
    RAISERROR(
                 N'This database version is not supported for dynamic execution. Please take the generated script in attached results instead and run it directly.',
                 10,
                 0
             );
END;
...
BEGIN
    SET @CMD
        = N'-- Retrieve buffer contents
IF OBJECT_ID(''tempdb..#events'') IS NOT NULL DROP TABLE #events
CREATE TABLE #events (event_xml XML);
INSERT INTO #events
' + CASE
        WHEN @UseFileTarget = 1 THEN
            N'SELECT xdata = CAST(event_data AS xml)
FROM (
select [TargetFileName] = REPLACE(c.column_value, ''.xel'', ''*.xel'')
from sys.dm_xe_' + CASE
                       WHEN @IsAzureSQLDB = 1 THEN
                           N'database_'
                       ELSE
                           N''
                   END + N'sessions AS s
join sys.dm_xe_' + CASE
                       WHEN @IsAzureSQLDB = 1 THEN
                           N'database_'
                       ELSE
                           N''
                   END
            + N'session_object_columns AS c ON s.address = c.event_session_address
where column_name = ''filename'' and s.name = ' + QUOTENAME(@BufferXESessionName, N'''')
            + N'
...
FROM sys.dm_xe_' + CASE
                       ...
                       ELSE
                           N''
                   END + N'session_targets AS xet  
JOIN sys.dm_xe_' + CASE
                       WHEN @IsAzureSQLDB = 1 THEN
                           N'database_'
                       ELSE
                           N''
                   END + N'sessions AS xe ON xe.address = xet.event_session_address
WHERE xe.name = ' + QUOTENAME(@BufferXESessionName, N'''')
            + N'
...
IF EXISTS (SELECT * FROM sys.' + CASE
                                     WHEN @IsAzureSQLDB = 1 THEN
                                         N'database'
                                     ELSE
                                         N'server'
                                 END + N'_event_sessions WHERE name = ''' + @BufferXESessionName
          + N''')
BEGIN
DROP EVENT SESSION ' + QUOTENAME(@BufferXESessionName) + N' ON ' + CASE
                                                                       WHEN @IsAzureSQLDB = 1 THEN
                                                                           N'DATABASE'
                                                                       ELSE
                                                                           N'SERVER'
...
SET @CMD
    = @CMD + N'
IF NOT EXISTS (SELECT * FROM sys.' + CASE
                                         WHEN @IsAzureSQLDB = 1 THEN
                                             N'database'
                                         ELSE
                                             N'server'
...
CREATE EVENT SESSION ' + QUOTENAME(@BufferXESessionName) + N' ON ' + CASE
                                                                         WHEN @IsAzureSQLDB = 1 THEN
                                                                             N'DATABASE'
                                                                         ELSE
                                                                             N'SERVER'
...
'
      + CASE
            WHEN @UseFileTarget = 1 THEN
                CONCAT(
                          N'ADD TARGET package0.event_file(SET filename=N',
                          QUOTENAME(ISNULL(@FileTargetPath, @BufferXESessionName + '.xel'), N''''),
                          N',max_file_size=(',
                          @FileTargetMaxFileSizeMB,
                          N'),max_rollover_files=(',
                          @FileTargetMaxFiles,
                          N'))'
                      )
            ELSE
                CONCAT(
                          N'ADD TARGET package0.ring_buffer(SET max_events_limit=(',
                          @BufferMaxEventsCount,
                          N'),max_memory=(',
                          (@BufferMaxMemoryMB * 1024),
                          N'))'
                      )
        END + N'
WITH (MAX_MEMORY=' + CONVERT(NVARCHAR, @BufferMaxMemoryMB)
      + N' MB,EVENT_RETENTION_MODE=ALLOW_SINGLE_EVENT_LOSS,MAX_DISPATCH_LATENCY=30 SECONDS,MAX_EVENT_SIZE=0 KB,MEMORY_PARTITION_MODE=PER_CPU,TRACK_CAUSALITY=OFF,STARTUP_STATE=ON)
;
END

-- Start the event session
IF NOT EXISTS (SELECT * FROM sys.dm_xe' + CASE
                                              WHEN @IsAzureSQLDB = 1 THEN
                                                  N'_database'
                                              ELSE
                                                  N''
                                          END + N'_sessions WHERE name = ''' + @BufferXESessionName
      + N''')
BEGIN
ALTER EVENT SESSION ' + QUOTENAME(@BufferXESessionName) + N' ON ' + CASE
                                                                        WHEN @IsAzureSQLDB = 1 THEN
                                                                            N'DATABASE'
                                                                        ELSE
                                                                            N'SERVER'
...
ELSE
BEGIN
    IF OBJECT_ID('tempdb..#xe') IS NOT NULL
        DROP TABLE #xe;
    CREATE TABLE #xe
    (
        xdata VARBINARY(MAX)
    ...
    IF @UseFileTarget = 1
        INSERT INTO #events
        SELECT xdata = CONVERT(XML, xdata)
        FROM #xe;
    ELSE
        INSERT INTO #events
        SELECT X.query('.')
        FROM
        (SELECT xdata = CONVERT(XML, xdata) FROM #xe) AS a
            CROSS APPLY xdata.nodes(N'//event') AS session_events(X);

    -- Unfurl raw data
    SELECT [server_name] = @@SERVERNAME,
           [database_name] = session_events.event_xml.value(
                                                               N'(event/action[@name="database_name"]/value)[1]',
                                                               N'SYSNAME'
                                                           ),
           event_name = session_events.event_xml.value(N'(event/@name)[1]', N'NVARCHAR(1000)'),
           event_timestamp_utc = session_events.event_xml.value(N'(event/@timestamp)[1]', N'DATETIME2(7)'),
           ...
           category_desc = session_events.event_xml.value(N'(event/data[@name="category"]/text)[1]', N'NVARCHAR(MAX)'),
           message = session_events.event_xml.value(N'(event/data[@name="message"]/value)[1]', N'NVARCHAR(MAX)'),
           client_app_name = session_events.event_xml.value(
                                                               N'(event/action[@name="client_app_name"]/value)[1]',
                                                               N'NVARCHAR(1000)'
                                                           ),
           client_host_name = session_events.event_xml.value(
                                                                N'(event/action[@name="client_hostname"]/value)[1]',
                                                                N'NVARCHAR(1000)'
                                                            ),
           client_process_id = session_events.event_xml.value(N'(event/action[@name="client_pid"]/value)[1]', N'BIGINT'),
           username = session_events.event_xml.value(N'(event/action[@name="username"]/value)[1]', N'SYSNAME'),
