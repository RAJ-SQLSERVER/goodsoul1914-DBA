-------------------------------------------------------------------------------
-- Creates an extended events session that monitors for long running 
-- SQL commands.
--
...
DECLARE @ProceduresToIgnore AS TABLE
(
    procname sysname
);
...
AND (' + CASE
             WHEN @CaptureAllTimeoutsOrAborts = 1 THEN
                 N'result = 2 OR '
             ELSE
                 N''
         END + N'duration >= ' + CONVERT(NVARCHAR, @MinimumDurationMilliSeconds * 1000) + N')';

SELECT @Filters
    = @Filters + CHAR(10) + N'AND (sqlserver.client_app_name <> ''' + REPLACE(appname, N'''', N'''''') + N''')'
FROM @ProgramsToIgnore;

SELECT @ProcFilters
    = ISNULL(@ProcFilters, N'') + CHAR(10) + N'AND (object_name <> ''' + REPLACE(procname, N'''', N'''''') + N''')'
...
WHERE xe.name = ''' + @BufferXESessionName
          + N'''
...
WHERE '    + @Filters + ISNULL(@ProcFilters, N'')
      + N'
)
...
ADD TARGET package0.ring_buffer(SET max_events_limit=(' + CONVERT(NVARCHAR, @BufferMaxEventsCount) + N'),max_memory=('
      + CONVERT(NVARCHAR, @BufferMaxMemoryMB * 1024) + N'))
...
BEGIN
    ...
    RAISERROR(
                 N'Azure SQL Databases are not supported for dynamic execution. Please take the generated script in attached results instead and run it directly.',
                 11,
                 0
             );
END;
...
BEGIN
    ...
    SELECT [server_name] = @@SERVERNAME,
           ...
           client_process_id = session_events.event_xml.value(N'(event/action[@name="client_pid"]/value)[1]', N'BIGINT'),
           username = session_events.event_xml.value(N'(event/action[@name="username"]/value)[1]', N'SYSNAME'),
           plan_handle = session_events.event_xml.value(
                                                           N'(event/action[@name="plan_handle"]/value)[1]',
                                                           N'VARBINARY(MAX)'
                                                       ),
           query_plan_hash = session_events.event_xml.value(
                                                               N'(event/action[@name="query_plan_hash"]/value)[1]',
                                                               N'VARBINARY(MAX)'
                                                           ),
           sql_text = CASE
                          WHEN @PreferObjectName = 1 THEN
                              COALESCE(
                                          session_events.event_xml.value(
                                                                            N'(event/data[@name="object_name"]/value)[1]',
                                                                            N'NVARCHAR(MAX)'
                                                                        ),
                                          session_events.event_xml.value(
                                                                            N'(event/action[@name="sql_text"]/value)[1]',
                                                                            N'NVARCHAR(MAX)'
                                                                        ),
                                          session_events.event_xml.value(
