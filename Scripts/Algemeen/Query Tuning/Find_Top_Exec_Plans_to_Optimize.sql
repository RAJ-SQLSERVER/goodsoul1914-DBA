...
FROM
(
    SELECT SQLBatchText = t.text,
           SQLStmtText = SUBSTRING(   t.text,
                                      CASE
                                          WHEN NULLIF(qs.statement_start_offset, -1) IS NULL THEN
                                              0
                                          ELSE
                                              qs.statement_start_offset / 2 + 1
                                      END,
                                      CASE
                                          WHEN NULLIF(qs.statement_end_offset, -1) IS NULL THEN
                                              LEN(t.text)
                                          ELSE
                                      (statement_end_offset - qs.statement_start_offset) / 2 + 1
                                      END
                                  ),
           QueryPlan = QP.query_plan,
           HasParallelism = QP.query_plan.query('.').exist('data(//RelOp[@PhysicalOp="Parallelism"][1])'),
           HasMissingIndexes = CASE
                                   WHEN QP.query_plan.query('.').exist('data(//MissingIndexes[1])') = 1 THEN
                                       1
                                   ELSE
                                       0
                               END,
    ...
    CROSS APPLY
(
    SELECT HighestStatementCost = MAX(S.node_xml.value('(@StatementSubTreeCost)[1]', 'float')),
