...
SET @MajorVersion = LEFT(CAST(SERVERPROPERTY('ProductVersion') AS NVARCHAR(MAX)), CHARINDEX(
                                                                                      '.',
                                                                                      CAST(SERVERPROPERTY(
                                                                                           'ProductVersion'
                                                                                           ) AS NVARCHAR(MAX))
...
CREATE TABLE #VLFInfo (ServerName VARCHAR(50), DatabaseName VARCHAR(50), VLFCount INT);
    ...
    BEGIN
        IF @ProductBuild >= 11 -- Applies TO: SQL Server 2014 (12.x) through SQL Server 2017, SQL DATABASE.
            BEGIN
                ...
                SET @SQLString2 = N'SELECT ''' + @@SERVERNAME + N''' AS Servername' + N', ''' + @DB_NAME
                                  + N''' AS database_name' + N', COUNT(database_id) AS vlf_count'
                                  + N' FROM sys.dm_db_log_info(' + CONVERT(VARCHAR, @DB_ID) + N')';
                PRINT @SQLString2;
        ...
        ELSE
            BEGIN
                SET @SQLString2 = N'DBCC LOGINFO (' + @DB_NAME + N')';
