...
INNER JOIN sys.partitions AS PRT ON AU.container_id = PRT.partition_id
	AND AU.type = 2
	OR AU.container_id = PRT.hobt_id
	AND AU.type IN (1, 3)
INNER JOIN sys.objects AS OBJ ON PRT.object_id = OBJ.object_id
INNER JOIN sys.schemas AS SCH ON OBJ.schema_id = SCH.schema_id
INNER JOIN sys.indexes AS IDX ON PRT.object_id = IDX.object_id
