...
(
    ...
    SELECT sys.dm_exec_requests.blocking_session_id AS "lead_session_id",
           sys.dm_exec_requests.blocking_session_id AS "blocking_session_id",
           0 AS "cnt"
    ...
    SELECT Blocked_Sessions.lead_session_id,
...
(
    ...
    SELECT lead_session_id,
           SUM (cnt) AS "sessions_blocked"
    FROM Blocked_Sessions
    GROUP BY lead_session_id
)
SELECT Blocked.*,
       DATEDIFF (s, Sess.last_request_start_time, GETDATE ()) AS "seconds_blocking",
       ISNULL (Req.status, 'sleeping') AS "status",
       SqlText.text AS "sql_text",
       STUFF ((
                  SELECT DISTINCT ISNULL (', ' + db.name, '')
                  FROM sys.databases AS db
                  JOIN sys.dm_tran_locks AS lcks
...
LEFT OUTER JOIN sys.dm_exec_sessions AS Sess
...
WHERE Blocked.sessions_blocked >= 1
      AND DATEDIFF (s, Sess.last_request_start_time, GETDATE ()) > 30; -- blocked for longer than 30 seconds