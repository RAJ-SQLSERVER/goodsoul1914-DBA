...
INNER JOIN sys.dm_os_waiting_tasks AS DOWT ON DTL.lock_owner_address = DOWT.resource_address
INNER JOIN sys.dm_exec_requests AS DER ON DOWT.session_id = DER.session_id
INNER JOIN sys.dm_exec_sessions AS DES_blocked ON DOWT.session_id = DES_Blocked.session_id
INNER JOIN sys.dm_exec_sessions AS DES_blocking ON DOWT.blocking_session_id = DES_Blocking.session_id
INNER JOIN sys.dm_exec_connections AS DEC ON DTL.request_session_id = DEC.most_recent_session_id
