...
ORDER BY lock_owner_address
    ...
    SELECT name,
           ...
           'SELECT ' + CAST(database_id AS VARCHAR(MAX))
           + ', o.[object_id], ISNULL(ix.index_id, 0), p.hobt_id,
         ...
         INNER JOIN [' + name
           + '].sys.partitions AS p WITH (NOLOCK) ON o.[object_id]=p.[object_id]
        ...
        '  AS "sql",
           ...
           'USE [' + name + '];
         SELECT ' + CAST(database_id AS VARCHAR(MAX))
           + ', f.[file_id], f.type_desc AS filetype,
         ...
         CROSS APPLY sys.dm_os_volume_stats(' + CAST(database_id AS VARCHAR(MAX)) + ', f.[file_id]) AS v'
           ...
           (CASE
                WHEN @version >= '12.0' THEN
                    '
                 UNION ALL
         SELECT ' + CAST(database_id AS VARCHAR(MAX))
                    + ', -1 AS [file_id], ''IN-MEMORY'' AS filetype,
         ...
         AS (	SELECT ps.data_space_id, pf.function_id, pf.name AS pf_name, val.boundary_id, pf.boundary_value_on_right,
            ...
            INNER JOIN [' + name
           + '].sys.partition_functions AS pf WITH (NOLOCK) ON val.function_id = pf.function_id
            INNER JOIN [' + name
           + '].sys.partition_schemes AS ps WITH (NOLOCK) ON pf.function_Id=ps.function_id),
        ...
        SELECT ' + CAST(database_id AS VARCHAR(MAX))
           + ', ix.[object_id], ix.index_id, p.hobt_id, ISNULL(rng.boundary_id, 0),
            ISNULL(rng.lower_rng, '''')+c.name+ISNULL(rng.upper_rng, '''') AS boundary, p.[rows]
        FROM rng
        INNER JOIN [' + name
           + '].sys.indexes AS ix WITH (NOLOCK) ON ix.data_space_id=rng.data_space_id
        INNER JOIN [' + name
           + '].sys.index_columns AS ic WITH (NOLOCK) ON ix.[object_id]=ic.[object_id] AND ix.index_id=ic.index_id AND ic.partition_ordinal>0
        INNER JOIN [' + name
    ...
    BEGIN;
        ...
        BEGIN TRY;
            INSERT INTO @database_procedures
            EXEC (@sql4);
        END TRY
        BEGIN CATCH;
