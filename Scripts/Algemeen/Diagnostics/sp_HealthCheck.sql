...
**********************************************************************************************************************************************************************************/
	...
	begin try
		...
		begin
			...
			select @p_Status_stmt = case @p_Status
										when 'ACTIVE' then 'and upper(coalesce(r.status, s.status)) not in (''SLEEPING'', ''BACKGROUND'')'
...
SELECT	s.session_id, 
		...
		[granted_query_memory] = CASE WHEN ((CAST(r.granted_query_memory AS numeric(20,2))*8)/1024/1024) >= 1.0
									  ...
									  END,
		r.query_hash, 
		r.query_plan_hash,' + case
								  when @p_getExecutionPlan = 1 then '
...
OUTER APPLY sys.dm_exec_sql_text(r.sql_handle) AS st' + case
															when @p_getExecutionPlan = 1 then '
...
' + @p_Status_stmt + ' order by ' + case @p_Orderby
										when 1 then 'request_logical_reads desc'
										when 2 then 'CPUTime desc'
										when 3 then 'SPID'
										when 4 then 'Order by DBName'
									else 'request_logical_reads desc'
									end;
		...
		if @p_Option = '0'
		   or @p_Option = '2'
		begin
			if OBJECT_ID(N'master.sys.dm_os_sys_memory') is null
			   or OBJECT_ID(N'master.sys.dm_os_ring_buffers') is null
			begin
				...
				from (select rx.record.value('(./Record/@id)[1]', 'int') as record_id, 
					  ...
					  from (select CONVERT(xml, record) as record
							...
							where ring_buffer_type = N'RING_BUFFER_SCHEDULER_MONITOR'
								  and record like '%<SystemHealth>%') as rx) as r
		...
		if @p_Option = '0'
		   or @p_Option = '3'
		begin
			if OBJECT_ID(N'master.sys.dm_os_volume_stats') is null
			   or OBJECT_ID(N'sys.dm_db_file_space_usage') is null
			begin
			...
			begin
				...
				select distinct 
					   ...
					   [Used (%)] = CONVERT(decimal(20, 2), CONVERT(decimal(20, 2), ( vs.total_bytes - vs.available_bytes ) / 1024. / 1024. / 1024.) / CONVERT(decimal(20, 2), vs.total_bytes / 1024. / 1024. / 1024.) * 100.0), 
					   [Free (%)] = CONVERT(decimal(20, 2), CONVERT(decimal(20, 2), vs.available_bytes / 1024. / 1024. / 1024.) / CONVERT(decimal(20, 2), vs.total_bytes / 1024. / 1024. / 1024.) * 100.0)
				from sys.master_files as mf
					 cross apply master.sys.dm_os_volume_stats(mf.database_id, mf.file_id) as vs
				...
				from (select totalFileSizeGB = CAST(SUM(unallocated_extent_page_count + version_store_reserved_page_count + user_object_reserved_page_count + internal_object_reserved_page_count + mixed_extent_page_count) * 8. / 1024. / 1024. as decimal(20, 2)), 
							 freeSpaceGB = CAST(SUM(unallocated_extent_page_count) * 8. / 1024. / 1024. as decimal(20, 2))
					  from tempdb.sys.dm_db_file_space_usage) as a
				order by Volume;
			end;
			...
			(
				...
				FileSize as case
								when FileSizeKB / 1024 / 1024 / 1024 >= 1.0 then CAST(CAST(FileSizeKB / 1024 / 1024 / 1024 as decimal(20, 2)) as varchar(23)) + ' TB'
							...
							else CAST(CAST(FileSizeKB as decimal(20, 2)) as varchar(23)) + ' KB'
							end, 
				SpaceUsed as case
								 when SpaceUsedKB / 1024 / 1024 / 1024 >= 1.0 then CAST(CAST(SpaceUsedKB / 1024 / 1024 / 1024 as decimal(20, 2)) as varchar(23)) + ' TB'
							 ...
							 else CAST(CAST(SpaceUsedKB as decimal(20, 2)) as varchar(23)) + ' KB'
							 end, 
				FreeSpace as case
								 when FreeSpaceKB / 1024 / 1024 / 1024 >= 1.0 then CAST(CAST(FreeSpaceKB / 1024 / 1024 / 1024 as decimal(20, 2)) as varchar(23)) + ' TB'
							 ...
							 else CAST(CAST(FreeSpaceKB as decimal(20, 2)) as varchar(23)) + ' KB'
							 end, 
...
****************************************************************************************************/
		...
		begin
			if not exists (select 1
						   from master.sys.dm_exec_requests as r
			...
			begin
				select LongRunningSPID = r.session_id, 
					   ...
					   currentQuery = SUBSTRING(t.text, r.statement_start_offset / 2 + 1, ( case
					   ...
					   r.command
				from master.sys.dm_exec_requests as r
					 outer apply master.sys.dm_exec_sql_text(r.sql_handle) as t
				where r.percent_complete <> 0
				union
				select r.session_id, 
					   dbname = DB_NAME(r.database_id), 
		...
		begin
			...
			begin
				...
				begin
					select AlwaysOn_dsn_name = agl.dns_name, 
					...
					from sys.availability_groups as ag(nolock)
						 join sys.dm_hadr_availability_group_states as ags(nolock) on ags.group_id = ag.group_id
						 join sys.availability_replicas as ar(nolock) on ar.group_id = ag.group_id
						 join sys.dm_hadr_availability_replica_states as ars(nolock) on ars.replica_id = ar.replica_id
																						and ars.group_id = ag.group_id
						 join sys.dm_hadr_database_replica_cluster_states as dr(nolock) on dr.replica_id = ar.replica_id
						 join sys.availability_group_listeners as agl(nolock) on agl.group_id = ag.group_id
					order by role_desc;
				end;
			end;
		end;
		...
		begin
			...
			begin
				if exists (select port
						   from master.sys.tcp_endpoints
						   where type_desc = 'DATABASE_MIRRORING')
				   and exists (select mirroring_role_desc
				...
				begin
					...
					from sys.database_mirroring as m
						 join sys.databases as d on m.database_id = d.database_id
					where m.mirroring_role_desc is not null
					order by m.mirroring_role_desc desc, 
							 d.name;
				end;
			end;
		end;
	...
	begin catch
		...
		set @errorMessage = 'Error Number: ' + CONVERT(varchar, @errorNumber) + '; ' + 'Line: ' + CONVERT(varchar, @errorLine) + '; ' + 'Message: "' + @errorMessage + '"';
