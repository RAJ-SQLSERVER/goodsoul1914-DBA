...
IF OBJECT_ID('dbo.usp_DataProfile') IS NOT NULL 
  DROP PROCEDURE dbo.usp_DataProfile;
GO

CREATE PROCEDURE dbo.usp_DataProfile
   ...
   @Verbose BIT = 0
/*
usp_DataProfile v0.3 - Apr 20, 2015
...
4 = Column Value Distribution - You need to provide a single column name in @ColumnList. 
                                If more than one is provided only the first one is used.

You can use @ShowIndexes = 1 and @ShowForeignKeys = 1 in any mode to see all of the indexes and foreign keys.

Example usage:
Table Overview
usp_DataProfile 'Users', 0;

Table Overview with Indexes and FKs
usp_DataProfile 'Users', 0, @ShowIndexes=1, @ShowForeignKeys=1;

Column Detail
usp_DataProfile 'Users', 1

Column Statistics - Only using 10% of the table values
usp_DataProfile 'Users', 2, @SampleValue = 10

Candidate Key Check
usp_DataProfile 'Users', 3, 'DisplayName, Location, WebsiteUrl, CreationDate'

Column Value Distribution
usp_DataProfile 'Posts', 4, 'PostTypeId'
