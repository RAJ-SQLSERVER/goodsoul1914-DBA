...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id = @jobId,
                                           @step_name = N'Write output to table',
                                           @step_id = 1,
                                           ...
                                           @os_run_priority = 0,
                                           @subsystem = N'TSQL',
                                           @command = N'IF NOT EXISTS ( SELECT 1 FROM DBATools.sys.tables WHERE NAME = ''sp_whoisactive_log'')
BEGIN
CREATE TABLE DBATools.dbo.sp_whoisactive_log
(
...
CREATE INDEX idx_collection_time ON DBATools.dbo.sp_whoisactive_log (collection_time)
END
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id = @jobId,
                                           ...
                                           @os_run_priority = 0,
                                           @subsystem = N'TSQL',
                                           @command = N'DELETE DBATools.dbo.sp_whoisactive_log
