/*
Check that the configured value for MAXDOP is in the recommended range, 
...
IF @Major < 10
    RAISERROR(
                 'This script is intended for SQL Server 2008 or higher. It will not work on version [%s].',
                 11,
                 1,
                 @ProductVersion
             );
...
FROM
       (
           ...
           WHERE    memory_node_id <> 64
                    AND node_id <> 64 --Excluded DAC node
           GROUP BY memory_node_id
       ) AS m;

IF @NumaNodeCount < 1
   OR @LogicalProcessorPerNumaNodeCount < 1
    RAISERROR(
                 'Could not capture NUMA node or logical processor count. Reported NUMA: [%d], Logical Processor: [%d]',
                 11,
                 1,
                 @NumaNodeCount,
                 @LogicalProcessorPerNumaNodeCount
             );

SET @LogicalProcessorThreshold = CASE
                                     WHEN @NumaNodeCount = 1 THEN
                                         8
                                     ELSE
                                         16
                                 END;
...
BEGIN
    SET @RecommendedMaxDOP = CASE
                                 WHEN @LogicalProcessorPerNumaNodeCount <= @LogicalProcessorThreshold THEN
                                     @LogicalProcessorPerNumaNodeCount
                                 WHEN @LogicalProcessorPerNumaNodeCount > @LogicalProcessorThreshold
                                      AND (@LogicalProcessorPerNumaNodeCount / 2) <= @LogicalProcessorThreshold THEN
    (@LogicalProcessorPerNumaNodeCount / 2)
                                 WHEN @LogicalProcessorPerNumaNodeCount > @LogicalProcessorThreshold THEN
                                     @LogicalProcessorThreshold
                             END;
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            SET @ResultMessage
                = N'MaxDOP should be set at half the number of logical processors per NUMA node with a MAX value of 16.';
...
IF @ResultMessage IS NOT NULL
   AND @EffectiveMaxDOP <> @RecommendedMaxDOP
BEGIN
    PRINT @ResultMessage + ' Changing MaxDOP ' + CONVERT(VARCHAR(10), @EffectiveMaxDOP) + ' to '
          + CONVERT(VARCHAR(10), @RecommendedMaxDOP);

    IF @WhatIf = 0
    BEGIN
        DECLARE @AdvancedOptionsWasOn BIT;

        SELECT @AdvancedOptionsWasOn = CAST(value AS BIT)
        FROM   sys.configurations
