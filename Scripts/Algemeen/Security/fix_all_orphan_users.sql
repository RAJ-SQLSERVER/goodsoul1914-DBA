...
*/
DECLARE @Database sysname = NULL; -- Filter by a specific database. Leave NULL for all databases.
...
BEGIN
    ...
    BEGIN
        ...
        IF @user = 'dbo'
            SET @Command
                = N'USE ' + QUOTENAME(@Database) + N'; ALTER AUTHORIZATION ON DATABASE::' + QUOTENAME(@Database)
                  + N' TO ' + QUOTENAME(@saName) + N' -- assign orphaned [dbo] to [sa]';
        ELSE IF @loginExists = 0
            SET @Command
                = N'USE ' + QUOTENAME(@Database) + N'; ' + ISNULL(@ownedSchemas, N'') + N' DROP USER '
                  + QUOTENAME(@user) + N' -- no existing login found';
        ELSE
            SET @Command
                = N'USE ' + QUOTENAME(@Database) + N'; ALTER USER ' + QUOTENAME(@user) + N' WITH LOGIN = '
                  + QUOTENAME(@user) + N' -- existing login found';
