...
SELECT [Security Report] = '-----SQL SERVER SECURITY AUDIT Report-----',
	'-----',
	'-----',
	'-----';
GO
...
FROM (
	...
	(
		...
		WHERE name NOT LIKE '%#%'
			AND isntuser = 1
		) b, --for login is windows user 
	(
		SELECT COUNT(name) c
		FROM sys.syslogins
		WHERE name NOT LIKE '%#%'
			AND isntname = 0
		) c, -- for login is sql server login 
	(
		SELECT COUNT(name) d
		FROM sys.syslogins
		WHERE name NOT LIKE '%#%'
...
SELECT [Database_Owner_details] = '--- DATABASE OWENER DETAILS----',
	' ----- ',
	' ----- ',
	' ----- ';
GO
