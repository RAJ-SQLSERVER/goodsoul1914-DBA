...
BEGIN
	SET @ProductVersion = N'SQL Server 2018';
...
BEGIN
	SET @NodeName1 = (
			SELECT TOP 1 NodeName
			FROM #nodes
			);
	SET @NodeName2 = (
			SELECT NodeName
			FROM #nodes
			WHERE NodeName <> @NodeName1
			);
...
SELECT @AuditLvltxt = CASE 
		...
		WHEN @AuditLevel = 3
			THEN 'Both successful and failed logins'
		ELSE 'Unknown'
		END;
...
SET @FileStreams = CONVERT(NVARCHAR(10), (
			SELECT value
			FROM sys.configurations
			WHERE name LIKE 'filestream access%'
			));
...
SET @BackUpCompression = CONVERT(NVARCHAR(10), (
			SELECT value
			FROM sys.configurations
			WHERE name LIKE 'backup compression%'
			));
...
BEGIN
	...
	DECLARE TraceFlagSet CURSOR LOCAL FAST_FORWARD
	FOR
	(
			SELECT TraceFlag,
				[Status],
...
BEGIN
	...
	DECLARE SysAccounts CURSOR LOCAL FAST_FORWARD
	FOR
	(
			SELECT SysAdmin
...
BEGIN
	...
	DECLARE SrvAccounts CURSOR LOCAL FAST_FORWARD
	FOR
	(
			SELECT ServerAdmins
			FROM #SrvAdmin
...
CREATE TABLE #TrustedDB (
	name VARCHAR(50),
	DBTrusted VARCHAR(50)
	);

INSERT INTO #TrustedDB
EXEC (
		'SELECT sys.server_principals.name as Owner, sys.databases.name FROM sys.databases 
...
BEGIN
	...
	SELECT '	',
		DBTrusted AS 'TRUSTWORTHY Database Name',
		name AS 'Database Owner'
	FROM #TrustedDB;
...
DECLARE @Valuedescript NVARCHAR(100),
	@ValueName2 NVARCHAR(100),
	@ValueInUse NVARCHAR(100);

DECLARE DangerousSettings CURSOR
FOR
SELECT [description],
	[name],
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO #SASATFailed
			SELECT 'I/O and Processor Affinity';
		END;
		...
		BEGIN
			...
			INSERT INTO #SASATFailed
			SELECT 'Ad Hoc Distributed queries';
		END;
		...
		BEGIN
			...
			INSERT INTO #SASATFailed
			SELECT 'Cross DB Ownership Chaining';
		END;
...
DECLARE AuditRpt CURSOR LOCAL FAST_FORWARD
FOR
(
		SELECT AuditName
		FROM #SASATFailed
