...
IF EXISTS (SELECT * FROM sys.objects WHERE name = N'IsReallyInFullRecovery')
    DROP FUNCTION IsReallyInFullRecovery;
GO

CREATE FUNCTION IsReallyInFullRecovery (@DBName sysname)
RETURNS BIT
AS
BEGIN
...
BEGIN
    ALTER DATABASE SimpleModeDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE SimpleModeDB;
END;
GO
IF DATABASEPROPERTYEX (N'BulkLoggedModeDB', N'Version') > 0
BEGIN
    ALTER DATABASE BulkLoggedModeDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE BulkLoggedModeDB;
END;
GO
IF DATABASEPROPERTYEX (N'FullModeDB', N'Version') > 0
BEGIN
    ALTER DATABASE FullModeDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
...
SELECT name,
       msdb.dbo.IsReallyInFullRecovery (name) AS "ReallyInFULL"
FROM sys.databases
...
TO  DISK = N'D:\SQLBackups\FullModeDB.bck'
WITH INIT;
GO

SELECT msdb.dbo.IsReallyInFullRecovery (N'FullModeDB') AS "ReallyInFULL";
GO
...
BACKUP DATABASE FullModeDB
TO  DISK = N'D:\SQLBackups\FullModeDB_Diff.bck'
WITH INIT,
     DIFFERENTIAL;
GO
