...
BEGIN
    ALTER DATABASE SlowLogFile SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE SlowLogFile;
END;
GO

CREATE DATABASE SlowLogFile
ON PRIMARY (
       NAME = N'SlowLogFile_data',
       FILENAME = N'D:\SDLData\SlowLogFile_data.mdf'
   )
LOG ON (
...
CREATE TABLE BadKeyTable (c1 INT, c2 DATETIME, c3 CHAR(100));
