...
IF EXISTS (SELECT * FROM tempdb.sys.objects WHERE name = N'##TempSpinlockStats1')
    DROP TABLE ##TempSpinlockStats1;

IF EXISTS (SELECT * FROM tempdb.sys.objects WHERE name = N'##TempSpinlockStats2')
...
LEFT OUTER JOIN ##TempSpinlockStats1 AS ts1
