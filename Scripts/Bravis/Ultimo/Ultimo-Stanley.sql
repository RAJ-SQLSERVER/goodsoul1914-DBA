DECLARE @PersNr VARCHAR(10) = '247410'

-- Bepaal ObjconEmpId op basis van personeelsnummer ==> 014149
SELECT        
   c.ObjconEmpId
FROM            
...
WHERE
	(a.EmpRecStatus >= 0) AND (a.EmpContext = 1) AND (d._ProfDoNotExport = 0) AND (c.ObjconRecStatus >= 0) AND LEFT(a.EmpExternalId, 12) = @PersNr

-- Haal alle records op uit de ObjectContact tabel
SELECT *
FROM Ultimo.dba.ObjectContact
WHERE (ObjconEmpId = '044085') AND (_ObjconValidFrom <= DATEADD(d, 30, GETDATE()) OR _ObjconValidFrom IS NULL) AND (_ObjconValidTill >= GETDATE() OR _ObjconValidTill IS NULL) --AND ObjconCreateEmpId = 'ULTIMO' AND _ObjconAssDepFunPrId IS NOT NULL
ORDER BY _ObjconLabourPart DESC, ObjconCreateDate DESC

-- Haal de dienstverbandgegevens op van de persoon
SELECT        
...
FROM            
   ...
   Ultimo.dba.Profession AS d ON c._ObjconFunctionId = d.ProfId
WHERE
	(a.EmpRecStatus >= 0) AND (a.EmpContext = 1) AND (d._ProfDoNotExport = 0) AND (c.ObjconRecStatus >= 0) AND LEFT(a.EmpExternalId, 12) = @PersNr AND c.ObjconId = '0492150'