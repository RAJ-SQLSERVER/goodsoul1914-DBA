/*
  ...
  v.6.1_01 Maart 2018, ADRZ - Daniël Pairoux, LUMC - Jos van Hoek
  v.6.2_01 Juli 2019, LLZ - Michiel van den Boogaard
  v.6.2.02 Juli 2019, WorkstationName indeling lokaal en SaCZ voor HiX 6.2
  v.6.2.03 Augustus 2019, ADRZ - Daniël Pairoux - Duration toegevoegd, IsSubAction en IsFirstTime gebruikt 
...
CREATE OR ALTER PROCEDURE dbo.SACZ_BRAVIS
AS
BEGIN
    ...
    FROM (
        ...
        FROM dbo.PERFLOG_ACTION AS L1 WITH (NOLOCK) -- Let op: PERFLOG_ACTION is een VIEW
