...
-- (c) 2015 Andy Yun
===============================================================================*/
CREATE PROCEDURE dbo.sp_helpExpandView (@ViewName        NVARCHAR(512),
...
AS
    BEGIN
        BEGIN TRY
            ...
            IF CHARINDEX ('.', @ViewName, 1) = 0 SET @ViewName = 'dbo.' + @ViewName;
            ...
            INSERT INTO #tmpObjectHierarchy (BaseObject_DBName,
                                             BaseObject_FullName,
                                             Child_FullName,
                                             Child_SchemaName,
                                             Child_ObjectName,
            ...
            WHILE @MinObjectHierarchyID < @MaxObjectHierarchyID
                BEGIN
                    IF @Debug = 1
                        SELECT @MinObjectHierarchyID AS "@MinObjectHierarchyID",
                               @MaxObjectHierarchyID AS "@MaxObjectHierarchyID";

                    -----
                    -- For each DB found in #tmpObjectHierarchy, loop through each recordset of each Database
                    DECLARE cursorEachDatabase CURSOR FAST_FORWARD FOR
                    SELECT DISTINCT Child_DBName
                    FROM #tmpObjectHierarchy;

                    OPEN cursorEachDatabase;

                    FETCH NEXT FROM cursorEachDatabase
                    INTO @DBName;
			...
			WHERE #tmpObjectHierarchy.Child_DBName = ''@DatabaseName''
				...
				AND synonyms.object_id = #tmpObjectHierarchy.Child_ObjectID;';
                    ...
                    SELECT @TmpObjectHierarchyID = @MinObjectHierarchyID,
                    ...
                    INSERT INTO #tmpObjectHierarchy (BaseObject_DBName,
                                                     ...
                                                     Child_DBName,
                                                     ParentObjectHierarchyID)
                    SELECT DISTINCT #tmpObjectHierarchy.BaseObject_DBName AS "BaseObject_DBName",
            ...
            IF EXISTS (SELECT 1 FROM #tmpObjectHierarchy WHERE Child_Type IS NULL)
                BEGIN
                    ...
                    -----
                    -- For each DB found in #tmpObjectHierarchy, loop through each recordset of each Database
                    DECLARE cursorEachDatabase_2 CURSOR FAST_FORWARD FOR
                    SELECT DISTINCT Child_DBName
                    FROM #tmpObjectHierarchy
                    ...
                    FETCH NEXT FROM cursorEachDatabase_2
                    INTO @DBName;
            ...
            -----
            -- Unflattened Output
            IF (@OutputFormat = 'Vertical' OR @OutputFormat = 'All')
                BEGIN
                    SELECT DISTINCT HierarchyLvl,
                                    BaseObject_FullName,
                                    Child_DBName,
                                    Child_FullName,
                                    Child_Type,
            ...
            IF (@OutputFormat = 'Horizontal' OR @OutputFormat = 'All')
                BEGIN
		...
		FROM #tmpObjectHierarchy Lvl1';
			...
			, COALESCE(Lvl'                  + CAST(@HierarchyID AS NVARCHAR(4000)) + N'.HierarchyLvl, '''
                                             + CAST(@HierarchyID AS NVARCHAR(4000)) + N''') AS Lvl_'
                                             + CAST(@HierarchyID AS NVARCHAR(4000)) + N'
			, COALESCE(Lvl'                  + CAST(@HierarchyID AS NVARCHAR(4000))
                                             + N'.Child_FullName, SPACE(0)) AS Obj_'
                                             + CAST(@HierarchyID AS NVARCHAR(4000)) + N'
			, COALESCE(Lvl'                  + CAST(@HierarchyID AS NVARCHAR(4000)) + N'.Child_Type, SPACE(0)) AS Typ_'
                                             + CAST(@HierarchyID AS NVARCHAR(4000));
                            SET @sqlFrom = @sqlFrom + N'
		LEFT OUTER JOIN #tmpObjectHierarchy Lvl' + CAST(@HierarchyID AS NVARCHAR(4000)) + N'
			ON Lvl'                        + CAST((@HierarchyID - 1) AS NVARCHAR(4000)) + N'.ObjectHierarchyID = Lvl'
                                           + CAST(@HierarchyID AS NVARCHAR(4000))
                                           + N'.ParentObjectHierarchyID
			AND Lvl'                       + CAST(@HierarchyID AS NVARCHAR(4000)) + N'.HierarchyLvl = '
                                           + CAST(@HierarchyID AS NVARCHAR(4000));
                            SET @HierarchyID = @HierarchyID + 1;
                        END;

                    SET @sqlCMD = @sqlSelect + @sqlFrom
                                  + N'
		...
		ORDER BY Lvl1.BaseObject_FullName, Lvl1.HierarchyLvl, Lvl1.Child_FullName;';
            ...
            IF @ShowObjectCount = 1
                BEGIN
                    SELECT CASE
                               WHEN #tmpObjectHierarchy.Child_Type = 'U' THEN 'User Table'
                               WHEN #tmpObjectHierarchy.Child_Type = 'S' THEN 'System Table'
                               WHEN #tmpObjectHierarchy.Child_Type = 'V' THEN 'View'
                               WHEN #tmpObjectHierarchy.Child_Type = 'FN' THEN 'Scalar Function'
                               WHEN #tmpObjectHierarchy.Child_Type = 'IF' THEN 'Inline Function'
                               WHEN #tmpObjectHierarchy.Child_Type = 'TF' THEN 'Table Valued Function'
                               WHEN #tmpObjectHierarchy.Child_Type = 'P' THEN 'Stored Procedure'
                               WHEN #tmpObjectHierarchy.Child_Type = 'SN' THEN 'Synonym'
