...
DECLARE @string_to_execute    NVARCHAR(1000),
        @current_name         NVARCHAR(200),
        @current_value_in_use SQL_VARIANT,
        @current_maximum      SQL_VARIANT;

CREATE TABLE #configs (
    name              NVARCHAR(200),
    value_in_use      SQL_VARIANT,
    maximum           SQL_VARIANT,
    clears_plan_cache BIT
);
...
WHILE @@FETCH_STATUS = 0
    BEGIN
        ...
        SET @string_to_execute = N'sp_configure ''' + @current_name + N''', '''
                                 + CAST(@current_maximum AS NVARCHAR(100)) + N'''; RECONFIGURE WITH OVERRIDE;';
        ...
        SET @string_to_execute = N'sp_configure ''' + @current_name + N''', '''
                                 + CAST(@current_value_in_use AS NVARCHAR(100)) + N'''; RECONFIGURE WITH OVERRIDE;';
        EXEC (@string_to_execute);
