/*---------------------------------------------------------------------
  $Header: /WWW/sqlutil/beta_lockinfo.sp 26    20-09-04 11:13 Sommar $
...
GO
-- Create shell.
IF OBJECT_ID ('beta_lockinfo') IS NULL
    EXEC ('CREATE PROCEDURE beta_lockinfo AS PRINT 12');
GO
...
DECLARE @dm_os_waiting_tasks TABLE (
    ...
    blocksamespidonly BIT          NOT NULL
        DEFAULT 0,
...
DECLARE @procs TABLE (
    ...
    spidstr               AS
        ...
        + CASE
              WHEN exec_context_id <> 0
                   OR request_id <> 0 THEN '/' + LTRIM (STR (exec_context_id)) + '/' + LTRIM (STR (request_id))
              ELSE ''
          END,
    ...
    quoted_id             BIT            NOT NULL
        DEFAULT 1,
    arithabort            BIT            NOT NULL
        DEFAULT 0,
    ansi_null_dflt        BIT            NOT NULL
        DEFAULT 1,
    ansi_defaults         BIT            NOT NULL
        DEFAULT 0,
    ansi_warns            BIT            NOT NULL
        DEFAULT 1,
    ansi_pad              BIT            NOT NULL
        DEFAULT 1,
    ansi_nulls            BIT            NOT NULL
        DEFAULT 1,
    concat_null           BIT            NOT NULL
        DEFAULT 1,
    isolation_lvl         SMALLINT       NOT NULL
        DEFAULT 2,
    ...
    blockercnt            INT            NULL,
    block_spidstr         AS
        LTRIM (STR (block_session_id))
        + CASE
              WHEN block_exec_context_id <> 0
                   OR block_request_id <> 0 THEN
                  '/' + LTRIM (STR (block_exec_context_id)) + '/' + LTRIM (STR (block_request_id))
              ELSE ''
          END + CASE
                    WHEN blockercnt > 1 THEN ' (+' + LTRIM (STR (blockercnt - 1)) + ')'
                    ELSE ''
                END,
    blocksamespidonly     BIT            NOT NULL
        DEFAULT 0,
    waiter_no_blocker     BIT            NOT NULL
        DEFAULT 0,
    wait_type             VARCHAR(60)    COLLATE Latin1_General_BIN2 NULL,
...
DECLARE @locks_takeone TABLE (
    ...
    activelock      AS CASE
                           WHEN rsc_type = 'DATABASE'
                                AND req_status = 'GRANT' THEN CONVERT (BIT, 0)
                           ELSE CONVERT (BIT, 1)
                       END,
...
CREATE TABLE #objects (
    idtype      CHAR(4)       NOT NULL CHECK (idtype IN ( 'OBJ', 'HOBT', 'AU', 'MISC' )),
    database_id INT           NOT NULL,
...
IF @archivemode IS NOT NULL
   AND NOT EXISTS (
    ...
    BEGIN
        RAISERROR (
            'To use @archivemode you need to have CONTROL permission in the database where beta_lockinfo is located.',
            16,
            1
        );
        RETURN;
...
INSERT @dm_os_waiting_tasks (wait_session_id, wait_task, block_session_id, block_task, wait_type, wait_duration_ms)
SELECT COALESCE (owt.session_id, -1 * (1000 + ROW_NUMBER () OVER (ORDER BY (SELECT 1)))),
       owt.waiting_task_address,
       owt.blocking_session_id,
       CASE
           WHEN owt.blocking_session_id IS NOT NULL THEN COALESCE (owt.blocking_task_address, 0x)
       END,
       owt.wait_type,
       owt.wait_duration_ms
FROM sys.dm_os_waiting_tasks AS owt
WHERE owt.session_id IS NOT NULL
      OR owt.wait_type NOT IN ( 'FT_IFTS_SCHEDULER_IDLE_WAIT', 'CLR_AUTO_EVENT', 'CLR_MANUAL_EVENT', 'CLR_SEMAPHORE',
                                'DISPATCHER_QUEUE_SEMAPHORE', 'HADR_CLUSAPI_CALL', 'FILESTREAM_WORKITEM_QUEUE',
                                'XTP_PREEMPTIVE_TASK', 'QDS_PERSIST_TASK_MAIN_LOOP_SLEEP', 'QDS_ASYNC_QUEUE'
);
...
(
    SELECT tst.session_id,
           ...
           tat.dtc_state,
           tst.is_bound,
           ROW_NUMBER () OVER (PARTITION BY tst.session_id
...
INSERT @transactions (session_id, is_user_trans, trans_start, trans_since, trans_type, trans_state, dtc_state, is_bound)
SELECT session_id,
       is_user_transaction,
       transaction_begin_time,
       CASE
           WHEN ABS (DATEDIFF (YEAR, transaction_begin_time, @now)) > 60 THEN NULL
...
SELECT es.session_id,
       COALESCE (ot.task_address, 0x),
       COALESCE (ot.exec_context_id, 0),
       COALESCE (er.request_id, 0),
       es.is_user_process,
       COALESCE (NULLIF(es.login_name, ''), SUSER_SNAME (es.security_id)),
       COALESCE (NULLIF(es.original_login_name, ''), SUSER_SNAME (es.original_security_id)),
       es.status,
       ot.task_state,
       es.endpoint_id,
       es.database_id,
       ...
       es.logical_reads,
       er.logical_reads,
       ssu.user_objects_alloc_page_count - ssu.user_objects_dealloc_page_count + ssu.internal_objects_alloc_page_count
       - ssu.internal_objects_dealloc_page_count,
       tsu.pages,
       NULLIF(er.percent_complete, 0),
       ...
       COALESCE (er.executing_managed_code, 0),
       er.nest_level,
       es.login_time,
       COALESCE (er.start_time, es.last_request_start_time),
       CASE
           WHEN ABS (DATEDIFF (YEAR, COALESCE (er.start_time, es.last_request_start_time), @now)) > 60 THEN NULL
       ...
       ROW_NUMBER () OVER (PARTITION BY es.session_id
ORDER BY ot.exec_context_id, er.request_id
       ...
       CASE -- A blocking task is always interesting.
           WHEN EXISTS (
                    SELECT *
                    FROM @dm_os_waiting_tasks AS owt
           ...
           WHEN ot.exec_context_id IS NOT NULL
                AND es.is_user_process = 1
                AND es.session_id <> @@spid
                AND COALESCE (er.wait_type, '') <> 'BROKER_RECEIVE_WAITFOR' THEN 0
           ...
           WHEN ot.exec_context_id IS NOT NULL
                AND er.wait_type = 'BROKER_RECEIVE_WAITFOR' THEN 1

           -- Processes with a high tempdb consumption are half-interesting.
           WHEN ssu.user_objects_alloc_page_count - ssu.user_objects_dealloc_page_count
                + ssu.internal_objects_alloc_page_count - ssu.internal_objects_dealloc_page_count > 1000 THEN 1
...
LEFT JOIN (
    SELECT session_id,
           request_id,
           SUM (
               ISNULL (user_objects_alloc_page_count, 0) - ISNULL (user_objects_dealloc_page_count, 0)
               + ISNULL (internal_objects_alloc_page_count, 0) - ISNULL (internal_objects_dealloc_page_count, 0)
           ) AS "pages"
...
SELECT COALESCE (pt.session_id, owt.wait_session_id),
       ...
       CASE
           WHEN pt.session_id IS NOT NULL THEN -9
           ELSE 0
       END,
       0,
       COALESCE (t.task_state, 'EXITED'),
       1,
       owt.wait_duration_ms,
       owt.wait_type,
       0
...
LEFT JOIN sys.dm_os_tasks AS t
    ON t.task_address = owt.wait_task
LEFT JOIN sys.dm_os_tasks AS pt
    ON t.parent_task_address = pt.task_address
WHERE owt.wait_session_id < -1000;

-- Delete these sessions, so they don't confuse the blocking chains.
DELETE @dm_os_waiting_tasks
WHERE wait_session_id < -1000;
...
IF @debug = 1
    ...
    BEGIN TRY
        ...
        JOIN msdb.dbo.sysjobs AS j
            ON j.job_id = CAST(CONVERT (BINARY(16), SUBSTRING (p.program_name, 30, 34), 1) AS UNIQUEIDENTIFIER)
...
(
    SELECT request_session_id,
           request_mode COLLATE Latin1_General_BIN2 AS "req_mode",
           resource_type COLLATE Latin1_General_BIN2 AS "rsc_type",
           resource_subtype COLLATE Latin1_General_BIN2 AS "rsc_subtype",
           request_status COLLATE Latin1_General_BIN2 AS "req_status",
           request_owner_type COLLATE Latin1_General_BIN2 AS "req_owner_type",
           CASE
               WHEN resource_type = 'APPLICATION' THEN NULLIF(resource_description COLLATE Latin1_General_BIN2, '')
           END AS "rsc_description",
           resource_lock_partition,
           resource_database_id,
           resource_associated_entity_id
    FROM sys.dm_tran_locks
)
INSERT @locks_takeone (session_id,
                       req_mode,
                       rsc_type,
                       rsc_subtype,
                       req_status,
...
SELECT request_session_id,
       ...
       resource_lock_partition,
       COUNT (*),
       RANK () OVER (ORDER BY request_session_id,
                              req_mode,
                              rsc_type,
                              rsc_subtype,
                              req_status,
...
IF @allprocesses < 2
    BEGIN
        ...
        WHERE EXISTS (
            SELECT *
            FROM @locks_takeone AS l
            WHERE l.session_id = p.session_id
                  AND l.activelock = 1
        );

        DELETE @procs
        WHERE activity_level > @allprocesses;
    END;
        ...
        SET curdbid = est.dbid,
            ...
            current_stmt = CASE
                               WHEN est.encrypted = 1 THEN
                                   '-- ENCRYPTED, pos ' + LTRIM (STR ((p.stmt_start + 2) / 2)) + ' - '
                                   + LTRIM (STR ((p.stmt_end + 2) / 2))
                               WHEN p.stmt_start = p.stmt_end THEN SUBSTRING (est.text, 1, 1000)
                               WHEN p.stmt_start >= 0 THEN
                                   SUBSTRING (
                                       est.text,
                                       (p.stmt_start + 2) / 2,
                                       CASE p.stmt_end
                           ...
                           END
        FROM @procs AS p
        CROSS APPLY sys.dm_exec_sql_text (p.sql_handle) AS est
...
IF @textmode = 0
    BEGIN
        ...
        BEGIN TRY
            ...
            FROM @procs AS p
            CROSS APPLY sys.dm_exec_query_statistics_xml (p.session_id) AS eqsx
            WHERE p.exec_context_id = 0
            ...
            FROM @procs AS p
            OUTER APPLY sys.dm_exec_text_query_plan (p.plan_handle, p.stmt_start, p.stmt_end) AS etqp
                ...
                WHILE 1 = 1
                    BEGIN
                        ...
                        BEGIN TRY
                            ...
                            UPDATE @procs
                            SET current_plan = (
                                    SELECT CONVERT (XML, etqp.query_plan)
                                    FROM sys.dm_exec_text_query_plan (@handle, @stmt_start, @stmt_end) AS etqp
                                )
                            FROM @procs AS p
                            WHERE p.session_id = @spid
                                  AND p.request_id = @request_id
...
UPDATE @procs
SET top5waits = (
        SELECT TOP (5) sws.wait_type + ':' + CONVERT (NVARCHAR(18), sws.wait_time_ms) + '; '
    ...
    inputbuffer = (
        SELECT REPLACE (ip.event_info COLLATE Latin1_General_BIN2, CHAR (0), '')
        FROM sys.dm_exec_input_buffer (p.session_id, p.request_id) AS ip
    )
FROM @procs AS p
WHERE p.exec_context_id = 0;
...
IF @debug = 1
    ...
    ;
WITH aggr AS (SELECT groupno, SUM (cnt) AS "cnt" FROM @locks_takeone GROUP BY groupno)
...
SELECT a.groupno,
       ...
       a.cnt,
       CASE
           WHEN lp.lock_partitions = '0' THEN ''
...
CROSS APPLY (
    SELECT TOP 1 b.session_id,
                 ...
                 b.entity_id,
                 b.activelock
    FROM @locks_takeone AS b
    WHERE a.groupno = b.groupno
    ORDER BY lock_partition
) AS b
OUTER APPLY
...
(
    SELECT CASE
               WHEN lp.prev IS NULL THEN lp.lpstr
               WHEN lp.lock_partition - lp.prev > 1 THEN
                   CASE
                       WHEN lp.prev - lp.prevprev = 1 THEN '-' + LTRIM (STR (lp.prev))
                       ELSE ''
                   END + ' ' + lp.lpstr
               WHEN lp.next IS NULL THEN CASE
                                             WHEN lp.lock_partition - lp.prev = 1 THEN '-' + lp.lpstr
                                             ELSE ''
                                         END
           END AS "text()"
    FROM (
        SELECT lt.lock_partition,
               LTRIM (STR (lt.lock_partition)) AS "lpstr",
               LAG (lt.lock_partition) OVER (ORDER BY lt.lock_partition) AS "prev",
               LAG (lt.lock_partition, 2) OVER (ORDER BY lt.lock_partition) AS "prevprev",
               LEAD (lt.lock_partition) OVER (ORDER BY lt.lock_partition) AS "next"
        FROM @locks_takeone AS lt
        WHERE a.groupno = lt.groupno
    ) AS lp
    ORDER BY lp.lock_partition
    FOR XML PATH ('')
) AS lp(lock_partitions)
...
WHERE (b.activelock = 1 AND b.session_id <> @@spid)
      OR EXISTS (SELECT * FROM @procs AS p WHERE b.session_id = p.session_id);
...
SELECT idtype,
       ...
       entity_id,
       CASE
           WHEN database_id = 2 THEN MIN (session_id)
       END
FROM (
    SELECT CASE
               WHEN rsc_type = 'OBJECT' THEN 'OBJ'
...
IF NOT EXISTS (
    ...
    BEGIN
        UPDATE #objects
        SET object_name = 'You do not have permissions to access the database ' + QUOTENAME (DB_NAME (database_id))
                          + '.'
...
WHILE 1 = 1
    BEGIN
        ...
        SELECT @objnameexpr = CASE @dbname
                                  WHEN '[tempdb]' THEN
                                      'CASE WHEN len(o.name) = 9 AND
        ...
        SELECT @parentjoin = CASE @dbname
                                 WHEN '[tempdb]' THEN
                                     ' LEFT JOIN tempdb.sys.objects par
        ...
        -- extras for tempdb.
        SELECT @indexnameexpr = N'
               CASE WHEN p.index_id <= 1
                     THEN "" ' + CASE
                                     WHEN @dbname = '[tempdb]' THEN
                                         +'
   ...
   '    ;
      ...
      ' ;
        ...
        SELECT @stmt = REPLACE (REPLACE (REPLACE (@stmt, '"', ''''), '@dbname', @dbname), '@dbidstr', @dbidstr);
...
FROM #objects AS ob
JOIN mintemp AS m
    ON m.object_name = ob.object_name
...
IF @rowc > 0
    BEGIN
        ...
        JOIN #objects AS ob
            ON l.database_id = ob.database_id
...
FROM @dm_os_waiting_tasks AS a
WHERE a.block_session_id IS NULL
      AND EXISTS (
    SELECT *
    FROM @dm_os_waiting_tasks AS b
...
INSERT @dm_os_waiting_tasks (wait_session_id, wait_task, block_level, lead_blocker_spid)
SELECT DISTINCT a.block_session_id,
                COALESCE (a.block_task, 0x),
                0,
                a.block_session_id
FROM @dm_os_waiting_tasks AS a
WHERE NOT EXISTS (
...
WHILE @rowc > 0
    BEGIN
        ...
        FROM @dm_os_waiting_tasks AS a
        JOIN @dm_os_waiting_tasks AS b
            ON a.block_session_id = b.wait_session_id
               AND a.block_task = b.wait_task
...
WITH block_chain AS
(
    SELECT wait_session_id,
           wait_task,
           block_session_id,
           block_task,
           CASE
               WHEN block_level >= 0 THEN block_level
               ELSE block_level - @lvl - 1
           END AS "block_level",
           wait_duration_ms,
           wait_type,
           blocksamespidonly,
           COUNT (*) OVER (PARTITION BY wait_task) AS "cnt",
           ROW_NUMBER () OVER (PARTITION BY wait_task
...
FROM @procs AS p
JOIN block_chain AS bc
    ON p.session_id = bc.wait_session_id
       AND p.task_address = bc.wait_task
       AND bc.rowno = 1
LEFT JOIN @procs AS p2
    ON bc.block_session_id = p2.session_id
...
IF @procdata = 'F'
    BEGIN
        ...
        (
            SELECT lockno,
                   ROW_NUMBER () OVER (PARTITION BY l.session_id
            ...
            LEFT JOIN #objects AS o
                ON l.database_id = o.database_id
...
WHERE EXISTS (
    ...
    WHERE l.req_status = 'WAIT'
          ...
          AND NOT EXISTS (SELECT * FROM @procs AS p2 WHERE p.session_id = l.session_id)
);
...
INSERT #output (spid,
                ...
                rowno,
                spidnum)
SELECT COALESCE (p.spidstr, LTRIM (STR (l.session_id))) AS "spid",
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN p.request_command
           ELSE ''
       END AS "command",
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               CASE
                   WHEN p.is_user_process = 0 THEN 'SYSTEM PROCESS'
                   ELSE p.orig_login + CASE
                                           WHEN p.current_login <> p.orig_login
                                                OR p.orig_login IS NULL THEN ' (' + RTRIM (p.current_login) + ')'
                                           ELSE ''
                                       END
               END
           ELSE ''
       ...
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN p.host_name
           ELSE ''
       END AS "host",
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN LTRIM (STR (p.host_process_id))
           ELSE ''
       END AS "hostprc",
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN e.name
           ELSE ''
       END AS "endpoint",
       CASE
           WHEN COALESCE (p.exec_context_id, 0) = 0
                AND COALESCE (l.lockno, 1) = 1 THEN p.program_name
           ELSE ''
       END AS "appl",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1
                AND COALESCE (p.exec_context_id, 0) = 0 THEN COALESCE (DB_NAME (p.request_dbid), DB_NAME (p.proc_dbid))
           ELSE ''
       END AS "dbname",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (p.task_state, p.session_state)
           ELSE ''
       END AS "prcstatus",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN
               CASE
                   WHEN p.quoted_id = 0 THEN 'qid '
                   ELSE ''
               END + CASE
                         WHEN p.arithabort = 1 THEN 'ARITH '
                         ELSE ''
                     END + CASE
                               WHEN p.ansi_defaults = 1 THEN 'ADEF '
                               ELSE ''
                           END + CASE
                                     WHEN p.ansi_null_dflt = 0 THEN 'ando '
                                     ELSE ''
                                 END + CASE
                                           WHEN p.ansi_warns = 0 THEN 'awarn '
                                           ELSE ''
                                       END + CASE
                                                 WHEN p.ansi_pad = 0 THEN 'apad '
                                                 ELSE ''
                                             END + CASE
                                                       WHEN p.ansi_nulls = 0 THEN 'anull '
                                                       ELSE ''
                                                   END + CASE
                                                             WHEN p.concat_null = 0 THEN 'cnyn '
                                                             ELSE ''
                                                         END
           ELSE ''
       END AS "ansiopts",
       ...
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN
               ...
               END + CASE
                         WHEN p.lock_timeout >= 0 THEN 'LT=' + LTRIM (STR (p.lock_timeout)) + '  '
                         ELSE ''
                     END + CASE
                               WHEN p.deadlock_pri <> 0 THEN 'DP=' + LTRIM (STR (p.deadlock_pri))
                               ELSE ''
                           END
           ELSE ''
       END AS "trnopts",
       CASE
           WHEN p.exec_context_id = 0 THEN COALESCE (LTRIM (STR (NULLIF(p.trancount, 0))), '')
           ELSE ''
       END AS "opntrn",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1
                AND p.exec_context_id = 0
                AND t.is_user_trans IS NOT NULL THEN
               CASE t.is_user_trans
                   WHEN 1 THEN 'U'
                   ELSE 'S'
               END + '-' + CASE t.trans_type
                               WHEN 1 THEN 'RW'
                               WHEN 2 THEN 'R'
                               WHEN 3 THEN 'SYS'
                               WHEN 4 THEN 'DIST'
                               ELSE LTRIM (STR (t.trans_type))
                           END + '-' + LTRIM (STR (t.trans_state)) + CASE t.dtc_state
                                                                         WHEN 0 THEN ''
                                                                         ELSE '-'
                                                                     END + CASE t.dtc_state
                                                                               WHEN 0 THEN ''
       ...
       END AS "trninfo",
       CASE
           WHEN p.block_level IS NOT NULL THEN CASE p.blocksamespidonly
                                                   WHEN 1 THEN '('
                                                   ELSE ''
                                               END + CASE
                                                         WHEN p.block_level = 0 THEN '!!'
                                                         ELSE LTRIM (STR (p.block_level))
                                                     END + CASE p.blocksamespidonly
                                                               WHEN 1 THEN ')'
                                                               ELSE ''
                                                           END
           ...
           -- dead lock.
           WHEN p.block_session_id IS NOT NULL THEN 'DD'
           WHEN p.waiter_no_blocker = 1 THEN '??'
           ELSE ''
       END AS "blklvl",
       COALESCE (p.block_spidstr, '') AS "blkby",
       CASE
           WHEN p.exec_context_id = 0
                AND p.request_id = 0 THEN COALESCE (LTRIM (STR (l.cnt)), '0')
           ELSE ''
       END AS "cnt",
       CASE l.rsc_type
           WHEN 'APPLICATION' THEN COALESCE (DB_NAME (l.database_id) + '|', '') + l.rsc_description
           ELSE COALESCE (o2.object_name, DB_NAME (l.database_id), '')
       END AS "object",
       COALESCE (l.rsc_type, '') AS "rsctype",
       COALESCE (l.req_mode, '') AS "locktype",
       CASE l.req_status
           WHEN 'GRANT' THEN LOWER (l.req_status)
           ELSE COALESCE (l.req_status, '')
       END AS "lstatus",
       CASE l.req_owner_type
           WHEN 'SHARED_TRANSACTION_WORKSPACE' THEN 'STW'
           ELSE COALESCE (l.req_owner_type, '')
       END AS "ownertype",
       COALESCE (l.rsc_subtype, '')
       + CASE
             WHEN LEN (l.lock_partitions) > 0 THEN CASE
                                                       WHEN LEN (l.rsc_subtype) > 0 THEN ' '
                                                       ELSE ''
                                                   END + '[' + l.lock_partitions + ']'
             ELSE ''
         END AS "rscsubtype",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (LTRIM (STR (p.wait_time, 18, 3)), '')
           ELSE ''
       END AS "waittime",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (p.wait_type, '')
           ELSE ''
       END AS "waittype",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (p.top5waits, '')
           ELSE ''
       END AS "waittype",
       p.spidstr AS "spid__",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               COALESCE (LTRIM (STR (p.session_cpu)), '')
               + CASE
                     WHEN p.request_cpu IS NOT NULL THEN ' (' + LTRIM (STR (p.request_cpu)) + ')'
                     ELSE ''
                 END
           ELSE ''
       END AS "cpu",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               COALESCE (LTRIM (STR (p.session_physio, 18)), '')
               + CASE
                     WHEN p.request_physio IS NOT NULL THEN ' (' + LTRIM (STR (p.request_physio, 18)) + ')'
                     ELSE ''
                 END
           ELSE ''
       END AS "physio",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               COALESCE (LTRIM (STR (p.session_logreads, 18)), '')
               + CASE
                     WHEN p.request_logreads IS NOT NULL THEN ' (' + LTRIM (STR (p.request_logreads, 18)) + ')'
                     ELSE ''
                 END
           ELSE ''
       END AS "logreads",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (LTRIM (STR (p.memory_grant, 18, 3)), '')
           ELSE ''
       END AS "memgrant",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (LTRIM (STR (p.percent_complete, 3)) + ' %', '')
           ELSE ''
       END AS "progress",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               COALESCE (LTRIM (STR (p.session_tempdb, 18)), '')
               + CASE
                     WHEN p.request_tempdb IS NOT NULL THEN ' (' + LTRIM (STR (p.request_tempdb)) + ')'
                     ELSE ''
                 END
           ELSE ''
       END AS "tempdb",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN CONVERT (CHAR(12), @now, 114)
           ELSE ''
       END AS "now",
       CASE
           WHEN p.exec_context_id = 0
               ...
               CASE DATEDIFF (DAY, p.login_time, @now)
                   WHEN 0 THEN CONVERT (VARCHAR(8), p.login_time, 8)
                   ELSE CONVERT (CHAR(7), p.login_time, 12) + CONVERT (VARCHAR(8), p.login_time, 8)
               END
           ELSE ''
       END AS "login_time",
       CASE
           WHEN p.exec_context_id = 0
               ...
               CASE DATEDIFF (DAY, p.last_batch, @now)
                   WHEN 0 THEN CONVERT (VARCHAR(8), p.last_batch, 8)
                   ELSE CONVERT (CHAR(7), p.last_batch, 12) + CONVERT (VARCHAR(8), p.last_batch, 8)
               END
           ELSE ''
       END AS "last_batch",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1
                AND t.trans_start IS NOT NULL THEN
               CASE DATEDIFF (DAY, t.trans_start, @now)
                   WHEN 0 THEN CONVERT (VARCHAR(8), t.trans_start, 8)
                   ELSE CONVERT (CHAR(7), t.trans_start, 12) + CONVERT (VARCHAR(8), t.trans_start, 8)
               END
           ELSE ''
       END AS "trn_start",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN
               CASE
                   WHEN p.last_since < 0 THEN STR (p.last_since, 17, 3)
                   ELSE
                       CASE
                           WHEN p.last_since >= 86400 THEN STR (CONVERT (INT, p.last_since) / 86400, 4)
                           ELSE SPACE (4)
                       END + SPACE (CASE
                                        WHEN p.last_since >= 36000 THEN 1
                       ...
                       + SUBSTRING (
                             CONVERT (CHAR(8), DATEADD (SECOND, p.last_since, '19000101'), 108),
                             CASE
                                 WHEN p.last_since >= 36000 THEN 1
                                 WHEN p.last_since >= 3600 THEN 2
                                 WHEN p.last_since >= 600 THEN 4
       ...
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1
                AND t.trans_since IS NOT NULL THEN
               CASE
                   WHEN t.trans_since < 0 THEN STR (t.trans_since, 17, 3)
                   ELSE
                       CASE
                           WHEN t.trans_since >= 86400 THEN STR (CONVERT (INT, t.trans_since) / 86400, 4)
                           ELSE SPACE (4)
                       END + SPACE (CASE
                                        WHEN t.trans_since >= 36000 THEN 1
                       ...
                       + SUBSTRING (
                             CONVERT (CHAR(8), DATEADD (SECOND, t.trans_since, '19000101'), 108),
                             CASE
                                 WHEN t.trans_since >= 36000 THEN 1
                                 WHEN t.trans_since >= 3600 THEN 2
                                 WHEN t.trans_since >= 600 THEN 4
       ...
       CASE
           WHEN p.exec_context_id = 0
                AND p.isclr = 1 THEN 'CLR'
           ELSE ''
       END AS "clr",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN COALESCE (LTRIM (STR (p.nest_level)), '')
           ELSE ''
       END AS "nstlvl",
       p.spidstr AS "spid___",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN COALESCE (p.inputbuffer, '')
           ELSE ''
       END AS "inputbuffer",
       COALESCE (o1.object_name, '') AS "current_sp",
       CASE
           WHEN COALESCE (l.lockno, 1) = 1 THEN COALESCE (p.current_stmt, '')
           ELSE COALESCE (SUBSTRING (p.current_stmt, 1, 50), '')
       END AS "curstmt",
       CASE
           WHEN p.exec_context_id = 0
                AND COALESCE (l.lockno, 1) = 1 THEN p.current_plan
       END AS "current_plan",
       ROW_NUMBER () OVER (ORDER BY COALESCE (p.session_id, l.session_id),
                                    p.exec_context_id,
       ...
       COALESCE (p.session_id, l.session_id) AS "spidnum"
FROM @procs AS p
LEFT JOIN #objects AS o1
    ON p.curdbid = o1.database_id
       AND p.curobjid = o1.entity_id
LEFT JOIN sys.endpoints AS e
    ON p.endpoint_id = e.endpoint_id
LEFT JOIN @transactions AS t
    ON t.session_id = p.session_id
FULL JOIN(@locks_final AS l
LEFT JOIN #objects AS o2
    ON l.database_id = o2.database_id
...
IF @textmode = 1
    BEGIN
        ...
        SELECT @spidlen = CONVERT (VARCHAR(5), COALESCE (NULLIF(MAX (COALESCE (LEN (spid), 4)), 0), 1)),
               ...
               @last_sincelen = CONVERT (
                                    VARCHAR(5), COALESCE (NULLIF(MAX (COALESCE (LEN (LTRIM (last_since)), 4)), 0), 1)
                                ),
               @trn_sincelen = CONVERT (
                                   VARCHAR(5), COALESCE (NULLIF(MAX (COALESCE (LEN (LTRIM (trn_since)), 4)), 0), 1)
                               ),
               @inputbufferlen = CONVERT (
                                     VARCHAR(5),
                                     COALESCE (NULLIF(MAX (COALESCE (LEN (SUBSTRING (inputbuffer, 1, 4000)), 4)), 0), 1)
                                 ),
...
do_archive:
IF @archivemode <= 0
   AND OBJECT_ID ('guest.beta_lockinfo') IS NOT NULL
...
IF OBJECT_ID ('guest.beta_lockinfo') IS NULL
    BEGIN
        ...
        CREATE TABLE guest.beta_lockinfo (
            ...
            queryplan   XML           NULL,
            rowno       INT           NOT NULL,
            CONSTRAINT beta_lockinfo_pk
                PRIMARY KEY (now, rowno)
        );
