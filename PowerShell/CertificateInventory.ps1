...
# To database
Invoke-Command -ComputerName $computers -ScriptBlock { Get-ChildItem -Path cert:\LocalMachine\My -Recurse -ExpiringInDays 60 } | 
...
Invoke-Command -ComputerName $computers -ScriptBlock {Get-ChildItem -Path cert:\LocalMachine\My -Recurse} | 
    ...
    Export-Excel -Path "c:\temp\certs.xlsx" -FreezeTopRow -BoldTopRow -AutoSize -TableStyle Dark10
