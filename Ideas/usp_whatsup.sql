...
/****** Object:  StoredProcedure [dbo].[usp_whatsup]    Script Date: 10-8-2021 07:59:38 ******/
SET ANSI_NULLS ON;
GO
...
CREATE PROCEDURE dbo.usp_whatsup
    @sqluptime BIT      = 1,
    @requests  BIT      = 1,
    @blocking  BIT      = 1,
    @spstats   BIT      = 0,
...
AS
    ...
    -- Returns running sessions/requests; blocking information; 
	-- sessions that have been granted locks or waiting for locks; 
	-- and optionally top SP/Query/Trigger/Function execution stats.
    ...
    BEGIN
        ...
        BEGIN
            SET @sqlcmdup
                = N'SELECT @StartDateOUT = login_time, @UpTimeOUT = DATEDIFF(mi, login_time, GETDATE()) FROM master..sysprocesses WHERE spid = 1';
        END;
        ELSE
        BEGIN
            SET @sqlcmdup
                = N'SELECT @StartDateOUT = sqlserver_start_time, @UpTimeOUT = DATEDIFF(mi,sqlserver_start_time,GETDATE()) FROM sys.dm_os_sys_info';
        END;

        SET @params = N'@StartDateOUT DATETIME OUTPUT, @UpTimeOUT VARCHAR(12) OUTPUT';

        EXECUTE sp_executesql @sqlcmdup,
                              @params,
                              @StartDateOUT = @StartDate OUTPUT,
                              @UpTimeOUT = @UpTime OUTPUT;

        SELECT 'Uptime_Information' AS Information,
               GETDATE () AS [Current_Time],
               @StartDate AS Last_Startup,
               CONVERT (VARCHAR(4), @UpTime / 60 / 24) + 'd ' + CONVERT (VARCHAR(4), @UpTime / 60 % 24) + 'h '
    ...
    BEGIN
        ...
        BEGIN
            SELECT @sqlcmd
                = N'SELECT ''Requests'' AS [Information], es.session_id, DB_NAME(er.database_id) AS [database_name], OBJECT_NAME(qp.objectid, qp.dbid) AS [object_name], -- NULL if Ad-Hoc or Prepared statements
...
ORDER BY er.total_elapsed_time DESC, er.logical_reads DESC, [database_name], session_id';
        END;
        ELSE IF @sqlmajorver IN ( 10, 11, 12 )
                OR (@sqlmajorver = 13 AND @sqlbuild < 4000)
        BEGIN
            SET @sqlcmd
                = N';WITH tsu AS (SELECT session_id, SUM(user_objects_alloc_page_count) AS user_objects_alloc_page_count, 
...
ORDER BY er.total_elapsed_time DESC, er.logical_reads DESC, [database_name], session_id';
        END;
        ELSE IF (@sqlmajorver = 13 AND @sqlbuild > 4000)
                OR @sqlmajorver = 14
                OR (@sqlmajorver = 15 AND @sqlbuild < 1400)
        BEGIN
            SELECT @sqlcmd
                = N'WITH tsu AS (SELECT session_id, SUM(user_objects_alloc_page_count) AS user_objects_alloc_page_count, 
SUM(user_objects_dealloc_page_count) AS user_objects_dealloc_page_count, 
...
ORDER BY er.total_elapsed_time DESC, er.logical_reads DESC, [database_name], session_id';
        END;
        ELSE IF (@sqlmajorver = 15 AND @sqlbuild >= 1400)
                OR @sqlmajorver > 15
        BEGIN
            SELECT @sqlcmd
                = N'WITH tsu AS (SELECT session_id, SUM(user_objects_alloc_page_count) AS user_objects_alloc_page_count, 
...
ORDER BY er.total_elapsed_time DESC, er.logical_reads DESC, [database_name], session_id';
    ...
    BEGIN
		...
		*/
               ...
               CASE
                   WHEN owt.pageid = 1
                        OR owt.pageid % 8088 = 0 THEN 'Is_PFS_Page'
                   WHEN owt.pageid = 2
                        OR owt.pageid % 511232 = 0 THEN 'Is_GAM_Page'
                   WHEN owt.pageid = 3
                        OR (owt.pageid - 1) % 511232 = 0 THEN 'Is_SGAM_Page'
               ...
               LEFT(CASE COALESCE (es.transaction_isolation_level, er.transaction_isolation_level)
                        ...
                        ELSE
                            CONVERT (
                                VARCHAR(30), COALESCE (es.transaction_isolation_level, er.transaction_isolation_level)
                            ) + '-UNKNOWN'
               ...
               CASE
                   ...
                   WHEN (er2.session_id IS NULL OR owt.blocking_session_id IS NULL)
                        AND (er.blocking_session_id = 0 OR er.session_id IS NULL) THEN 1
                                                                                                                                      ...
                                                                                                                                      REPLACE (
                                                                                                                                          REPLACE (
                                                                                                                                              SUBSTRING (
                                                                                                                                                  ...
                                                                                                                                                  1
                                                                                                                                                  + (CASE
                                                                                                                                                         WHEN er2.statement_start_offset = 0 THEN
                                                                                                                                                             0
                                                                                                                                                         ELSE
                                                                                                                                                             er2.statement_start_offset
                                                                                                                                                             / 2
                                                                                                                                                     END
                                                                                                                                                    ),
                                                                                                                                                  1
                                                                                                                                                  + (CASE
                                                                                                                                                         WHEN er2.statement_end_offset = -1 THEN
                                                                                                                                                             DATALENGTH (
                                                                                                                                                                 qt2.text
                                                                                                                                                             )
                                                                                                                                                         ELSE
                                                                                                                                                             er2.statement_end_offset
                                                                                                                                                             / 2
                                                                                                                                                     END
                                                                                                                                                     - (CASE
                                                                                                                                                            WHEN er2.statement_start_offset = 0 THEN
               ...
               LEFT(CASE COALESCE (er2.transaction_isolation_level, es.transaction_isolation_level)
                        ...
                        ELSE
                            CONVERT (
                                VARCHAR(30), COALESCE (er2.transaction_isolation_level, es.transaction_isolation_level)
                            ) + '-UNKNOWN'
               ...
               CASE
                   WHEN es.session_id = -2 THEN 'Orphaned_distributed_tran'
                   WHEN es.session_id = -3 THEN 'Defered_recovery_tran'
                   WHEN es.session_id = -4 THEN 'Unknown_tran'
                   ELSE NULL
               ...
               CASE
                   WHEN es2.session_id = -2 THEN 'Orphaned_distributed_tran'
                   WHEN es2.session_id = -3 THEN 'Defered_recovery_tran'
                   WHEN es2.session_id = -4 THEN 'Unknown_tran'
                   ELSE NULL
               END AS blocker_session_comment,
               es2.is_user_process AS blocker_is_user_process
        FROM sys.dm_exec_sessions (NOLOCK) AS es
        LEFT OUTER JOIN sys.dm_exec_requests (NOLOCK) AS er
            ON es.session_id = er.session_id
        LEFT OUTER JOIN sys.dm_exec_connections (NOLOCK) AS ec
            ON es.session_id = ec.session_id
        LEFT OUTER JOIN sys.dm_os_tasks (NOLOCK) AS ot
            ON er.session_id = ot.session_id
               AND er.request_id = ot.request_id
        LEFT OUTER JOIN sys.dm_exec_sessions (NOLOCK) AS es2
            ON er.blocking_session_id = es2.session_id
        LEFT OUTER JOIN sys.dm_exec_requests (NOLOCK) AS er2
            ON es2.session_id = er2.session_id
        LEFT OUTER JOIN sys.dm_exec_connections (NOLOCK) AS ec2
        ...
        LEFT OUTER JOIN (
            ...
            SELECT waiting_task_address,
                   ...
                   CASE
                       WHEN wait_type LIKE 'PAGE%'
                            AND resource_description LIKE '%:%' THEN
                           CAST(RIGHT(resource_description, LEN (resource_description)
                                                            - CHARINDEX (
                                                                  ':',
                                                                  resource_description,
                                                                  LEN (resource_description)
                                                                  - CHARINDEX (':', REVERSE (resource_description))
                                                              )) AS INT)
                       WHEN wait_type LIKE 'LCK%'
                            AND resource_description LIKE '%pageid%'
                            AND ISNUMERIC (
                                    RIGHT(LEFT(resource_description, CHARINDEX (
                                                                         'dbid=',
                                                                         resource_description,
                                                                         CHARINDEX ('pageid=', resource_description)
                                                                         + 6
                                                                     ) - 1), CHARINDEX (
                                                                                 '=',
                                                                                 REVERSE (
                                                                                     RTRIM (
                                                                                         LEFT(resource_description, CHARINDEX (
                                                                                                                        'dbid=',
                                                                                                                        resource_description,
                                                                                                                        CHARINDEX (
                                                                                                                            'pageid=',
                           ...
                           CAST(RIGHT(LEFT(resource_description, CHARINDEX (
                                                                     'dbid=',
                                                                     resource_description,
                                                                     CHARINDEX ('pageid=', resource_description) + 6
                                                                 ) - 1), CHARINDEX (
                                                                             '=',
                                                                             REVERSE (
                                                                                 RTRIM (
                   ...
                   CASE
                       WHEN wait_type LIKE 'LCK%'
                            AND resource_description LIKE '%associatedObjectId%'
                            AND ISNUMERIC (
                                    RIGHT(resource_description, CHARINDEX ('=', REVERSE (resource_description)) - 1)
                                ) = 1 THEN
                           CAST(RIGHT(resource_description, CHARINDEX ('=', REVERSE (resource_description)) - 1) AS BIGINT)
                       ELSE NULL
                   END AS objid,
            ...
            ON ot.task_address = owt.waiting_task_address
               AND owt.row_num = 1
        ...
        WHERE es.session_id <> @@SPID
              ...
              AND (
                  es.session_id IN ( SELECT er3.blocking_session_id FROM sys.dm_exec_requests (NOLOCK) AS er3 )
                  OR er.blocking_session_id IS NOT NULL
    ...
    BEGIN
        SELECT @sqlcmd
            = N'SELECT' + CASE
                              ...
                              ELSE ' TOP ' + CONVERT (NVARCHAR(10), @top)
                          END
              + ' CASE WHEN CONVERT(int,pa.value) = 32767 THEN ''ResourceDB'' ELSE DB_NAME(CONVERT(int,pa.value)) END AS DatabaseName,
	(SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
		st.text,
		NCHAR(0),N''?''),NCHAR(1),N''?''),NCHAR(2),N''?''),NCHAR(3),N''?''),NCHAR(4),N''?''),NCHAR(5),N''?''),NCHAR(6),N''?''),NCHAR(7),N''?''),NCHAR(8),N''?''),NCHAR(11),N''?''),NCHAR(12),N''?''),NCHAR(14),N''?''),NCHAR(15),N''?''),NCHAR(16),N''?''),NCHAR(17),N''?''),NCHAR(18),N''?''),NCHAR(19),N''?''),NCHAR(20),N''?''),NCHAR(21),N''?''),NCHAR(22),N''?''),NCHAR(23),N''?''),NCHAR(24),N''?''),NCHAR(25),N''?''),NCHAR(26),N''?''),NCHAR(27),N''?''),NCHAR(28),N''?''),NCHAR(29),N''?''),NCHAR(30),N''?''),NCHAR(31),N''?'') 
		AS [text()]
	...
	CASE WHEN qs.total_used_threads IS NOT NULL THEN qs.total_used_threads/qs.execution_count ELSE -1 END AS avg_used_threads'
                    ELSE ''
                END
              + CASE
                    WHEN @sqlmajorver >= 15
                         OR (@sqlmajorver = 13 AND @sqlbuild >= 5026)
                         OR (@sqlmajorver = 14 AND @sqlbuild >= 3015) THEN
                        ',
	...
	CASE WHEN qs.total_spills IS NOT NULL THEN qs.total_spills/qs.execution_count ELSE -1 END AS avg_spills'
                    ELSE ''
                END
              + '
    ...
    BEGIN
        SET @sqlcmd
            = N'SELECT' + CASE
                              ...
                              ELSE ' TOP ' + CONVERT (NVARCHAR(10), @top)
                          END
              + ' CASE WHEN ps.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(ps.database_id) END AS DatabaseName, 
    ...
    BEGIN
        SET @sqlcmd
            = N'SELECT' + CASE
                              ...
                              ELSE ' TOP ' + CONVERT (NVARCHAR(10), @top)
                          END
              + ' CASE WHEN ts.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(ts.database_id) END AS DatabaseName, 
    ...
    BEGIN
        SET @sqlcmd
            = N'SELECT' + CASE
                              ...
                              ELSE ' TOP ' + CONVERT (NVARCHAR(10), @top)
                          END
              + ' CASE WHEN fs.database_id = 32767 THEN ''ResourceDB'' ELSE DB_NAME(fs.database_id) END AS DatabaseName, 
