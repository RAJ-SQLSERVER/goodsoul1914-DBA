...
ON dbo.session_capture (date_captured ASC, session_id ASC);

CREATE NONCLUSTERED INDEX IDX2
ON dbo.session_capture (date_captured ASC, wait_time ASC, blocking_session_id ASC);

CREATE NONCLUSTERED INDEX IDX3
ON dbo.session_capture (database_id ASC, wait_time ASC, wait_type ASC, host_name ASC);

GO

--3. Create the following 2 Stored Procedures in your utility DB:-
CREATE PROC dbo.SessionCapture @TargetSizeMB BIGINT       = 4000,  -- Purge oldest 20% of data if SessionCapture table is larger than this size (MB)
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                ...
                SET @tableHTML = N'<H5 style="font-family:verdana"> Blocking and blocked sessions</H5>'
                ...
                CAST((
                    SELECT DISTINCT sp.spid AS "td",
                                    ...
                                    sp.hostname AS "td",
                                    '     ',
                                    sp.nt_username + ' ' AS "td",
                                    '     ',
                                    sd.name AS "td",
                                    '     ',
                                    CAST(st.text AS NVARCHAR(500)) AS "td",
                                    '     ',
                                    CASE
                                        WHEN sp.stmt_start > 0 THEN
                                            CASE
                                                ...
                                                ELSE SUBSTRING (st2.text, 1 + sp.stmt_start / 2, LEN (st2.text))
                                            END
                                        ELSE ''
                                    END AS "td",
                                    '     ',
                                    ...
                                    END AS "td",
                                    '     '
                    FROM sys.sysprocesses (NOLOCK) AS sp
                    LEFT OUTER JOIN sys.sysprocesses (NOLOCK) AS s2
                        ON sp.spid = s2.blocked
                    INNER JOIN sys.sysdatabases (NOLOCK) AS sd
                        ON sp.dbid = sd.dbid
                    LEFT OUTER JOIN sys.dm_exec_requests AS er
                        ON sp.spid = er.session_id
                    CROSS APPLY (SELECT text AS "text()" FROM sys.dm_exec_sql_text (sp.sql_handle) ) AS st(text)
        ...
        BEGIN
            ...
            DELETE sc
            FROM (SELECT TOP (@totalrows) * FROM session_capture ORDER BY date_captured) AS sc;
