...
IF OBJECT_ID ('tempdb..#Power') IS NOT NULL 
	DROP TABLE #Power;

CREATE TABLE #Power (PowerScheme VARCHAR(1000));

INSERT #Power (PowerScheme)
EXEC sys.xp_cmdshell 'powercfg /list';

SELECT PowerScheme
FROM #Power
WHERE RIGHT(LTRIM (RTRIM (PowerScheme)), 1) = '*';

-- Set to High Performance
IF @isGettinFixed = 1
BEGIN
    EXEC sys.xp_cmdshell 'powercfg.exe /setactive 8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c';
END;
