...
-- 
-- If your DBA database is not named 'DBA', you'll want to replace it with your DB name
...
CREATE TABLE dbo.IndexUsageStats_LastCumulative (
    ...
    CONSTRAINT PK_IUS_C
        PRIMARY KEY CLUSTERED (ServerNameID, DatabaseID, ObjectID, IndexID)
);
GO
...
CREATE TABLE dbo.IndexUsageStats (
    ...
    System_Updates BIGINT       NOT NULL,
    CONSTRAINT PK_IUS
        PRIMARY KEY CLUSTERED (StatsDate, ServerNameID, DatabaseID, ObjectID, IndexID),
...
SELECT s.StatsDate,
       ...
       s.System_Lookups,
       s.System_Updates
FROM dbo.IndexUsageStats AS s
INNER JOIN dbo.Names AS vn
    ON s.ServerNameID = vn.ID
INNER JOIN dbo.Names AS dbn
    ON s.DatabaseNameID = dbn.ID
INNER JOIN dbo.Names AS sn
    ON s.SchemaNameID = sn.ID
INNER JOIN dbo.Names AS tn
    ON s.TableNameID = tn.ID
LEFT JOIN dbo.Names AS dn
