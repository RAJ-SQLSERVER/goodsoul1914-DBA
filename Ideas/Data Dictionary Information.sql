...
SELECT @InstanceName AS "InstanceName",
       ...
       CASE
           ...
           WHEN st.name IN ( 'binary', 'char', 'text', 'varbinary', 'varchar' )
                AND c.max_length > -1 THEN st.name + '(' + CONVERT (VARCHAR(6), c.max_length) + ')'
           --Unicode fields require twice the storage as standard variable length fields
           WHEN st.name IN ( 'nchar', 'ntext', 'nvarchar' )
                AND c.max_length > -1 THEN st.name + '(' + CONVERT (VARCHAR(6), c.max_length / 2) + ')'
           ...
           WHEN st.name IN ( 'decimal', 'numeric' ) THEN
               st.name + '(' + CONVERT (VARCHAR(3), c.precision) + ', ' + CONVERT (VARCHAR(3), c.scale) + ')'
