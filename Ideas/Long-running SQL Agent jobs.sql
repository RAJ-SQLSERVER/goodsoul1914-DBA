...
DELETE FROM @temp_xp_sqlagent_enum_jobs
WHERE --not executing
    job_state != 1;

--sanity check: do we have any running jobs? If not, exit
IF 0 = (SELECT COUNT (*) FROM @temp_xp_sqlagent_enum_jobs) RETURN;
...
DECLARE @TableHtml NVARCHAR(MAX) = (
            ...
            SELECT --job name
                ...
                CONVERT (
                    INT,
                    CASE
                        WHEN MAX (CURRENTLY_RUNNING.job_state) = 1
                             AND MAX (sja.start_execution_date) IS NOT NULL THEN
                            DATEDIFF (MINUTE, MAX (sja.start_execution_date), GETDATE ())
                    END
                ) AS "td",
                --average for all executions from recent history
                MAX (AVERAGES.avg_duration_in_mins) AS "td"
            FROM --from docs: "Stores the information for each scheduled job to be executed by SQL Server Agent."
                msdb..sysjobs AS j
            INNER JOIN @temp_xp_sqlagent_enum_jobs AS CURRENTLY_RUNNING
                ON j.job_id = CURRENTLY_RUNNING.job_id
            INNER JOIN
                ...
                sja.start_execution_date = (
                       ...
                       FROM msdb..sysjobactivity AS sja2
                       WHERE sja.job_id = sja2.job_id
                   )
                   AND
                --not finished
            ...
            LEFT OUTER JOIN (
                ...
                SELECT I.job_id,
                       ...
                       CONVERT (INT, AVG (I.run_duration_in_secs) / 60) AS "avg_duration_in_mins"
                FROM (
                    ...
                    SELECT JH.job_id,
                           ...
                           msdb.dbo.agent_datetime (JH.run_date, JH.run_time) AS "date_executed",
                           ...
                           + (JH.run_duration % 100) AS "run_duration_in_secs"
                    FROM msdb..sysjobhistory AS JH
                    INNER JOIN
                        --currently running jobs
                        @temp_xp_sqlagent_enum_jobs AS CURRENTLY_RUNNING
                        ON JH.job_id = CURRENTLY_RUNNING.job_id
                ...
                ON j.job_id = AVERAGES.job_id
            GROUP BY j.job_id,
                     j.name
            HAVING --is running
                MAX (CURRENTLY_RUNNING.job_state) = 1
                AND MAX (sja.start_execution_date) IS NOT NULL
                AND
...
SET @TableHtml = N'<table><tr><th>Job</th><th>Started</th><th>Running for (minutes)</th><th>Average duration (minutes)</th></tr>'
                 + @TableHtml + N'</table>';

--set e-mail body, inserting table HTML
SET @EmailBody = N'<body><p>The following jobs have been running for more than 30 minutes on SQL Server '
                 + @@SERVERNAME + N' and should be investigated:</p>' + @TableHtml
...
EXEC msdb.dbo.sp_send_dbmail @recipients = 'mboomaars@gmail.com',
