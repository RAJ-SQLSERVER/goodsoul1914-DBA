...
SELECT InstanceName = @InstanceName,
       ...
       SQLServerDataType = CASE
                               ...
                               WHEN st.name IN ( 'binary', 'char', 'text', 'varbinary', 'varchar' )
                                    AND c.max_length > -1 THEN
                                   st.name + '(' + CONVERT(VARCHAR(6), c.max_length) + ')'
                               --Unicode fields require twice the storage as standard variable length fields
                               WHEN st.name IN ( 'nchar', 'ntext', 'nvarchar' )
                                    AND c.max_length > -1 THEN
                                   st.name + '(' + CONVERT(VARCHAR(6), c.max_length / 2) + ')'
                               --Scale based lengths for date and time fields
                               WHEN st.name IN ( 'datetime2', 'datetimeoffset', 'time' ) THEN
                                   st.name + '(' + CONVERT(VARCHAR(3), c.scale) + ')'
                               --Numeric fields with precision and scale
                               WHEN st.name IN ( 'decimal', 'numeric' ) THEN
                                   st.name + '(' + CONVERT(VARCHAR(3), c.precision) + ', '
                                   + CONVERT(VARCHAR(3), c.scale) + ')'
                               --Large Object types (LOB)
                               WHEN st.name NOT IN ( 'geography', 'geometry', 'xml' )
                                    AND c.max_length = -1 THEN
                                   st.name + '(MAX)'
                               --Everything else
                               ELSE
...
WHERE o.type_desc NOT IN ( 'INTERNAL_TABLE', 'SYSTEM_TABLE' )
      AND s.name NOT IN ( 'cdc', 'sys' )
      AND o.name NOT LIKE 'MSp%'
      AND o.name NOT LIKE 'sys%'
      AND o.name NOT LIKE 'SYNC_%'
ORDER BY CASE o.type_desc
             WHEN 'USER_TABLE' THEN
                 1
             WHEN 'VIEW' THEN
                 2
             WHEN 'SQL_INLINE_TABLE_VALUED_FUNCTION' THEN
                 3
             WHEN 'SQL_TABLE_VALUED_FUNCTION' THEN
                 4
             ELSE
                 5
         END,
