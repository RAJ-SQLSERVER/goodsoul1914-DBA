...
IF EXISTS (
    ...
    WHERE AC.object_id = OBJECT_ID (N'sys.indexes', N'V')
          AND AC.name = N'has_filter'
)
BEGIN

    INSERT INTO @Index_Filters (object_id, index_id, filter_definition)
    EXECUTE (N'
				SELECT
					 I.[object_id]
					,I.index_id
...
SELECT sqI.object_id,
       ...
       RANK () OVER (ORDER BY sqI.object_id,
                              CONVERT (NVARCHAR(MAX), sqI.index_key),
                              sqI.filter_definition
               ) AS "dupe_rank"
INTO dbo.#temp_index_breakdown_keys_filters
FROM (
    SELECT I.object_id,
           I.index_id,
           (
               SELECT N', ' + C.name + N' • '
                      ...
                      + (CASE
                             WHEN LOWER (TYPE_NAME (C.system_type_id)) IN ( N'nchar', N'ntext', N'nvarchar' ) THEN
                                 CONVERT (VARCHAR(6), C.max_length / 2)
                             WHEN LOWER (TYPE_NAME (C.system_type_id)) NOT IN ( N'bigint', N'bit', N'date',
                                                                                N'datetime', N'datetime2',
                                                                                N'datetimeoffset', N'decimal',
                                                                                N'float', N'int', N'money', N'numeric',
                                                                                N'real', N'smalldatetime', N'smallint',
                                                                                N'smallmoney', N'time', N'tinyint'
                             ) THEN
                                 CONVERT (VARCHAR(6), C.max_length)
                             ELSE
                                 CONVERT (VARCHAR(6), C.max_length) + ' ('
                                 + CONVERT (VARCHAR(11), COLUMNPROPERTY (C.object_id, C.name, 'Precision')) + ','
                                 + ISNULL (CONVERT (VARCHAR(11), COLUMNPROPERTY (C.object_id, C.name, 'Scale')), 0)
                                 + ')'
                         END
                        ) + N' ' + (CASE
           ...
           (
               SELECT N', ' + C.name + N' • ' + TYPE_NAME (C.user_type_id)
                      + ISNULL (
                            (N': [ '
                             + (CASE
                      ...
                      + (CASE
                             WHEN TY.name NOT IN ( N'bigint', N'bit', N'date', N'datetime', N'datetime2',
                                                   N'datetimeoffset', N'decimal', N'float', N'int', N'money',
                                                   N'numeric', N'real', N'smalldatetime', N'smallint', N'smallmoney',
                                                   N'time', N'tinyint'
                             ) THEN CONVERT (NVARCHAR(30), C.max_length)
                             ELSE
                                 CONVERT (NVARCHAR(30), C.max_length) + N' ('
                                 + CONVERT (NVARCHAR(30), COLUMNPROPERTY (C.object_id, C.name, 'Precision')) + N','
                                 + ISNULL (CONVERT (NVARCHAR(30), COLUMNPROPERTY (C.object_id, C.name, 'Scale')), 0)
                                 + N')'
                         END
                        ) AS "text()"
               FROM sys.index_columns AS IC
    ...
    LEFT JOIN @Index_Filters AS tvIF
        ON tvIF.object_id = I.object_id
...
SELECT DDPS.object_id,
       DDPS.index_id,
       SUM (CASE WHEN DDPS.index_id < 2 THEN DDPS.row_count END) AS "rows",
       SUM (DDPS.reserved_page_count) AS "total_pages",
       SUM (DDPS.used_page_count) AS "used_pages",
       SUM (
           CASE
               WHEN DDPS.index_id < 2 THEN
                   DDPS.in_row_data_page_count + DDPS.lob_used_page_count + DDPS.row_overflow_used_page_count
               ELSE DDPS.lob_used_page_count + DDPS.row_overflow_used_page_count
           END
       ) AS "data_pages"
INTO dbo.#temp_index_breakdown_size_info
...
SELECT (CASE WHEN sqBAQ.row_filter = 1 THEN sqBAQ.type ELSE '' END) AS "object_type",
       (CASE WHEN sqBAQ.row_filter = 1 THEN DB_NAME () ELSE '' END) AS "database_name",
       (CASE
       ...
       (CASE WHEN sqBAQ.row_filter = 1 THEN sqBAQ.create_date ELSE '' END) AS "create_date",
       (CASE WHEN sqBAQ.row_filter = 1 THEN caMDKL.modify_date ELSE '' END) AS "modify_date",
       (CASE
            WHEN sqBAQ.row_filter = 1 THEN
                REVERSE (SUBSTRING (REVERSE (CONVERT (VARCHAR(23), CONVERT (MONEY, sqBAQ.rows), 1)), 4, 23))
            ELSE ''
        END
       ) AS "rows",
       (CASE
       ...
       (CASE
            WHEN sqBAQ.row_filter = 1 THEN
                CONVERT (
                    VARCHAR(6), CONVERT (DECIMAL(5, 2), ISNULL (((sqBAQ.data_pages + .0) / sqBAQ.used_pages) * 100, 0))
                )
            ELSE ''
        END
       ) AS "pct_data",
       (CASE
            WHEN sqBAQ.row_filter = 1 THEN
                CONVERT (
                    VARCHAR(6),
                    CONVERT (DECIMAL(5, 2), ISNULL (((sqBAQ.index_pages + .0) / sqBAQ.used_pages) * 100, 0))
                )
            ELSE ''
        END
       ) AS "pct_index",
       ...
       (CASE
            ...
            WHEN sqBAQ.is_primary_key = 0
                 AND sqBAQ.is_unique = 0 THEN REPLICATE ('.', 6)
            WHEN sqBAQ.is_system_named = 0 THEN 'No'
            WHEN sqBAQ.is_system_named = 1 THEN 'Yes'
            ELSE ''
        END
       ) AS "system_named",
       (CASE WHEN sqBAQ.is_primary_key = 1 THEN 'Yes' ELSE '' END) AS "is_pk",
       (CASE
       ...
       (CASE WHEN sqBAQ.is_disabled = 1 THEN 'Yes' ELSE '' END) AS "disabled",
       (CASE WHEN sqBAQ.is_hypothetical = 1 THEN 'Yes' ELSE '' END) AS "hypothetical",
       ...
       (CASE
            WHEN sqBAQ.type_desc = N'HEAP' THEN ''
            ELSE
                REVERSE (SUBSTRING (REVERSE (CONVERT (VARCHAR(23), CONVERT (MONEY, caMDKL.include_columns), 1)), 4, 23))
        END
       ) AS "include_columns",
       (CASE
            WHEN sqBAQ.type_desc = N'HEAP' THEN ''
        ...
        END
       ) AS "index_pct_of_columns",
       (CASE
            WHEN sqBAQ.type_desc = N'HEAP' THEN ''
            ELSE CONVERT (VARCHAR(23), CONVERT (MONEY, (caMDKL.include_columns / sqCC.total_columns) * 100), 1)
        END
       ) AS "include_pct_of_columns",
       (CASE
            WHEN sqBAQ.type_desc = N'HEAP' THEN ''
            ELSE
                CONVERT (
                    VARCHAR(23),
                    CONVERT (MONEY, ((caMDKL.index_columns + caMDKL.include_columns) / sqCC.total_columns) * 100),
                    1
                )
        END
       ) AS "total_pct_of_columns",
       CONVERT (VARCHAR(23), CONVERT (MONEY, (ISNULL (sqBAQ.individual_index_pages, 0) * 8) / 1024.0), 1) AS "key_mb",
       CONVERT (
           VARCHAR(6),
           CONVERT (DECIMAL(5, 2), ISNULL (((sqBAQ.individual_index_pages + .0) / sqBAQ.index_pages) * 100, 0))
       ) AS "key_mb_pct",
       (CASE
            WHEN sqBAQ.type_desc = N'HEAP' THEN ''
            ELSE
                REVERSE (SUBSTRING (REVERSE (CONVERT (VARCHAR(23), CONVERT (MONEY, sqKL.total_max_length), 1)), 4, 23))
        END
       ) AS "max_key_size_bytes",
       (CASE WHEN sqKL.total_max_length > 900 THEN 'Yes' ELSE '' END) AS "over_key_size_limit",
       ...
       (CASE
            WHEN sqED02.dupe_id IS NOT NULL THEN
                CONVERT (VARCHAR(20), sqED02.dupe_id) + ' - [' + CONVERT (VARCHAR(11), sqED02.total_dupes) + ']'
            ELSE ''
        END
       ) AS "dupe_id",
       ...
       (CASE WHEN sqBAQ.fill_factor = 0 THEN 100 ELSE sqBAQ.fill_factor END) AS "fill_factor",
       (CASE
...
FROM (
    ...
    INNER JOIN (
        ...
        FROM dbo.#temp_index_breakdown_size_info AS ttIBSI
        GROUP BY ttIBSI.object_id
    ) AS sqDDPS
        ON sqDDPS.object_id = O.object_id
    INNER JOIN (
        SELECT I.object_id,
               I.type_desc,
               I.name,
               ...
               (CASE
                    ...
                    WHEN DDIUS.object_id IS NOT NULL THEN
               (CASE
               ...
               (CASE
                    ...
                    ELSE ROW_NUMBER () OVER (PARTITION BY I.object_id
                                             ORDER BY I.is_primary_key DESC,
                                                      (CASE WHEN I.type = 0 THEN 'Z' ELSE 'A' END),
                                                      I.type,
                                                      I.name
                                       )
                END
        ...
        FROM sys.indexes AS I
        LEFT JOIN dbo.#temp_index_breakdown_size_info AS ttIBSI
            ON ttIBSI.object_id = I.object_id
        ...
        LEFT JOIN master.sys.dm_db_index_usage_stats AS DDIUS
            ON DDIUS.object_id = I.object_id
               AND DDIUS.index_id = I.index_id
               AND DDIUS.database_id = @Database_ID
    ) AS sqI
        ON sqI.object_id = O.object_id
    WHERE O.type IN ( 'U', 'V' )
...
INNER JOIN (
    ...
    ON sqCC.object_id = sqBAQ.object_id
LEFT JOIN dbo.#temp_index_breakdown_keys_filters AS ttIBKF
    ON ttIBKF.object_id = sqBAQ.object_id
...
LEFT JOIN (
    ...
    ON sqKL.object_id = sqBAQ.object_id
       AND sqKL.index_id = sqBAQ.index_id
LEFT JOIN (
    SELECT sqED01.dupe_rank,
           sqED01.total_dupes,
           ROW_NUMBER () OVER (ORDER BY (SELECT NULL)) AS "dupe_id"
    FROM (
        SELECT ttIBKF.dupe_rank,
               COUNT (*) AS "total_dupes"
        FROM dbo.#temp_index_breakdown_keys_filters AS ttIBKF
        GROUP BY ttIBKF.dupe_rank
        HAVING COUNT (*) > 1
    ) AS sqED01
) AS sqED02
    ON sqED02.dupe_rank = ttIBKF.dupe_rank
CROSS APPLY (
    SELECT (CASE
