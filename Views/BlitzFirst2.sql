...
CREATE VIEW dbo.BlitzFirstPerfmonStats_Deltas2
AS
WITH RowDates
AS (
SELECT   ROW_NUMBER() OVER (ORDER BY ServerName, CheckDate) AS "ID",
         CheckDate
FROM     dbo.BlitzFirstPerfmonStats
GROUP BY ServerName,
         CheckDate
),
     CheckDates
...
JOIN   RowDates AS LastDate
    ON ThisDate.ID = LastDate.ID + 1
)
SELECT     pMon.ServerName,
           pMon.CheckDate,
           pMon.object_name,
           pMon.counter_name,
           pMon.instance_name,
...
FROM       dbo.BlitzFirstPerfmonStats AS pMon
INNER JOIN CheckDates AS Dates
    ON Dates.CheckDate = pMon.CheckDate
JOIN       dbo.BlitzFirstPerfmonStats AS pMonPrior
...
WHERE      DATEDIFF(MI, pMonPrior.CheckDate, pMon.CheckDate)
BETWEEN    1 AND 60;
GO

CREATE VIEW dbo.BlitzFirstPerfmonStats_Actuals2
AS
WITH PERF_AVERAGE_BULK
AS (
SELECT ServerName,
       object_name,
       instance_name,
       counter_name,
       CASE
           WHEN CHARINDEX('(', counter_name) = 0 THEN
               counter_name
           ELSE
               LEFT(counter_name, CHARINDEX('(', counter_name) - 1)
       END AS "counter_join",
       CheckDate,
       cntr_delta
FROM   dbo.BlitzFirstPerfmonStats_Deltas2
...
JOIN   PERF_LARGE_RAW_BASE AS DEN
    ON NUM.counter_join = DEN.counter_join
...
CREATE VIEW dbo.BlitzFirstFileStats_Deltas2
AS
WITH RowDates
AS (
SELECT   ROW_NUMBER() OVER (ORDER BY ServerName, CheckDate) AS "ID",
         CheckDate
FROM     dbo.BlitzFirstFileStats
GROUP BY ServerName,
         CheckDate
),
     CheckDates
...
SELECT     f.ServerName,
           ...
           CASE
               WHEN (f.num_of_reads - fPrior.num_of_reads) = 0 THEN
                   0
               ELSE
           (f.io_stall_read_ms - fPrior.io_stall_read_ms) / (f.num_of_reads - fPrior.num_of_reads)
           END AS "io_stall_read_ms_average",
           ...
           CASE
               WHEN (f.num_of_writes - fPrior.num_of_writes) = 0 THEN
                   0
               ELSE
           (f.io_stall_write_ms - fPrior.io_stall_write_ms) / (f.num_of_writes - fPrior.num_of_writes)
           END AS "io_stall_write_ms_average",
...
FROM       dbo.BlitzFirstFileStats AS f
INNER JOIN CheckDates AS DATES
    ON f.CheckDate = DATES.CheckDate
INNER JOIN dbo.BlitzFirstFileStats AS fPrior
    ON f.ServerName = fPrior.ServerName
...
WHERE      f.num_of_reads >= fPrior.num_of_reads
           ...
           AND DATEDIFF(MI, fPrior.CheckDate, f.CheckDate)
           BETWEEN 1 AND 60;
GO

CREATE VIEW dbo.BlitzFirstWaitStats_Deltas2
AS
WITH RowDates
AS (
SELECT   ROW_NUMBER() OVER (ORDER BY ServerName, CheckDate) AS "ID",
         CheckDate
FROM     dbo.BlitzFirstWaitStats
GROUP BY ServerName,
         CheckDate
),
     CheckDates
...
FROM            dbo.BlitzFirstWaitStats AS w
INNER JOIN      CheckDates AS Dates
    ON Dates.CheckDate = w.CheckDate
INNER JOIN      dbo.BlitzFirstWaitStats AS wPrior
    ON w.ServerName = wPrior.ServerName
       AND w.wait_type = wPrior.wait_type
       AND Dates.PreviousCheckDate = wPrior.CheckDate
LEFT OUTER JOIN dbo.BlitzFirstWaitStats_Categories AS wc
    ON w.wait_type = wc.WaitType
WHERE           DATEDIFF(MI, wPrior.CheckDate, w.CheckDate)
                BETWEEN 1 AND 60
